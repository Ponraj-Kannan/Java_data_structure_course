...
const CODES = {
  java: [
    ...
    ['c_setnull', '        this.next = null;'], ['', '    }'], ['', '}'], ['', ''],
    ['', 'class Queue {'], ['c_sethead', '    Node front, rear;'], ['c_sethead', '    int size;'], ['', ''],
    ['c_enqueue', '    void enqueue(int data) {'],
    ['c_nodeNew', '        Node newNode = new Node(data);'],
    ['c_setnull', '        newNode.next = null;'],
  ...
  python: [
    ...
    ['c_nodeNew', '        self.data = data'], ['c_setnull', '        self.next = None'], ['', ''],
    ['', 'class Queue:'], ['c_sethead', '    def __init__(self):'],
  ...
  javascript: [
    ...
    ['c_nodeNew', '    this.data = data;'], ['c_setnull', '    this.next = null;'],
    ['', '  }'], ['', '}'], ['', ''],
    ['', 'class Queue {'], ['c_sethead', '  constructor() {'],
...
function buildSteps(values) {
  ...
  values.forEach((v) => {
    ...
    steps.push({
      ...
      badge: 'create newNode { data: ' + v + ', next: null } @' + newAddr, code: 'c_nodeNew',
  ...
  steps.push({
    ...
    badge: nodes.length
      ? 'All enqueue operations complete \u2192 queue (front \u2192 rear): ' + nodes.map((n) => n.val).join(', ')
                ...
                <div class="ll-nwrap">
                  ...
                  <div class="ll-node">
                    ...
                    >
                      ...
                      <div class="ll-ptr">
                        <small>next</small>{{ idx < s.nodes.length - 1 ? fmt(s.nodes[idx + 1].addr) : (s.pendVal !== undefined && s.pendLinked ? fmt(s.neuAddr) : 'null') }}
                      </div>
                    </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm — Enqueue</h3>
            <p>
              Enqueue inserts a new node at the <code>rear</code> of the queue. Because the queue keeps a direct
              <code>rear</code> pointer, no traversal is needed — unlike inserting at the end of a plain
              singly linked list. If the queue was empty, the new node becomes both <code>front</code> and <code>rear</code>.
            </p>
            ...
            <ol>
              <li>Enqueue always operates at the <code>rear</code> end; Dequeue always operates at the <code>front</code> end — this separation is what makes both O(1).</li>
              <li>When the queue is empty, <code>front</code> and <code>rear</code> must both be updated to the new node.</li>
              <li>A linked-list queue grows one node at a time and dynamically allocates memory as needed — there is no fixed-size buffer to manage or resize.</li>
            </ol>
            <h3>Edge Cases</h3>
            <ol>
              <li><b>Empty queue:</b> first enqueue sets <code>front = rear = newNode</code>.</li>
              <li><b>Multiple insertions:</b> each call links <code>rear.next</code> then advances <code>rear</code>.</li>
            </ol>
            <p class="ll-note">
              Note: unlike an array-based queue, a linked-list queue has <b>no fixed capacity</b> and therefore no
              <code>isFull()</code> check — it can keep growing until system memory is exhausted (a boundary condition
              rather than a design limit). <b>Queue Underflow</b> (removing from an empty queue) is a
