...
function buildSteps(values) {
  ...
  steps.push({
    ...
    badge: nodes.length
      ? 'Values enqueued in order: ' + values.join(', ') + ' \u2192 queue (front \u2192 rear): ' + nodes.map((n) => n.val).join(', ')
  ...
  steps.push({
    ...
    badge: 'return size \u2192 no traversal from front to rear is needed to answer this', code: 'c_return',
      ...
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            ...
            <div v-if="s.reading" class="ll-size-banner">no nodes are visited — size is a maintained counter, not computed by traversal</div>
          </div>
        </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm — Size</h3>
            <p>
              Size reports how many elements are currently in the queue. A well-implemented linked-list queue
              keeps a dedicated <code>size</code> field on the queue object, incrementing it inside
              <code>enqueue()</code> and decrementing it inside <code>dequeue()</code>. Because that bookkeeping
              is done as each element is added or removed at the <code>rear</code> or <code>front</code>,
              <code>size()</code> itself does nothing more than return the already-current value of that field —
              it never has to walk the list from <code>front</code> to <code>rear</code>.
            </p>
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Best / Average / Worst</td><td>O(1)</td><td>the <code>size</code> field is read directly — no traversal from <code>front</code> to <code>rear</code></td></tr>
            ...
            <ol>
              ...
              <li>A queue implementation that skips the counter field would instead need to traverse from <code>front</code> to <code>null</code>, counting nodes as it goes — that alternative is O(n) instead of O(1).</li>
              <li>Size is a read-only operation: it never changes <code>front</code>, <code>rear</code>, or <code>size</code> itself, and never touches any node's <code>next</code> pointer.</li>
            </ol>
            <h3>Edge Cases</h3>
            <ol>
            ...
            <p class="ll-note">
              ...
              core queue operations to reason about.
            </p>
          </div>
        </div>
      </div>
