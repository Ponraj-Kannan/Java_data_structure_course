...
const PSEUDOCODE = [
  ...
  '        return null',
  '    return front.data   // no removal, queue unchanged',
];
...
function buildSteps(values) {
  ...
  steps.push({
    ...
    badge: nodes.length
      ? 'Initial queue (front \u2192 rear): ' + nodes.map((n) => n.val).join(', ')
  ...
  if (nodes.length === 0) {
    ...
    steps.push({
      nodes: [...nodes], frontAddr, rearAddr, size, curAddr: frontAddr, peekVal: nodes[0].val,
      badge: 'front() complete \u2192 queue UNCHANGED (still front \u2192 rear: ' + nodes.map((n) => n.val).join(', ') + ')', code: 'c_front', done: true,
      vars: [mainFrame()],
    });
  }
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm — Front (Peek)</h3>
            <p>
              Front (also called Peek) returns the data stored at the <code>front</code> node <b>without removing it</b>.
              The queue's structure, size, and both pointers remain completely unchanged — this is what
              distinguishes Front from Dequeue.
            </p>
            ...
            <ol>
              <li>Front is a <b>read-only</b> operation — it never mutates <code>front</code>, <code>rear</code>, or <code>size</code>.</li>
              <li>It is commonly used before a <code>dequeue()</code> call to inspect the value that would be removed.</li>
              <li>Because the queue keeps a direct <code>front</code> pointer, this is O(1) regardless of queue length.</li>
            </ol>
            <h3>Edge Cases</h3>
            <ol>
              ...
              <li><b>Single-node queue:</b> <code>front</code> and <code>rear</code> point to the same node; peeking returns that node's data.</li>
              <li><b>Repeated calls:</b> calling <code>front()</code> multiple times in a row always returns the same value, since nothing is removed.</li>
            </ol>
            <p class="ll-note">
