...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int top;'],
    ['', '};'], ['', ''],
    ['c_size', 'int size(struct Stack *s) {'],
    ['c_calc', '    return s->top + 1;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Stack {'],
...
//   'cur'    - every occupied slot (0..top), highlighted together while
//              the size calculation is explained - this is the set of
//              cells that top + 1 is counting
function buildSteps(capacity, values) {
  ...
  pushStep({
    badge: 'Initial stack \u2014 size ' + sizeVal + ' / capacity ' + capacity + ' \u2014 ' + fnLabel + ' called',
    code: 'c_size',
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]])],
  });

  // --- return top + 1: highlight every occupied cell together - these
  // are exactly the elements the calculation is counting.
  for (let i = 0; i <= topIdx; i++) stateAt[i] = 'cur';
  pushStep({
    badge: sizeVal === 0
      ? 'size = top + 1 = -1 + 1 = 0 \u2192 the stack is empty, nothing is occupied'
      : 'size = top + 1 = ' + topIdx + ' + 1 = ' + sizeVal + ' \u2192 indices 0 through ' + topIdx + ' are occupied',
    code: 'c_calc',
    result: sizeVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [['top + 1', String(sizeVal), true]])],
  });

  for (let i = 0; i <= topIdx; i++) stateAt[i] = 'normal';
  pushStep({
    badge: fnLabel + ' returns ' + sizeVal + ' \u2014 current stack size is ' + sizeVal + ' / ' + capacity,
    code: 'c_size',
    result: sizeVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>counted by top + 1</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty stack</td><td>O(1)</td><td>a single addition, <code>-1 + 1</code>, gives 0 directly</td></tr>
                <tr><td>Non-empty stack (any size)</td><td>O(1)</td><td>same single addition &mdash; no counting or scanning of the array</td></tr>
                <tr><td>Full stack</td><td>O(1)</td><td>still just <code>top + 1</code> &mdash; the calculation never depends on capacity</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only the computed integer is returned &mdash; nothing is allocated</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: because <code>top</code> is always kept in sync with the number of
              elements actually present, the size never needs to be counted by scanning the array
              &mdash; it falls straight out of the pointer's current value. This is what makes size an
              O(1) operation, unlike a plain array where you might otherwise need to track a separate
              count or scan for the last non-empty slot.
            </p>
            <h3>Advantages</h3>
            <p>Instant, constant-time access to how many elements are currently stored, with no extra counter variable needed &mdash; <code>top</code> already carries that information.</p>
            <h3>Limitations</h3>
            <p>It reports the size at this exact moment only; if the stack changes right after the call (another push or pop), the value must be recomputed &mdash; it is not automatically kept up to date for the caller.</p>
            <h3>Key Observations</h3>
            <p>Size and <code>isFull</code> are two sides of the same relationship: the stack is full exactly when <code>size() == capacity</code>, i.e. when <code>top + 1 == capacity</code>.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty stack:</b> <code>top == -1</code>, so size correctly returns <code>0</code> with no cells highlighted. <b>Full stack:</b> every cell from <code>0</code> to <code>capacity - 1</code> is highlighted together, and size returns exactly <code>capacity</code>. <b>Capacity 0:</b> there are no cells at all, and size is trivially <code>0</code>.</p>
          </div>
        </div>
      </div>
    </div>
