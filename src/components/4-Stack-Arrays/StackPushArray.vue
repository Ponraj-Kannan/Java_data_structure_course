...
const CODES = {
  java: [
    ...
    ['c_ctor', '        top = -1;'],
    ['c_ctor', '    }'], ['', ''],
    ['c_push', '    void push(int value) {'],
    ['c_overflow', '        if (top == capacity - 1) {'],
    ['c_overflow', '            System.out.println("Stack Overflow");'],
    ['c_overflow', '            return;'],
    ['c_overflow', '        }'],
    ['c_inc', '        top = top + 1;'],
    ['c_insert', '        arr[top] = value;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['', '#define MAX 100'], ['', ''],
    ...
    ['c_fields', '    int top;'],
    ['', '};'], ['', ''],
    ['c_push', 'void push(struct Stack *s, int value) {'],
    ['c_overflow', '    if (s->top == s->capacity - 1) {'],
    ['c_overflow', '        printf("Stack Overflow\\n");'],
    ['c_overflow', '        return;'],
    ['c_overflow', '    }'],
    ['c_inc', '    s->top = s->top + 1;'],
    ['c_insert', '    s->arr[s->top] = value;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Stack {'],
    ...
    ['c_push', '    void push(int value) {'],
    ['c_overflow', '        if (top == capacity - 1) {'],
    ['c_overflow', '            cout << "Stack Overflow" << endl;'],
    ['c_overflow', '            return;'],
    ['c_overflow', '        }'],
    ['c_inc', '        top = top + 1;'],
    ['c_insert', '        arr[top] = value;'],
  ...
  python: [
    ...
    ['c_ctor', '        self.top = -1'], ['', ''],
    ['c_push', '    def push(self, value):'],
    ['c_overflow', '        if self.top == self.capacity - 1:'],
    ['c_overflow', '            print("Stack Overflow")'],
    ['c_overflow', '            return'],
    ['c_inc', '        self.top = self.top + 1'],
    ['c_insert', '        self.arr[self.top] = value'],
  ],
  javascript: [
    ['', 'class Stack {'],
    ...
    ['c_ctor', '  }'], ['', ''],
    ['c_push', '  push(value) {'],
    ['c_overflow', '    if (this.top === this.capacity - 1) {'],
    ['c_overflow', '      console.log("Stack Overflow");'],
    ['c_overflow', '      return;'],
    ['c_overflow', '    }'],
    ['c_inc', '    this.top = this.top + 1;'],
    ['c_insert', '    this.arr[this.top] = value;'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'empty'  - unused slot beyond the current top, drawn dashed and blank
//   'normal' - an occupied, settled slot (includes the pushed value once
//              the operation has fully completed)
//   'cur'    - the slot top has just moved onto, before a value is written
//   'new'    - the slot that just received the pushed value, until the
//              operation finishes
function buildSteps(capacity, values, pushValue) {
  const steps = [];
  const phaseStarts = { push: 0 };
  ...
  if (topIdx === capacity - 1) {
    pushStep({
      badge: 'top == capacity - 1 \u2192 Stack Overflow, ' + pushValue + ' cannot be inserted',
      code: 'c_overflow', invalid: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [['value', String(pushValue), true]])],
    });
    return { steps, phaseStarts };
  }

  pushStep({
    badge: 'top != capacity - 1 \u2192 stack is not full, proceed',
    code: 'c_overflow',
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [['value', String(pushValue), true]])],
  });

  // --- top = top + 1: the pointer moves onto the next free slot first.
  // Nothing has been written there yet - it is only previewed (dashed).
  topIdx = topIdx + 1;
  stateAt[topIdx] = 'cur';
  pushStep({
    badge: 'top = top + 1 \u2192 top now points to index ' + topIdx + ' (still empty)',
    code: 'c_inc',
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [['value', String(pushValue), true]])],
  });

  // --- arr[top] = value: the value is actually written into that slot.
  valAt[topIdx] = pushValue;
  stateAt[topIdx] = 'new';
  pushStep({
    badge: 'arr[top] = ' + pushValue + ' \u2192 index ' + topIdx + ' now holds ' + pushValue,
    code: 'c_insert',
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [['value', String(pushValue), true]])],
  });

  stateAt[topIdx] = 'normal';
  pushStep({
    badge: fnLabel + ' complete \u2014 stack size now ' + (topIdx + 1) + ' / ' + capacity,
    code: 'c_push',
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]])],
  });
...
function applyInput() {
  ...
  if (isNaN(val)) { alert('Enter a valid number to push.'); return; }
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>top's target slot</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-new"></span>newly pushed value</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty stack / first push</td><td>O(1)</td><td>just increment <code>top</code> from -1 to 0 and write the value</td></tr>
                <tr><td>Normal push</td><td>O(1)</td><td>direct write to <code>arr[top]</code> &mdash; no traversal or shifting</td></tr>
                <tr><td>Full stack (overflow)</td><td>O(1)</td><td>a single comparison detects it, nothing is written</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is allocated per push &mdash; the array is pre-allocated once</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: because a push always happens at the <code>top</code> end and the array
              already has a reserved slot waiting for it, no elements ever need to move. This is what
              makes push O(1) on an array-based stack &mdash; unlike inserting into the middle or front
              of a plain array, which would require shifting every following element.
            </p>
            <h3>Advantages</h3>
            <p>Simple to implement, cache-friendly contiguous memory, and O(1) push with no pointer/node overhead per element.</p>
            <h3>Limitations</h3>
            <p>The capacity is fixed at creation time. Once <code>top == capacity - 1</code>, further pushes fail with <b>Stack Overflow</b> until something is popped &mdash; the array cannot silently grow the way a linked-list-based stack can.</p>
            <h3>Key Observations</h3>
            <p><code>top</code> starts at <b>-1</b> for an empty stack, and the highest usable index is always <code>capacity - 1</code>. The current size is always <code>top + 1</code>.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty stack:</b> the very first push sets <code>top</code> from -1 to 0. <b>Full stack:</b> when <code>top == capacity - 1</code>, push must detect overflow and leave the stack completely unchanged &mdash; no write happens and <code>top</code> does not move.</p>
          </div>
        </div>
      </div>
    </div>
