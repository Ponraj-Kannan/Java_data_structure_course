...
const CODES = {
  java: [
    ...
    ['c_underflow', '            System.out.println("Stack is Empty");'],
    ['c_underflow', '            return -1;'],
    ['c_underflow', '        }'],
    ['c_return', '        return arr[top];'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['', '#define MAX 100'], ['', ''],
    ...
    ['c_fields', '    int top;'],
    ['', '};'], ['', ''],
    ['c_peek', 'int peek(struct Stack *s) {'],
    ['c_underflow', '    if (s->top == -1) {'],
    ['c_underflow', '        printf("Stack is Empty\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_return', '    return s->arr[s->top];'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Stack {'],
    ...
    ['c_peek', '    int peek() {'],
    ['c_underflow', '        if (top == -1) {'],
    ['c_underflow', '            cout << "Stack is Empty" << endl;'],
    ['c_underflow', '            return -1;'],
    ['c_underflow', '        }'],
    ['c_return', '        return arr[top];'],
    ['', '    }'], ['', '};'],
  ...
  python: [
    ...
    ['c_underflow', '            print("Stack is Empty")'],
    ['c_underflow', '            return None'],
    ['c_return', '        return self.arr[self.top]'],
  ],
  javascript: [
    ['', 'class Stack {'],
    ...
    ['c_underflow', '      console.log("Stack is Empty");'],
    ['c_underflow', '      return undefined;'],
    ['c_underflow', '    }'],
    ['c_return', '    return this.arr[this.top];'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'cur'    - the top slot, selected and read into `value`; peek never
//              changes the stack, so this is the only transient state -
//              it always settles back to 'normal', never to 'del'
function buildSteps(capacity, values) {
  ...
  if (topIdx === -1) {
    pushStep({
      badge: 'top == -1 \u2192 Stack is Empty, nothing to peek at',
  ...
  // --- return arr[top]: read and report the top element. Nothing about
  // the stack itself changes - this is purely a lookup.
  const peekVal = valAt[topIdx];
  stateAt[topIdx] = 'cur';
  pushStep({
    badge: 'return arr[top] \u2192 peeks at ' + peekVal + ' from index ' + topIdx + ' without removing it',
    code: 'c_return',
    peekVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [['value', String(peekVal), true]])],
  });

  stateAt[topIdx] = 'normal';
  pushStep({
    badge: fnLabel + ' complete \u2014 top element is ' + peekVal + ', stack unchanged (size still ' + (topIdx + 1) + ')',
    code: 'c_peek',
    peekVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.peekVal !== undefined" class="ll-ptr-chip">
                value = <b class="ll-c-red">{{ s.peekVal }}</b>
              </div>
            </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>top &mdash; peeked at, not removed</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty stack</td><td>O(1)</td><td>a single comparison detects it, nothing is read</td></tr>
                <tr><td>Non-empty stack (any size)</td><td>O(1)</td><td>direct read of <code>arr[top]</code> &mdash; no traversal needed</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only the read value is returned &mdash; nothing extra is allocated</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: peek is a pure lookup. Unlike pop, <code>top</code> never moves and no
              slot is cleared &mdash; the array is left in exactly the state it was in before the call,
              which is why peek is safe to call as often as you like without affecting the stack.
            </p>
            <h3>Advantages</h3>
            <p>Lets code inspect the most recent element (e.g. to decide whether to pop it) without any risk of accidentally losing data, in constant time.</p>
            <h3>Limitations</h3>
            <p>Only the topmost element is visible this way &mdash; peek cannot be used to inspect any other element in the stack without first popping everything above it.</p>
            <h3>Key Observations</h3>
            <p>Peek and pop share the same underflow check and read the same index (<code>arr[top]</code>), but peek stops there &mdash; it never touches <code>top</code> itself.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty stack:</b> <code>top == -1</code> means there is nothing to peek at, and the appropriate message is shown with no further reads attempted. <b>Non-empty stack:</b> the stack's contents and <code>top</code> are guaranteed to be identical before and after the call.</p>
          </div>
        </div>
      </div>
    </div>
