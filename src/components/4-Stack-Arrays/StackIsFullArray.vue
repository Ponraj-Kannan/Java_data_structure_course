...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int top;'],
    ['', '};'], ['', ''],
    ['c_isfull', 'int isFull(struct Stack *s) {'],
    ['c_check', '    return s->top == s->capacity - 1;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Stack {'],
  ...
  python: [
    ...
    ['c_check', '        return self.top == self.capacity - 1'],
  ],
  javascript: [
    ['', 'class Stack {'],
    ...
    ['c_ctor', '  }'], ['', ''],
    ['c_isfull', '  isFull() {'],
    ['c_check', '    return this.top === this.capacity - 1;'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'cur'    - the cell used as visual evidence for the check result: the
//              top cell itself when full (no room left), or the very next
//              free slot when not full (proof that room remains)
function buildSteps(capacity, values) {
  ...
  // --- return top == capacity - 1: evaluate the condition. Highlight
  // whichever cell explains the result - the full top cell, or the next
  // free slot proving there is still room.
  const evidenceIdx = full ? topIdx : topIdx + 1;
  if (evidenceIdx >= 0 && evidenceIdx < capacity) stateAt[evidenceIdx] = 'cur';
  pushStep({
    badge: full
      ? (capacity === 0
          ? 'top == capacity - 1 \u2192 true \u2192 capacity is 0, so the stack is trivially Full (no cells exist at all)'
          : 'top == capacity - 1 \u2192 true \u2192 the stack is Full (no free slot left, index ' + topIdx + ' is the last one)')
      : 'top == capacity - 1 \u2192 false (top = ' + topIdx + ') \u2192 the stack is Not Full (index ' + evidenceIdx + ' is still free)',
    code: 'c_check',
    result: full,
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [['top == capacity - 1', String(full), true]])],
  });

  if (evidenceIdx >= 0 && evidenceIdx < capacity) {
    stateAt[evidenceIdx] = full ? 'normal' : undefined; // full: it holds a real value; not full: it was only ever a preview of an empty slot
  }
  pushStep({
    badge: fnLabel + ' returns ' + full + ' \u2014 Stack is ' + (full ? 'Full' : 'Not Full'),
    code: 'c_isfull',
    result: full,
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]])],
  });
...
function cellLabel(cell) {
  const labels = [];
  if (cell.idx === s.value.topIdx && s.value.topIdx !== -1) labels.push('top \u2192');
  if (cell.state === 'cur' && cell.idx !== s.value.topIdx) labels.push('next free \u2192');
  return labels.join(' ');
}
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
          ...
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.result !== undefined" class="ll-ptr-chip">
                isFull = <b :class="s.result ? 'll-c-red' : 'll-c-green'">{{ s.result }}</b>
              </div>
            </div>
                ...
                <div class="ll-arr-cell-wrap">
                  ...
                  <div
                    ...
                    :class="{
                      ...
                      'll-box-pending': cell.state === 'cur' && cell.val === undefined
                    }"
                  >
                    {{ cell.state === 'empty' || cell.val === undefined ? '' : cell.val }}
                  </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>evidence cell (full top, or next free slot)</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Full stack</td><td>O(1)</td><td>a single comparison, <code>top == capacity - 1</code>, is enough</td></tr>
                <tr><td>Not-full stack (any size)</td><td>O(1)</td><td>same single comparison &mdash; no scan of the array is needed</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only a boolean result is produced &mdash; nothing is allocated</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>capacity - 1</code> is always the highest valid index in the array.
              Once <code>top</code> reaches that index, every slot from <code>0</code> to
              <code>capacity - 1</code> is occupied &mdash; there is no index left to increment
              <code>top</code> onto, so any further push must be rejected as a <b>Stack Overflow</b>
              rather than silently writing past the end of the array.
            </p>
            <h3>Advantages</h3>
            <p>A constant-time guard that push relies on before writing, protecting the fixed-size array from being written out of bounds.</p>
            <h3>Limitations</h3>
            <p>It only reports the capacity status right now &mdash; if the stack is popped or pushed afterwards, isFull must be re-checked, since a fixed-capacity array cannot grow to make more room on its own.</p>
            <h3>Key Observations</h3>
            <p>The stack is full precisely when its size, <code>top + 1</code>, equals <code>capacity</code> &mdash; equivalently, when <code>top == capacity - 1</code>. There is no other state in which a push can safely proceed.</p>
            <h3>Edge Cases</h3>
            <p><b>Just reached capacity:</b> after the last possible push, <code>top == capacity - 1</code> and isFull correctly reports <code>true</code>. <b>After popping from a full stack:</b> <code>top</code> drops below <code>capacity - 1</code> and isFull reports <code>false</code> again, freeing up the top slot. <b>Capacity 0:</b> the stack is trivially full (and simultaneously empty) since there is no slot to ever occupy.</p>
          </div>
        </div>
      </div>
    </div>
