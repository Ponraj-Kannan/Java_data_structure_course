...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int top;'],
    ['', '};'], ['', ''],
    ['c_pop', 'int pop(struct Stack *s) {'],
    ['c_underflow', '    if (s->top == -1) {'],
    ['c_underflow', '        printf("Stack Underflow\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_read', '    int value = s->arr[s->top];'],
    ['c_dec', '    s->top = s->top - 1;'],
    ['c_return', '    return value;'],
    ['', '}'],
  ],
  ...
  python: [
    ...
    ['c_ctor', '        self.top = -1'], ['', ''],
    ['c_pop', '    def pop(self):'],
    ['c_underflow', '        if self.top == -1:'],
    ['c_underflow', '            print("Stack Underflow")'],
    ['c_underflow', '            return None'],
    ['c_read', '        value = self.arr[self.top]'],
    ['c_dec', '        self.top = self.top - 1'],
    ['c_return', '        return value'],
  ],
  javascript: [
    ['', 'class Stack {'],
    ...
    ['c_ctor', '  }'], ['', ''],
    ['c_pop', '  pop() {'],
    ['c_underflow', '    if (this.top === -1) {'],
    ['c_underflow', '      console.log("Stack Underflow");'],
    ['c_underflow', '      return undefined;'],
    ['c_underflow', '    }'],
    ['c_read', '    const value = this.arr[this.top];'],
    ['c_dec', '    this.top = this.top - 1;'],
    ['c_return', '    return value;'],
    ['', '  }'], ['', '}'],
  ],
};
...
//              after it has been popped and settled)
//   'normal' - an occupied, settled slot
//   'cur'    - the top slot, selected and read into `value`, still fully
//              part of the stack
//   'del'    - top has already moved past it; it is logically removed
...
function buildSteps(capacity, values) {
  ...
  if (topIdx === -1) {
    pushStep({
      badge: 'top == -1 \u2192 Stack Underflow, nothing to pop',
      code: 'c_underflow', invalid: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [])],
    });
    return { steps, phaseStarts };
  }

  pushStep({
    badge: 'top != -1 \u2192 stack is not empty, proceed',
    code: 'c_underflow',
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [])],
  });

  // --- value = arr[top]: read the top element. Nothing has been removed
  // yet, so the slot stays fully part of the stack - just selected.
  const poppedVal = valAt[topIdx];
  stateAt[topIdx] = 'cur';
  pushStep({
    badge: 'value = arr[top] \u2192 reads ' + poppedVal + ' from index ' + topIdx,
    code: 'c_read',
    poppedVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [['value', String(poppedVal), true]])],
  });

  // --- top = top - 1: the pointer retreats. The old top slot is now
  // logically unreachable (index > top), even though its value briefly
  // lingers on screen to show it hasn't been physically wiped.
  const oldTopIdx = topIdx;
  stateAt[oldTopIdx] = 'del';
  topIdx = topIdx - 1;
  pushStep({
    badge: 'top = top - 1 \u2192 top retreats to ' + topIdx + '; index ' + oldTopIdx + ' is no longer part of the stack',
    code: 'c_dec',
    poppedVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]]), frame(fnLabel, [['value', String(poppedVal), true]])],
  });

  // --- return value: the operation hands the popped value back. Only now
  // does the slot visually clear to an ordinary empty slot.
  delete stateAt[oldTopIdx];
  delete valAt[oldTopIdx];
  pushStep({
    badge: 'return value \u2192 ' + fnLabel + ' returns ' + poppedVal,
    code: 'c_return',
    poppedVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]])],
  });
  ...
  pushStep({
    badge: fnLabel + ' complete \u2014 popped ' + poppedVal + ', stack size now ' + (topIdx + 1) + ' / ' + capacity,
    code: 'c_pop',
    vars: [frame('main()', [['capacity', String(capacity), true], ['top', String(topIdx), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.poppedVal !== undefined" class="ll-ptr-chip">
                value = <b class="ll-c-red">{{ s.poppedVal }}</b>
              </div>
            </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>selected (top), still in the stack</span>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty stack (underflow)</td><td>O(1)</td><td>a single comparison detects it, nothing is read or changed</td></tr>
                <tr><td>Single-element stack</td><td>O(1)</td><td>read <code>arr[0]</code>, then <code>top</code> drops to -1</td></tr>
                <tr><td>Multiple-element stack</td><td>O(1)</td><td>direct read of <code>arr[top]</code> and a decrement &mdash; no traversal or shifting</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only the returned value is held temporarily &mdash; no extra memory is allocated</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: popping only ever touches the <code>top</code> end, so no elements ever
              need to shift. The popped slot's old value can be left in the array untouched &mdash; once
              <code>top</code> moves past it, that slot is simply outside the stack's valid range and will
              be silently overwritten the next time something is pushed.
            </p>
            <h3>Advantages</h3>
            <p>Extremely fast O(1) removal with no pointer/node bookkeeping, and no memory to free since the array is reused in place.</p>
            <h3>Limitations</h3>
            <p>Popping from an empty stack must be explicitly checked (<b>Stack Underflow</b>) &mdash; there is no way to "pop past zero" safely without that guard, since <code>arr[-1]</code> is not a valid index.</p>
            <h3>Key Observations</h3>
            <p>Only the element at index <code>top</code> can ever be popped. After popping, the new <code>top</code> is always <code>old top - 1</code>, and the current size is always <code>top + 1</code>.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty stack:</b> <code>top == -1</code> means underflow &mdash; the stack is left completely unchanged. <b>Single-element stack:</b> popping the only element brings <code>top</code> back to -1, making the stack empty again. <b>Multiple elements:</b> only the topmost value is affected; everything below <code>top</code> stays exactly as it was.</p>
          </div>
        </div>
      </div>
    </div>
