...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int size;'],
    ['', '};'], ['', ''],
    ['c_isfull', 'int isFull(struct PriorityQueue *q) {'],
    ['c_check', '    if (q->size == q->capacity) {'],
    ['c_true', '        return 1;'],
    ['c_check', '    }'],
    ['c_false', '    return 0;'],
    ['', '}'],
  ...
  python: [
    ...
    ['c_check', '        if self.size == self.capacity:'],
    ['c_true', '            return True'],
    ['c_false', '        return False'],
  ],
  javascript: [
    ...
    ['c_ctor', '  }'], ['', ''],
    ['c_isfull', '  isFull() {'],
    ['c_check', '    if (this.size === this.capacity) {'],
    ['c_true', '      return true;'],
    ['c_check', '    }'],
    ['c_false', '    return false;'],
    ['', '  }'], ['', '}'],
...
// though isFull() itself never inspects the array's contents or order.
...
// isFull() never reads or reorders any array slot - it only compares the
// size counter to the capacity counter. Cells are shown purely for context
// (what the queue currently holds), so every occupied slot is just 'normal'
// and every unused slot beyond size is 'empty'; no 'cur'/'shift'/'del'
// states ever occur here.
function buildSteps(capacity, elements) {
  ...
  pushStep({
    badge: 'checking condition: size == capacity \u2192 ' + size + ' == ' + capacity + ' is ' + (size === capacity),
    code: 'c_check',
  ...
  if (size === capacity) {
    pushStep({
      badge: 'size == capacity is true \u2192 queue is full, ' + fnLabel + ' returns true',
      code: 'c_true',
      returnVal: 'true',
      vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]]), frame(fnLabel, [['result', 'true', true]])],
    });
  } else {
    const room = capacity - size;
    pushStep({
      badge: 'size == capacity is false \u2192 queue has room for ' + room + ' more element' + (room === 1 ? '' : 's') + ', ' + fnLabel + ' returns false',
      code: 'c_false',
      returnVal: 'false',
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm</h3>
            <p><code>isFull()</code> never inspects any element in the <code>values</code> or <code>priorities</code> arrays at all &mdash; it only compares the <code>size</code> counter to the <code>capacity</code> counter. If <code>size</code> equals <code>capacity</code>, it returns <code>true</code> (no more elements can be inserted without first extracting one); otherwise it returns <code>false</code>.</p>
            <h3>Time &amp; Space Complexity</h3>
            <table class="ll-complexity-table">
              <thead><tr><th>Case</th><th>Time</th><th>Why</th></tr></thead>
              <tbody>
                <tr><td>Any queue state</td><td>O(1)</td><td>a single comparison between two counters, independent of how many elements are stored</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used &mdash; nothing is read from or written to the arrays</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>isFull()</code> is the mirror image of <code>isEmpty()</code> &mdash; both are O(1) counter comparisons, just checking opposite ends of the same <code>size</code> range (<code>0</code> for empty, <code>capacity</code> for full).
            </p>
            <h3>Advantages</h3>
            <p>Extremely cheap check &mdash; a single comparison, independent of capacity or how many elements are stored. It's commonly used as a guard before calling <code>insert()</code>, which must reject an insertion with <b>Queue Overflow</b> once the array is full.</p>
            <h3>Limitations</h3>
            <p>Provides no information about which elements are present or their priorities &mdash; only whether the queue currently has any free slots left.</p>
            <h3>Queue vs Circular Queue vs Deque vs Priority Queue</h3>
            <p>A <b>Queue</b>, <b>Circular Queue</b>, <b>Deque</b>, and <b>Priority Queue</b> all determine fullness identically: <code>size == capacity</code>. The underlying storage strategy &mdash; linear, circular, double-ended, or sorted-by-priority &mdash; never changes how fullness is checked; it only changes what happens next if you try to insert anyway.</p>
            <h3>Key Observations</h3>
            <p><code>isFull()</code> is typically called defensively before <code>insert()</code> to avoid triggering a Queue Overflow. It has no interaction with the <code>values</code> or <code>priorities</code> arrays, and calling it never changes the queue's state.</p>
            <h3>Edge Cases</h3>
            <p><b>Newly constructed queue</b> (size = 0, capacity &gt; 0): returns false. <b>Queue with exactly one free slot</b>: returns false, becomes true after one more insert. <b>Fully packed queue</b> (size == capacity): returns true. <b>Capacity 0:</b> size starts at 0, which already equals capacity, so <code>isFull()</code> returns true immediately &mdash; a capacity-0 queue is simultaneously empty and full, since no element can ever be inserted into it.</p>
          </div>
        </div>
      </div>
    </div>
