...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int size;'],
    ['', '};'], ['', ''],
    ['c_peek', 'int peek(struct PriorityQueue *q) {'],
    ['c_underflow', '    if (q->size == 0) {'],
    ['c_underflow', '        printf("Queue Underflow\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_return', '    return q->values[0];'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class PriorityQueue {'],
...
//   'cur'    - index 0, the highest-priority element being inspected
function buildSteps(capacity, elements) {
  ...
  function pushStep(opts) {
    steps.push({
      cells: cells(),
      capacity, size,
      peekedVal: opts.peekedVal,
      peekedPri: opts.peekedPri,
      badge: opts.badge,
      code: opts.code,
  ...
  if (size === 0) {
    pushStep({
      badge: 'size == 0 \u2192 Queue Underflow, nothing to peek at',
  ...
  // --- return arr[0]: inspect the highest-priority element. Nothing is
  // removed and no structural change happens to the array.
  stateAt[0] = 'cur';
  const peekedVal = valAt[0];
  const peekedPri = priAt[0];
  pushStep({
    badge: 'arr[0] = (' + peekedVal + ', ' + peekedPri + ') \u2192 highest priority element found, nothing removed',
    code: 'c_return',
    peekedVal, peekedPri,
    vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]]), frame(fnLabel, [['arr[0]', '(' + peekedVal + ', ' + peekedPri + ')', true]])],
  });

  pushStep({
    badge: fnLabel + ' complete \u2014 returns (' + peekedVal + ', ' + peekedPri + '), queue unchanged \u2014 size still ' + size + ' / ' + capacity,
    code: 'c_return',
    peekedVal, peekedPri,
    vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]])],
  });
...
// keeps arr[0] genuinely the highest-priority element, which peek() relies on.
...
function cellLabel(cell) {
  if (cell.state === 'cur') return 'top (highest priority) \u2192';
  return '';
}
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip" v-if="s.peekedVal !== undefined">peeked = <b class="ll-c-orange">({{ s.peekedVal }}, {{ s.peekedPri }})</b></div>
            </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>highest priority (peeked, not removed)</span>
        </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm</h3>
            <p>Because the array is always kept sorted in descending priority order, the highest-priority element is guaranteed to sit at index 0. <code>peek()</code> simply reads that slot and returns it &mdash; no comparisons, no shifting, and no change to <code>size</code> or the array contents.</p>
            <h3>Time &amp; Space Complexity</h3>
            <table class="ll-complexity-table">
              <thead><tr><th>Case</th><th>Time</th><th>Why</th></tr></thead>
              <tbody>
                <tr><td>Empty queue (underflow)</td><td>O(1)</td><td>a single comparison detects it, nothing is read</td></tr>
                <tr><td>Non-empty queue</td><td>O(1)</td><td>direct read of <code>arr[0]</code> &mdash; no traversal needed</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used &mdash; the value is simply returned</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: peek is always O(1) regardless of how many elements are in the queue, because
              maintaining sorted order during insertion is exactly what makes the highest-priority element
              instantly available at index 0.
            </p>
            <h3>Advantages</h3>
            <p>Constant-time access to the highest-priority element with zero risk of accidentally mutating the queue &mdash; useful for decision logic that needs to inspect what would be served next without committing to removing it.</p>
            <h3>Limitations</h3>
            <p>Peek only ever exposes the single highest-priority element; inspecting the second-highest or any other element still requires either <code>extractMax()</code> calls or direct indexing outside the intended API.</p>
            <h3>Queue vs Circular Queue vs Deque vs Priority Queue</h3>
            <p>In a <b>Queue</b>, peek reads <code>arr[front]</code>. In a <b>Circular Queue</b>, peek reads <code>arr[front]</code> with wraparound already accounted for. In a <b>Deque</b>, peek can read either end. In a <b>Priority Queue</b>, peek always reads index 0 &mdash; the highest-priority element &mdash; regardless of insertion order.</p>
            <h3>Key Observations</h3>
            <p>Peek and <code>extractMax()</code> share the same underflow check and read the same slot; the only difference is that <code>extractMax()</code> shifts the array afterward while <code>peek()</code> leaves it completely untouched.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> peek is rejected with Queue Underflow and no value is returned. <b>Non-empty queue:</b> the element at index 0 is returned and the queue remains exactly as it was, ready for the next operation.</p>
          </div>
        </div>
      </div>
    </div>
