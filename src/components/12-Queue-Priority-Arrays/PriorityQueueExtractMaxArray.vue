...
const CODES = {
  java: [
    ...
    ['c_underflow', '        if (size == 0) {'],
    ['c_underflow', '            System.out.println("Queue Underflow");'],
    ['c_underflow', '            return -1;'],
    ['c_underflow', '        }'],
    ['c_read', '        int value = values[0];'],
    ['c_read', '        int priority = priorities[0];'],
    ['c_loop', '        for (int i = 0; i < size - 1; i++) {'],
    ['c_shift', '            values[i] = values[i + 1];'],
    ['c_shift', '            priorities[i] = priorities[i + 1];'],
    ['', '        }'],
    ['c_sizedec', '        size = size - 1;'],
    ['c_return', '        return value;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['', '#define MAX 100'], ['', ''],
    ...
    ['c_fields', '    int size;'],
    ['', '};'], ['', ''],
    ['c_extract', 'int extractMax(struct PriorityQueue *q) {'],
    ['c_underflow', '    if (q->size == 0) {'],
    ['c_underflow', '        printf("Queue Underflow\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_read', '    int value = q->values[0];'],
    ['c_read', '    int priority = q->priorities[0];'],
    ['c_loop', '    for (int i = 0; i < q->size - 1; i++) {'],
    ['c_shift', '        q->values[i] = q->values[i + 1];'],
    ['c_shift', '        q->priorities[i] = q->priorities[i + 1];'],
    ['', '    }'],
    ['c_sizedec', '    q->size = q->size - 1;'],
    ['c_return', '    return value;'],
    ['', '}'],
  ],
  cpp: [
  ...
  python: [
    ...
    ['c_underflow', '        if self.size == 0:'],
    ['c_underflow', '            print("Queue Underflow")'],
    ['c_underflow', '            return None'],
    ['c_read', '        value = self.values[0]'],
    ['c_read', '        priority = self.priorities[0]'],
    ['c_loop', '        for i in range(self.size - 1):'],
    ['c_shift', '            self.values[i] = self.values[i + 1]'],
    ['c_shift', '            self.priorities[i] = self.priorities[i + 1]'],
    ['c_sizedec', '        self.size = self.size - 1'],
    ['c_return', '        return value'],
  ],
  javascript: [
    ['', 'class PriorityQueue {'],
    ...
    ['c_underflow', '    if (this.size === 0) {'],
    ['c_underflow', '      console.log("Queue Underflow");'],
    ['c_underflow', '      return undefined;'],
    ['c_underflow', '    }'],
    ['c_read', '    const value = this.values[0];'],
    ['c_read', '    const priority = this.priorities[0];'],
    ['c_loop', '    for (let i = 0; i < this.size - 1; i++) {'],
    ['c_shift', '      this.values[i] = this.values[i + 1];'],
    ['c_shift', '      this.priorities[i] = this.priorities[i + 1];'],
    ['', '    }'],
    ['c_sizedec', '    this.size = this.size - 1;'],
    ['c_return', '    return value;'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'cur'    - index 0, the highest-priority element being read/removed
//   'shift'  - a slot that has just received a value shifted left from i+1
//   'del'    - the trailing slot left behind after the shift, about to clear
function buildSteps(capacity, elements) {
  const steps = [];
  const phaseStarts = { extractMax: 0 };
  ...
  function pushStep(opts) {
    steps.push({
      cells: cells(),
      capacity, size,
      extractedVal: opts.extractedVal,
      extractedPri: opts.extractedPri,
      badge: opts.badge,
      code: opts.code,
  ...
  if (size === 0) {
    pushStep({
      badge: 'size == 0 \u2192 Queue Underflow, nothing to extract',
  ...
  pushStep({
    badge: 'size != 0 \u2192 queue is not empty, proceed',
    code: 'c_underflow',
    vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]]), frame(fnLabel, [])],
  });

  // --- value = arr[0]: read the highest-priority element. Nothing removed
  // yet, the slot is only selected.
  stateAt[0] = 'cur';
  const extractedVal = valAt[0];
  const extractedPri = priAt[0];
  pushStep({
    badge: 'value = arr[0].value, priority = arr[0].priority \u2192 highest priority element is (' + extractedVal + ', ' + extractedPri + ')',
    code: 'c_read',
    extractedVal, extractedPri,
    vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]]), frame(fnLabel, [['value', String(extractedVal), true], ['priority', String(extractedPri), true]])],
  });

  // --- for i = 0 to size - 2: arr[i] = arr[i + 1]. Every remaining element
  // shifts one slot left so the next-highest priority element lands at
  // index 0, keeping the array compact and sorted.
  for (let i = 0; i < size - 1; i++) {
    stateAt[i + 1] = stateAt[i + 1] === undefined ? 'normal' : stateAt[i + 1];
    const movedVal = valAt[i + 1];
    const movedPri = priAt[i + 1];
    stateAt[i + 1] = 'shift';
    pushStep({
      badge: 'i = ' + i + ' \u2192 comparing/shifting: arr[' + i + '] = arr[' + (i + 1) + ']',
      code: 'c_loop',
      extractedVal, extractedPri,
      vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]]), frame(fnLabel, [['value', String(extractedVal), true], ['priority', String(extractedPri), true], ['i', String(i), true]])],
    });
    valAt[i] = movedVal;
    priAt[i] = movedPri;
    stateAt[i] = 'normal';
    stateAt[i + 1] = 'normal';
    pushStep({
      badge: 'arr[' + i + '] = arr[' + (i + 1) + '] \u2192 index ' + i + ' now holds (' + movedVal + ', ' + movedPri + ')',
      code: 'c_shift',
      extractedVal, extractedPri,
      vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]]), frame(fnLabel, [['value', String(extractedVal), true], ['priority', String(extractedPri), true], ['i', String(i), true]])],
    });
  ...
  // --- the trailing slot (old size - 1) is now a leftover duplicate; mark
  // it for removal before it clears to an ordinary empty slot.
  const trailingIdx = size - 1;
  stateAt[trailingIdx] = 'del';
  pushStep({
    badge: 'loop finished \u2014 index ' + trailingIdx + ' is a leftover duplicate, no longer part of the queue',
    code: 'c_sizedec',
    extractedVal, extractedPri,
    vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]]), frame(fnLabel, [['value', String(extractedVal), true], ['priority', String(extractedPri), true]])],
  });

  delete stateAt[trailingIdx];
  delete valAt[trailingIdx];
  delete priAt[trailingIdx];
  size = size - 1;
  pushStep({
    badge: 'size = size - 1 \u2192 size now ' + size,
    code: 'c_sizedec',
    extractedVal, extractedPri,
    vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]]), frame(fnLabel, [['value', String(extractedVal), true], ['priority', String(extractedPri), true]])],
  });
  ...
  pushStep({
    badge: fnLabel + ' complete \u2014 returns (' + extractedVal + ', ' + extractedPri + '), queue size now ' + size + ' / ' + capacity,
    code: 'c_return',
    extractedVal, extractedPri,
    vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]])],
  });
...
// keeps arr[0] genuinely the highest-priority element, which extractMax()
// relies on removing.
...
function cellLabel(cell) {
  if (cell.state === 'del') return 'removed \u2192';
  if (cell.state === 'cur') return 'highest priority \u2192';
  if (s.value.size > 0 && cell.idx === 0 && cell.state === 'normal') return 'top \u2192';
  return '';
}
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip" v-if="s.extractedVal !== undefined">extracted = <b class="ll-c-orange">({{ s.extractedVal }}, {{ s.extractedPri }})</b></div>
            </div>

            <div class="ll-arr-track">
              <template v-for="cell in s.cells" :key="cell.idx">
                <div class="ll-arr-cell-wrap">
                  <div class="ll-who" :class="{ 'll-c-orange': cell.state === 'cur', 'll-c-purple2': cell.state === 'shift', 'll-c-red': cell.state === 'del' }">{{ cellLabel(cell) }}</div>
                  <div
                    class="ll-arr-box"
                    :class="{
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>highest priority (being extracted)</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-shift"></span>shifting left</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-del"></span>leftover / removed</span>
        </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm</h3>
            <p>Because the array is always kept sorted in descending priority order, the highest-priority element is always at index 0. <code>extractMax()</code> reads that element, then shifts every remaining element one position left to close the gap &mdash; keeping the array compact so the next-highest priority element ends up at index 0.</p>
            <h3>Time &amp; Space Complexity</h3>
            <table class="ll-complexity-table">
              <thead><tr><th>Case</th><th>Time</th><th>Why</th></tr></thead>
              <tbody>
                <tr><td>Empty queue (underflow)</td><td>O(1)</td><td>a single comparison detects it, nothing is read or shifted</td></tr>
                <tr><td>Single-element queue</td><td>O(1)</td><td>the shift loop runs zero times &mdash; size drops straight to 0</td></tr>
                <tr><td>Multiple-element queue</td><td>O(n)</td><td>every remaining element must shift one slot left to close the gap at index 0</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is allocated per extraction &mdash; the array is pre-allocated once</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: extraction always happens at index 0, the opposite end from where insertion
              shifts occur. This is the trade-off of keeping the array sorted: <code>peek()</code> and
              <code>extractMax()</code> become O(1) lookups plus an O(n) shift, while <code>insert()</code>
              absorbs the cost of finding the right sorted position.
            </p>
            <h3>Advantages</h3>
            <p>The highest-priority element is always trivially located at index 0 &mdash; no search is ever needed to find it. Contiguous memory keeps the shift cache-friendly.</p>
            <h3>Limitations</h3>
            <p>Every extraction costs O(n) in the worst case due to the left shift. Extracting from an empty queue must be explicitly guarded against with <b>Queue Underflow</b>, since there is nothing to read at index 0.</p>
            <h3>Queue vs Circular Queue vs Deque vs Priority Queue</h3>
            <p>A <b>Queue</b> removes strictly in FIFO order from the front. A <b>Circular Queue</b> also removes from the front but wraps the index around to reuse freed space. A <b>Deque</b> can remove from either end. A <b>Priority Queue</b> always removes the highest-priority element regardless of arrival order &mdash; here, that means always removing index 0 of the sorted array.</p>
            <h3>Key Observations</h3>
            <p>After extraction, the element that was at index 1 moves to index 0 and becomes the new highest-priority element. The trailing slot at the old <code>size - 1</code> becomes a leftover duplicate value until it is cleared and <code>size</code> is decremented.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> extraction is rejected with Queue Underflow and the array is left completely unchanged. <b>Single-element queue:</b> the one element is read, no shifting occurs, and the queue becomes empty. <b>Multiple-element queue:</b> every element shifts left by one, and the new element at index 0 becomes the highest-priority element for the next extraction.</p>
          </div>
        </div>
      </div>
    </div>
