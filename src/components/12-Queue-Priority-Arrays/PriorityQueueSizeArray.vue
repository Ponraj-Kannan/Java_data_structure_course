...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int size;'],
    ['', '};'], ['', ''],
    ['c_size', 'int getSize(struct PriorityQueue *q) {'],
    ['c_return', '    return q->size;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class PriorityQueue {'],
...
// though size() itself never inspects the array's contents or order.
...
// size() never reads or reorders any array slot, and it has no condition to
// branch on - it simply hands back the size counter that insert()/
// extractMax() already maintain. Cells are shown purely for context (what
// the queue currently holds), so every occupied slot is just 'normal' and
// every unused slot beyond size is 'empty'; no 'cur'/'shift'/'del' states
// ever occur here.
function buildSteps(capacity, elements) {
  ...
  pushStep({
    badge: fnLabel + ' returns ' + size + ' \u2014 the queue currently holds ' + size + ' element' + (size === 1 ? '' : 's') + ' out of ' + capacity + ' capacity',
    code: 'c_return',
    returnVal: String(size),
    vars: [frame('main()', [['capacity', String(capacity), true], ['size', String(size), true]]), frame(fnLabel, [['result', String(size), true]])],
  });
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip" v-if="s.returnVal !== undefined">returns = <b class="ll-c-green">{{ s.returnVal }}</b></div>
            </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm</h3>
            <p><code>size()</code> never inspects any element in the <code>values</code> or <code>priorities</code> arrays at all &mdash; it simply hands back the <code>size</code> counter, which <code>insert()</code> and <code>extractMax()</code> already keep up to date on every call. There is no condition to check and nothing that can go wrong.</p>
            <h3>Time &amp; Space Complexity</h3>
            <table class="ll-complexity-table">
              <thead><tr><th>Case</th><th>Time</th><th>Why</th></tr></thead>
              <tbody>
                <tr><td>Any queue state</td><td>O(1)</td><td>a single field read, independent of capacity or how many elements are stored</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used &mdash; nothing is read from or written to the arrays</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: because <code>size</code> is maintained incrementally by <code>insert()</code> (increment) and <code>extractMax()</code> (decrement), <code>size()</code> never has to count elements or scan the array &mdash; it just returns the counter's current value.
            </p>
            <h3>Advantages</h3>
            <p>The cheapest possible read on the structure &mdash; a single O(1) field access with no comparisons or branching at all. Both <code>isEmpty()</code> and <code>isFull()</code> can be expressed purely in terms of the value <code>size()</code> returns.</p>
            <h3>Limitations</h3>
            <p>Tells you how many elements are stored, but nothing about their values or priorities. It's only accurate as long as every insertion and extraction correctly updates the <code>size</code> field &mdash; a bug that mutates the arrays without also updating <code>size</code> would make this return a stale count.</p>
            <h3>Queue vs Circular Queue vs Deque vs Priority Queue</h3>
            <p>A <b>Queue</b>, <b>Circular Queue</b>, <b>Deque</b>, and <b>Priority Queue</b> all expose <code>size()</code> identically: a direct read of a maintained counter. The underlying storage strategy &mdash; linear, circular, double-ended, or sorted-by-priority &mdash; never changes how the count is retrieved.</p>
            <h3>Key Observations</h3>
            <p><code>size()</code> is the building block <code>isEmpty()</code> (<code>size() == 0</code>) and <code>isFull()</code> (<code>size() == capacity</code>) are defined in terms of. It has no interaction with the <code>values</code> or <code>priorities</code> arrays, and calling it never changes the queue's state.</p>
            <h3>Edge Cases</h3>
            <p><b>Newly constructed queue:</b> returns 0. <b>After several inserts with no extractions:</b> returns the exact count of successful inserts. <b>After an extraction:</b> returns one less than before. <b>Capacity 0:</b> no element can ever be inserted, so <code>size()</code> always returns 0.</p>
          </div>
        </div>
      </div>
    </div>
