...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_isempty', 'int isEmpty(struct Queue *q) {'],
    ['c_check1', '    if (q->front == -1) {'],
    ['c_true1', '        return 1;'],
    ['c_check1', '    }'],
    ['c_check2', '    return q->front > q->rear;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Queue {'],
  ...
  python: [
    ...
    ['c_check1', '        if self.front == -1:'],
    ['c_true1', '            return True'],
    ['c_check2', '        return self.front > self.rear'],
  ],
  javascript: [
    ['', 'class Queue {'],
    ...
    ['c_ctor', '  }'], ['', ''],
    ['c_isempty', '  isEmpty() {'],
    ['c_check1', '    if (this.front === -1) {'],
    ['c_true1', '      return true;'],
    ['c_check1', '    }'],
    ['c_check2', '    return this.front > this.rear;'],
    ['', '  }'], ['', '}'],
  ],
};
...
// front and rear pointers, so every cell simply stays 'normal' / 'empty'.
function buildSteps(capacity, values) {
  ...
  // --- if front == -1: the queue has never received a single element.
  if (front === -1) {
    pushStep({
      badge: 'front == -1 \u2192 true; the queue has never had an element inserted',
      code: 'c_check1', result: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [])],
    });
    pushStep({
      badge: fnLabel + ' returns true \u2192 Queue is Empty',
      code: 'c_true1', result: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
    });
  ...
  pushStep({
    badge: 'front != -1 \u2192 the queue has received at least one element, check front > rear next',
    code: 'c_check1',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [])],
  });

  // --- return front > rear: every element ever inserted has since been
  // dequeued (front has advanced past rear).
  const result = front > rear;
  pushStep({
    badge: result
      ? 'front > rear \u2192 true; every inserted element has already been dequeued'
      : 'front <= rear \u2192 false; the queue still holds ' + size + ' element(s)',
    code: 'c_check2', result,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [])],
  });

  pushStep({
    badge: fnLabel + ' returns ' + result + ' \u2192 ' + (result ? 'Queue is Empty' : 'Queue is Not Empty'),
    code: 'c_isempty', result,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
          ...
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.result !== undefined" class="ll-ptr-chip">
                isEmpty = <b class="ll-c-red">{{ s.result }}</b>
              </div>
            </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><b class="ll-c-green">front</b>&nbsp;/&nbsp;<b class="ll-c-orange">rear</b> pointers are inspected, not moved</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Any queue state</td><td>O(1)</td><td>only two pointer comparisons &mdash; never inspects the array contents</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only a boolean result is held temporarily</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>isEmpty()</code> never looks at <code>arr</code> at all &mdash; it is
              purely a comparison of <code>front</code> and <code>rear</code>. This is why every queue
              operation (enqueue, dequeue, front) can cheaply check emptiness first without any cost
              proportional to how many elements are actually stored.
            </p>
            <h3>Advantages</h3>
            <p>O(1) with no side effects; used internally as a guard before <code>dequeue()</code> and <code>front()</code> to detect <b>Queue Underflow</b> before attempting to read.</p>
            <h3>Limitations</h3>
            <p>None on its own &mdash; but relying on it does not tell you anything about how much <i>room</i> is left; that is a separate question answered by <code>isFull()</code>.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p>The emptiness check &mdash; <code>front == -1</code> or <code>front &gt; rear</code> &mdash; is identical for a Simple Queue and a Circular Queue. What differs between the two is only how <code>front</code> and <code>rear</code> are advanced during <code>enqueue()</code>/<code>dequeue()</code> (straight-line vs. wrap-around), not how emptiness is detected.</p>
            <h3>Key Observations</h3>
            <p><code>front == -1</code> covers the case where nothing has ever been enqueued. <code>front &gt; rear</code> covers the case where everything that was enqueued has since been dequeued. Either condition alone is sufficient to declare the queue empty.</p>
            <h3>Edge Cases</h3>
            <p><b>Freshly created queue:</b> <code>front</code> and <code>rear</code> are both -1, so <code>isEmpty()</code> is true. <b>After enqueues:</b> <code>front</code> becomes 0 and <code>isEmpty()</code> is false as long as <code>front &lt;= rear</code>. <b>After every element is dequeued:</b> <code>front</code> ends up greater than <code>rear</code>, so <code>isEmpty()</code> becomes true again.</p>
          </div>
        </div>
      </div>
    </div>
