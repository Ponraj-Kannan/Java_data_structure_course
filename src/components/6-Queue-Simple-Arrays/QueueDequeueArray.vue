...
const CODES = {
  java: [
    ...
    ['c_underflow', '        if (front == -1 || front > rear) {'],
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_dequeue', 'int dequeue(struct Queue *q) {'],
    ['c_underflow', '    if (q->front == -1 || q->front > q->rear) {'],
    ['c_underflow', '        printf("Queue Underflow\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_read', '    int value = q->arr[q->front];'],
    ['c_inc', '    q->front = q->front + 1;'],
...
//              part of the queue
//   'del'    - front has already moved past it; it is logically removed
//              (unreachable) even though the value briefly lingers on screen
function buildSteps(capacity, values) {
  ...
  if (front === -1 || front > rear) {
    pushStep({
      badge: 'front == -1 or front > rear \u2192 Queue Underflow, nothing to dequeue',
  ...
  pushStep({
    badge: 'front != -1 and front <= rear \u2192 queue is not empty, proceed',
    code: 'c_underflow',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [])],
  });
  ...
  // --- front = front + 1: the pointer advances. The old front slot is now
  // logically unreachable (index < front), even though its value briefly
  // lingers on screen to show it hasn't been physically wiped.
  const oldFrontIdx = front;
  stateAt[oldFrontIdx] = 'del';
  front = front + 1;
  pushStep({
    badge: 'front = front + 1 \u2192 front advances to ' + front + '; index ' + oldFrontIdx + ' is no longer part of the queue',
    code: 'c_inc',
    dequeuedVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(dequeuedVal), true]])],
  });

  // --- if front > rear: the queue has just become empty, so both pointers
  // reset to -1 rather than leaving a stale front sitting past a stale rear.
  if (front > rear) {
    front = -1;
    rear = -1;
    pushStep({
      badge: 'front > rear \u2192 queue is now empty, reset front = -1 and rear = -1',
      code: 'c_reset',
      dequeuedVal,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(dequeuedVal), true]])],
    });
    ...
    pushStep({
      badge: 'front <= rear \u2192 queue still has elements, front and rear otherwise unchanged',
      code: 'c_reset',
      dequeuedVal,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(dequeuedVal), true]])],
    });
  ...
  pushStep({
    badge: fnLabel + ' complete \u2014 dequeued ' + dequeuedVal + ', queue size now ' + size() + ' / ' + capacity,
    code: 'c_dequeue',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.dequeuedVal !== undefined" class="ll-ptr-chip">
                value = <b class="ll-c-red">{{ s.dequeuedVal }}</b>
              </div>
            </div>

            <div class="ll-arr-track">
              <template v-for="cell in s.cells" :key="cell.idx">
                <div class="ll-arr-cell-wrap" :class="{ 'll-nwrap-pending': cell.state === 'del' }">
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty queue (underflow)</td><td>O(1)</td><td>a single comparison detects it, nothing is read or changed</td></tr>
                <tr><td>Single-element queue</td><td>O(1)</td><td>read <code>arr[front]</code>, then front and rear both reset to -1</td></tr>
                <tr><td>Multiple-element queue</td><td>O(1)</td><td>direct read of <code>arr[front]</code> and an increment &mdash; no traversal or shifting</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only the returned value is held temporarily &mdash; no extra memory is allocated</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: dequeuing only ever touches the <code>front</code> end, so no elements ever
              need to shift. The dequeued slot's old value can be left in the array untouched &mdash; once
              <code>front</code> moves past it, that slot is simply outside the queue's valid range. Note
              that this slot is <b>not</b> reused by later enqueues in a Simple Queue &mdash; only a
              Circular Queue reclaims it.
            </p>
            <h3>Advantages</h3>
            <p>Extremely fast O(1) removal with no pointer/node bookkeeping, and no memory to free since the array is reused in place.</p>
            <h3>Limitations of a Simple Queue</h3>
            <p>Dequeuing from an empty queue must be explicitly checked (<b>Queue Underflow</b>) &mdash; there is no way to read past the front safely without that guard. Also, every dequeue permanently &ldquo;wastes&rdquo; the slot it vacates: those low indexes can never be reused for a future enqueue, which is exactly the limitation a Circular Queue is designed to fix.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p>In a <b>Simple (Linear) Queue</b>, once <code>front</code> advances past an index, that index is gone for good &mdash; <code>rear</code> never comes back around to reuse it. In a <b>Circular Queue</b>, both <code>front</code> and <code>rear</code> wrap around to index 0 using modulo arithmetic once they pass the last index, so the space freed up by a dequeue can be reused by a later enqueue.</p>
            <h3>Key Observations</h3>
            <p>Only the element at index <code>front</code> can ever be dequeued. After dequeuing, the new <code>front</code> is always <code>old front + 1</code>, and the current size is always <code>rear - front + 1</code>. If that increment makes <code>front &gt; rear</code>, the queue has become empty and both pointers reset to <b>-1</b>.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> <code>front == -1</code> (or <code>front &gt; rear</code>) means underflow &mdash; the queue is left completely unchanged. <b>Single-element queue:</b> dequeuing the only element makes <code>front &gt; rear</code>, so both <code>front</code> and <code>rear</code> reset to -1, making the queue empty again. <b>Multiple elements:</b> only the frontmost value is affected; everything from the new <code>front</code> to <code>rear</code> stays exactly as it was.</p>
          </div>
        </div>
      </div>
    </div>
