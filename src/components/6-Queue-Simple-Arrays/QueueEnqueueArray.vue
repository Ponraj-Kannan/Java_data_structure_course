...
const CODES = {
  java: [
    ...
    ['c_overflow', '        if (rear == capacity - 1) {'],
    ['c_overflow', '            System.out.println("Queue Overflow");'],
    ['c_overflow', '            return;'],
    ['c_overflow', '        }'],
    ['c_frontinit', '        if (front == -1) {'],
    ['c_frontinit', '            front = 0;'],
    ['c_frontinit', '        }'],
    ['c_inc', '        rear = rear + 1;'],
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_enqueue', 'void enqueue(struct Queue *q, int value) {'],
    ['c_overflow', '    if (q->rear == q->capacity - 1) {'],
    ['c_overflow', '        printf("Queue Overflow\\n");'],
    ['c_overflow', '        return;'],
    ['c_overflow', '    }'],
    ['c_frontinit', '    if (q->front == -1) {'],
    ['c_frontinit', '        q->front = 0;'],
    ['c_frontinit', '    }'],
    ['c_inc', '    q->rear = q->rear + 1;'],
  ...
  python: [
    ...
    ['c_overflow', '        if self.rear == self.capacity - 1:'],
    ['c_overflow', '            print("Queue Overflow")'],
    ['c_overflow', '            return'],
    ['c_frontinit', '        if self.front == -1:'],
    ['c_frontinit', '            self.front = 0'],
    ['c_inc', '        self.rear = self.rear + 1'],
  ...
  javascript: [
    ...
    ['c_overflow', '    if (this.rear === this.capacity - 1) {'],
    ['c_overflow', '      console.log("Queue Overflow");'],
    ['c_overflow', '      return;'],
    ['c_overflow', '    }'],
    ['c_frontinit', '    if (this.front === -1) {'],
    ['c_frontinit', '      this.front = 0;'],
    ['c_frontinit', '    }'],
    ['c_inc', '    this.rear = this.rear + 1;'],
...
//   'empty'  - unused slot beyond the current rear, drawn dashed and blank
//   'normal' - an occupied, settled slot (includes the enqueued value once
//              the operation has fully completed)
//   'cur'    - the slot rear has just moved onto, before a value is written
//   'new'    - the slot that just received the enqueued value, until the
//              operation finishes
function buildSteps(capacity, values, enqueueValue) {
  const steps = [];
  ...
  if (rear === capacity - 1) {
    pushStep({
      badge: 'rear == capacity - 1 \u2192 Queue Overflow, ' + enqueueValue + ' cannot be inserted',
  ...
  pushStep({
    badge: 'rear != capacity - 1 \u2192 queue is not full, proceed',
    code: 'c_overflow',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
  });

  // --- if front == -1: front = 0. Only happens the very first time an
  // element is inserted into a completely empty queue.
  if (front === -1) {
    front = 0;
    pushStep({
      badge: 'front == -1 \u2192 queue was empty, front = 0',
      code: 'c_frontinit',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
    });
  } else {
    pushStep({
      badge: 'front != -1 \u2192 queue already has elements, front unchanged',
      code: 'c_frontinit',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
    });
  }

  // --- rear = rear + 1: the pointer moves onto the next free slot first.
  // Nothing has been written there yet - it is only previewed (dashed).
  rear = rear + 1;
  stateAt[rear] = 'cur';
  pushStep({
    badge: 'rear = rear + 1 \u2192 rear now points to index ' + rear + ' (still empty)',
    code: 'c_inc',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
  });
...
const stepsData = reactive(buildSteps(5, [10, 20, 30], 40));
...
function applyInput() {
  ...
  playing.value = false;
  const built = buildSteps(cap, arr, val);
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty queue / first insertion</td><td>O(1)</td><td>just set <code>front = 0</code>, increment <code>rear</code>, and write the value</td></tr>
                <tr><td>Normal enqueue</td><td>O(1)</td><td>direct write to <code>arr[rear]</code> &mdash; no traversal or shifting</td></tr>
                <tr><td>Full queue (overflow)</td><td>O(1)</td><td>a single comparison detects it, nothing is written</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is allocated per enqueue &mdash; the array is pre-allocated once</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: because an enqueue always happens at the <code>rear</code> end and the
              array already has a reserved slot waiting for it, no elements ever need to move. This is
              what makes enqueue O(1) on an array-based queue &mdash; unlike inserting into the middle
              of a plain array, which would require shifting every following element.
            </p>
            <h3>Advantages</h3>
            <p>Simple to implement, cache-friendly contiguous memory, and O(1) enqueue with no pointer/node overhead per element.</p>
            <h3>Limitations of a Simple Queue</h3>
            <p>The capacity is fixed at creation time, and <code>rear</code> only ever moves forward. Once <code>rear == capacity - 1</code>, further enqueues fail with <b>Queue Overflow</b> &mdash; even if earlier dequeues have freed up slots at the start of the array, those slots (indexes below <code>front</code>) can never be reused.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p>A <b>Simple (Linear) Queue</b> never reclaims the space left behind by dequeued elements &mdash; <code>rear</code> marches straight from index 0 towards <code>capacity - 1</code> and then stops for good. A <b>Circular Queue</b> solves this by wrapping <code>rear</code> (and <code>front</code>) back to index 0 once it passes the last index, using modulo arithmetic, so freed-up slots at the front of the array can be reused by later enqueues.</p>
            <h3>Key Observations</h3>
            <p><code>front</code> and <code>rear</code> both start at <b>-1</b> for an empty queue. The very first enqueue sets <code>front</code> to <b>0</b>; every enqueue after that leaves <code>front</code> untouched and only advances <code>rear</code>. The current size is always <code>rear - front + 1</code> (or 0 when the queue is empty).</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> the first enqueue sets <code>front</code> from -1 to 0 in addition to placing the value. <b>Multiple insertions:</b> each subsequent enqueue only advances <code>rear</code> and writes the new value, leaving <code>front</code> unchanged. <b>Full queue:</b> when <code>rear == capacity - 1</code>, enqueue must detect overflow and leave the queue completely unchanged &mdash; no write happens and neither pointer moves.</p>
          </div>
        </div>
      </div>
    </div>
