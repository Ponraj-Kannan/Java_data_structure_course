...
const CODES = {
  java: [
    ...
    ['c_empty', '            System.out.println("Queue is Empty");'],
    ['c_empty', '            return -1;'],
    ['c_empty', '        }'],
    ['c_return', '        return arr[front];'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['', '#define MAX 100'], ['', ''],
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_front', 'int front(struct Queue *q) {'],
    ['c_empty', '    if (q->front == -1 || q->front > q->rear) {'],
    ['c_empty', '        printf("Queue is Empty\\n");'],
    ['c_empty', '        return -1;'],
    ['c_empty', '    }'],
    ['c_return', '    return q->arr[q->front];'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Queue {'],
    ...
    ['c_front', '    int front() {'],
    ['c_empty', '        if (front == -1 || front > rear) {'],
    ['c_empty', '            cout << "Queue is Empty" << endl;'],
    ['c_empty', '            return -1;'],
    ['c_empty', '        }'],
    ['c_return', '        return arr[front];'],
    ['', '    }'], ['', '};'],
  ...
  python: [
    ...
    ['c_empty', '            print("Queue is Empty")'],
    ['c_empty', '            return None'],
    ['c_return', '        return self.arr[self.front]'],
  ],
  javascript: [
    ['', 'class Queue {'],
    ...
    ['c_empty', '      console.log("Queue is Empty");'],
    ['c_empty', '      return undefined;'],
    ['c_empty', '    }'],
    ['c_return', '    return this.arr[this.front];'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'cur'    - the front slot, selected and read \u2014 nothing is removed or
//              rewritten, the queue is left exactly as it was
function buildSteps(capacity, values) {
  ...
  if (front === -1 || front > rear) {
    pushStep({
      badge: 'front == -1 or front > rear \u2192 Queue is Empty, no front element',
      code: 'c_empty', invalid: true,
  ...
  // --- return arr[front]: the front slot is selected and read, but it
  // stays a completely normal, occupied part of the queue.
  const frontVal = valAt[front];
  stateAt[front] = 'cur';
  pushStep({
    badge: 'return arr[front] \u2192 front element is ' + frontVal + ' at index ' + front,
    code: 'c_return',
    frontVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['front element', String(frontVal), true]])],
  });

  pushStep({
    badge: fnLabel + ' complete \u2014 returns ' + frontVal + ', queue unchanged (front and rear stay the same)',
    code: 'c_front',
    frontVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.frontVal !== undefined" class="ll-ptr-chip">
                front value = <b class="ll-c-red">{{ s.frontVal }}</b>
              </div>
            </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>front element (peeked, not removed)</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty queue</td><td>O(1)</td><td>a single comparison detects it, nothing is read</td></tr>
                <tr><td>Non-empty queue</td><td>O(1)</td><td>direct read of <code>arr[front]</code> &mdash; no traversal</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only the returned value is held temporarily &mdash; no extra memory is allocated</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>front()</code> is a pure read. Unlike <code>dequeue()</code>, neither
              <code>front</code> nor <code>rear</code> ever changes, and no array cell is written to or
              cleared &mdash; the queue looks identical before and after the call.
            </p>
            <h3>Advantages</h3>
            <p>Lets you inspect the next element that will be removed without committing to removing it &mdash; useful for lookahead logic, and O(1) with zero side effects.</p>
            <h3>Limitations</h3>
            <p>Peeking at an empty queue must be explicitly checked; there is no valid value to return, so a guard against <code>front == -1</code> (or <code>front &gt; rear</code>) is required, mirroring the check used by <code>dequeue()</code>.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p><code>front()</code> behaves identically on a Simple Queue and a Circular Queue &mdash; both simply read <code>arr[front]</code>. The only difference between the two queue types shows up in how <code>enqueue()</code> and <code>dequeue()</code> advance <code>front</code>/<code>rear</code> (straight-line vs. wrap-around with modulo arithmetic); reading the front element is unaffected.</p>
            <h3>Key Observations</h3>
            <p>The front element is always at index <code>front</code>, never at index <code>0</code> once elements have been dequeued. <code>front()</code> never changes <code>front</code>, <code>rear</code>, or the array contents &mdash; it is purely observational.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> <code>front == -1</code> (or <code>front &gt; rear</code>) means there is no front element to return. <b>Non-empty queue:</b> the value at <code>arr[front]</code> is returned and the queue is left completely untouched.</p>
          </div>
        </div>
      </div>
    </div>
