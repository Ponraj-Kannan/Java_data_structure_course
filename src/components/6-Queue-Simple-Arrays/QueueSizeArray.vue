...
const CODES = {
  java: [
    ...
    ['c_ctor', '    }'], ['', ''],
    ['c_size', '    int size() {'],
    ['c_empty', '        if (front == -1 || front > rear) {'],
    ['c_empty', '            return 0;'],
    ['c_empty', '        }'],
    ['c_calc', '        return rear - front + 1;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['', '#define MAX 100'], ['', ''],
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_size', 'int size(struct Queue *q) {'],
    ['c_empty', '    if (q->front == -1 || q->front > q->rear) {'],
    ['c_empty', '        return 0;'],
    ['c_empty', '    }'],
    ['c_calc', '    return q->rear - q->front + 1;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Queue {'],
  ...
  python: [
    ...
    ['c_empty', '        if self.front == -1 or self.front > self.rear:'],
    ['c_empty', '            return 0'],
    ['c_calc', '        return self.rear - self.front + 1'],
  ],
  javascript: [
    ['', 'class Queue {'],
    ...
    ['c_ctor', '  }'], ['', ''],
    ['c_size', '  size() {'],
    ['c_empty', '    if (this.front === -1 || this.front > this.rear) {'],
    ['c_empty', '      return 0;'],
    ['c_empty', '    }'],
    ['c_calc', '    return this.rear - this.front + 1;'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'cur'    - occupied slots between front and rear, highlighted together
//              while the size calculation is being explained
function buildSteps(capacity, values) {
  ...
  function pushStep(opts) {
    steps.push({
      cells: cells(),
      front, rear, capacity,
      size: opts.size !== undefined ? opts.size : undefined,
      badge: opts.badge,
      code: opts.code,
  ...
  pushStep({
    badge: 'Initial queue \u2014 front = ' + front + ', rear = ' + rear + ', capacity = ' + capacity + ' \u2014 ' + fnLabel + ' called',
    code: 'c_size',
  ...
  if (isEmpty) {
    pushStep({
      badge: 'front == -1 or front > rear \u2192 queue is empty, Current Queue Size = 0',
      code: 'c_empty', size: 0,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [])],
    });
    pushStep({
      badge: fnLabel + ' returns 0 \u2192 Current Queue Size = 0',
      code: 'c_size', size: 0,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
    });
  ...
  pushStep({
    badge: 'front != -1 and front <= rear \u2192 queue is not empty, compute rear - front + 1',
    code: 'c_empty',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [])],
  });

  // --- highlight every occupied cell (front..rear) together while the
  // calculation is explained.
  for (let i = front; i <= rear; i++) stateAt[i] = 'cur';
  pushStep({
    badge: 'return rear - front + 1 \u2192 ' + rear + ' - ' + front + ' + 1 = ' + size + ' occupied cell(s) highlighted',
    code: 'c_calc', size,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['rear - front + 1', String(size), true]])],
  });

  pushStep({
    badge: fnLabel + ' returns ' + size + ' \u2192 Current Queue Size = ' + size,
    code: 'c_size', size,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.size !== undefined" class="ll-ptr-chip">
                size = <b class="ll-c-purple">{{ s.size }}</b>
              </div>
            </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>counted towards size (front &hellip; rear)</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty queue</td><td>O(1)</td><td>the emptiness check short-circuits straight to 0</td></tr>
                <tr><td>Non-empty queue</td><td>O(1)</td><td>a single subtraction, <code>rear - front + 1</code> &mdash; no traversal or counting</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only the computed integer is held temporarily</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: because <code>front</code> and <code>rear</code> always mark exactly the
              first and last occupied indexes, the size never needs to be tracked with a separate counter
              or computed by scanning the array &mdash; it falls straight out of the two pointers.
            </p>
            <h3>Advantages</h3>
            <p>O(1) size lookup with no extra bookkeeping variable to keep in sync during enqueue/dequeue &mdash; it is always derivable directly from <code>front</code> and <code>rear</code>.</p>
            <h3>Limitations</h3>
            <p>The formula only holds because a Simple Queue's occupied elements are always contiguous between <code>front</code> and <code>rear</code>; if pointers could ever wrap around (as in a Circular Queue), this exact subtraction would no longer be correct on its own.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p>In a <b>Simple Queue</b>, size is simply <code>rear - front + 1</code> (or 0 when empty), since the occupied range never wraps. In a <b>Circular Queue</b>, <code>rear</code> can be numerically smaller than <code>front</code> after wrapping around, so size must instead be computed as <code>(rear - front + capacity) % capacity + 1</code>, or tracked with a separate counter updated on every enqueue/dequeue.</p>
            <h3>Key Observations</h3>
            <p>When the queue is empty, <code>front == -1</code> (or <code>front &gt; rear</code>) and the size is defined as exactly <b>0</b>, not a negative number. Otherwise, the count of occupied cells is always <code>rear - front + 1</code>, inclusive of both endpoints.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> <code>size()</code> must special-case this and return 0 directly, since <code>rear - front + 1</code> would otherwise be meaningless when both are -1. <b>Single-element queue:</b> <code>front == rear</code>, giving <code>rear - front + 1 = 1</code>. <b>Full queue:</b> <code>rear = capacity - 1</code>; if <code>front</code> is still 0, size equals <code>capacity</code>.</p>
          </div>
        </div>
      </div>
    </div>
