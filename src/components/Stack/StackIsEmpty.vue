...
function buildSteps(values) {
  ...
  function phase(nodes, introBadge) {
    ...
    steps.push({
      ...
      badge: introBadge, code: 'c_isempty',
      vars: [mainFrame()],
    });

    steps.push({
      nodes: [...nodes], topAddr, size, curAddr: topAddr,
      badge: 'call isEmpty() \u2192 check top == null', code: 'c_check',
      vars: [mainFrame(), frame('isEmpty()', [])],
    });

    if (nodes.length === 0) {
      steps.push({
        nodes: [...nodes], topAddr, size, result: true, done: true,
        badge: 'top == null \u2192 return true \u2192 Stack is Empty', code: 'c_true',
        vars: [mainFrame(), frame('isEmpty()', [['return', 'true']])],
      });
    } else {
      steps.push({
        nodes: [...nodes], topAddr, size, curAddr: topAddr, result: false, done: true,
        badge: 'top != null \u2192 return false \u2192 Stack is Not Empty', code: 'c_false',
  ...
  let nodesA = [];
  values.forEach((v, i) => {
    nodesA = [{ val: v, addr: ADDR(i) }, ...nodesA];
  });
  phase(
    nodesA,
    nodesA.length
      ? 'Values pushed in order: ' + values.join(', ') + ' \u2192 final stack (top \u2192 bottom): ' + nodesA.map((n) => n.val).join(', ')
      : 'Stack starts empty'
  );

  if (nodesA.length) {
    phase([], 'Now demonstrate the other branch \u2192 pop everything so the stack becomes empty');
  } else {
    const nodesB = [{ val: 99, addr: ADDR(500) }];
    phase(nodesB, 'Now demonstrate the other branch \u2192 push a node so the stack is no longer empty');
  }
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            ...
            <div v-if="s.result !== undefined" class="ll-result-banner" :class="s.result ? 'll-result-true' : 'll-result-false'">
              {{ s.result ? '✔ Stack is Empty' : '✔ Stack is Not Empty' }}
            </div>
          </div>
        </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>top (being checked)</span>
        </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm — isEmpty</h3>
            <p>
              <code>isEmpty()</code> answers a single question: does the stack currently have any nodes? Because
              a linked-list stack keeps a direct <code>top</code> pointer, the check is just <code>top == null</code>
              — no traversal, no counting, and no structural change of any kind.
            </p>
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Best / Average / Worst</td><td>O(1)</td><td>a single pointer comparison — the size of the stack never matters</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used</td></tr>
              </tbody>
            </table>
            <h3>Key Observations</h3>
            <ol>
              <li><code>isEmpty()</code> is the guard every other operation should call first: <code>pop()</code> and <code>peek()</code> both rely on it (directly or via the same <code>top == null</code> check) to detect Stack Underflow before touching any pointers.</li>
              <li>Checking <code>top == null</code> is equivalent to checking <code>size == 0</code>, but it doesn't depend on a separate counter being kept in sync — some implementations skip the <code>size</code> field entirely and rely only on <code>top</code>.</li>
              <li>A single-node stack is <b>not</b> empty — only when <code>top</code> itself is <code>null</code> does the stack qualify as empty.</li>
            </ol>
            <h3>Edge Cases</h3>
            <ol>
              <li><b>Freshly created stack:</b> <code>top</code> starts as <code>null</code>, so <code>isEmpty()</code> returns <code>true</code> immediately.</li>
              <li><b>After popping the last node:</b> once the only remaining node is removed, <code>top</code> becomes <code>null</code> again and <code>isEmpty()</code> returns <code>true</code>.</li>
              <li><b>Any non-empty stack:</b> regardless of how many nodes exist (one or a thousand), <code>isEmpty()</code> returns <code>false</code> the same way — in O(1).</li>
            </ol>
            <p class="ll-note">
              Note: unlike an array-based stack, there is no companion <code>isFull()</code> to pair with
              <code>isEmpty()</code> here — a linked-list stack has no fixed capacity, so only the empty boundary
              is ever meaningful.
            </p>
          </div>
        </div>
