...
const CODES = {
  java: [
    ...
    ['', ''],
    ['c_delete', '    void deleteEnd() {'],
    ['c_check', '        if (head == null) return;'],
    ['c_single', '        if (head.next == null) {'],
    ['c_single', '            head = null;'],
    ['c_single', '            return;'],
    ['c_single', '        }'],
    ...
    ['c_unlink', '        temp.prev.next = null;'],
    ['c_unlink', '        // temp is freed'],
    ['', '    }'],
    ['', ''],
  ...
  cpp: [
    ...
    ['c_check', '    if (!head) return;'],
    ['c_single', '    if (!head->next) {'],
    ['c_single', '        delete head;'],
    ['c_single', '        head = nullptr;'],
    ['c_single', '        return;'],
    ['c_single', '    }'],
    ['c_walk', '    Node *temp = head;'],
    ['c_walk', '    while (temp->next)'],
    ['c_walk', '        temp = temp->next;'],
    ['c_unlink', '    temp->prev->next = nullptr;'],
    ['c_unlink', '    delete temp;'],
    ['}', ''],
    ['', ''],
    ['c_display', 'void display() {'],
    ['c_display', '    Node *t = head;'],
  ...
  python: [
    ...
    ['', ''],
    ['c_delete', '    def delete_end(self):'],
    ['c_check', '        if not self.head:'],
    ['c_check', '            return'],
    ['c_single', '        if not self.head.next:'],
    ['c_single', '            self.head = None'],
    ['c_single', '            return'],
    ['c_walk', '        temp = self.head'],
    ['c_walk', '        while temp.next:'],
    ['c_walk', '            temp = temp.next'],
    ['c_unlink', '        temp.prev.next = None'],
    ['', ''],
...
function buildSteps(values) {
  ...
  steps.push({
    ...
    badge: 'deleteEnd(): check head != null', code: 'c_check',
    vars: [frame('deleteEnd()', [['head', fmt(oldHead), true]])],
  });
  if (nodes.length === 1) {
    const delAddr = nodes[0].addr;
    steps.push({
      nodes: nodes.slice(), head: oldHead, delAddr,
      badge: 'only one node → head = null, free @' + delAddr, code: 'c_single',
      vars: [frame('deleteEnd()', [['head', fmt(oldHead), true]])],
    });
    nodes = [];
  } else {
    steps.push({
      nodes: nodes.slice(), head: oldHead,
      badge: 'head.next != null → walk to last node', code: 'c_walk',
      vars: [frame('deleteEnd()', [['temp', fmt(oldHead), true]])],
    });
    for (let t = 0; t < nodes.length - 1; t++) {
      const nextA = t + 1 < nodes.length ? nodes[t + 1].addr : null;
      steps.push({
        nodes: nodes.slice(), head: oldHead, tempAddr: nodes[t].addr, curAddr: nodes[t].addr,
        badge: 'walk: temp @' + nodes[t].addr + (nextA ? ' → next @' + nextA : ' (last, stop)'), code: 'c_walk',
        vars: [frame('deleteEnd()', [['temp', fmt(nodes[t].addr), true]])],
      });
    }
    const delAddr = nodes[nodes.length - 1].addr;
    const prevAddr = nodes[nodes.length - 2].addr;
    steps.push({
      nodes: nodes.slice(), head: oldHead, tempAddr: delAddr, delAddr,
      badge: 'temp @' + delAddr + ' is last node (next=null)', code: 'c_walk',
      vars: [frame('deleteEnd()', [['temp', fmt(delAddr), true]])],
    });
    nodes.pop();
    steps.push({
      nodes: nodes.slice(), head: headAddr(), tempAddr: prevAddr,
      badge: 'temp.prev.next=null → unlink @' + delAddr, code: 'c_unlink',
      vars: [frame('deleteEnd()', [['temp', fmt(delAddr)], ['prev', fmt(prevAddr), true]])],
    });
  }
            ...
            <div class="ll-stage">
              <div v-for="(n, idx) in s.nodes" :key="n.addr" class="ll-nwrap">
                ...
                <div
                  class="ll-node"
                  :class="{
                    'll-node-new': n.addr === s.neuAddr,
                    'll-node-cur': n.addr === s.curAddr,
                    'll-node-del': n.addr === s.delAddr,
                  }"
                >
                  <div class="ll-box">
                    <div class="ll-ptr ll-ptr-prev"><small>prev</small>{{ fmt(prevAddrOf(idx)) }}</div>
                    <div class="ll-data">{{ n.val }}</div>
                    <div class="ll-ptr ll-ptr-next"><small>next</small>{{ fmt(nextAddrOf(idx)) }}</div>
                  </div>
                  <span
                    class="ll-arrow"
              ...
              <div v-if="s.pending" class="ll-nwrap" style="margin-left: 22px">
                ...
                <div class="ll-node ll-node-new">
                  <div class="ll-box">
                    ...
                    <div class="ll-ptr ll-ptr-next"><small>next</small>{{ fmt(s.pendNext) }}</div>
                  </div>
                </div>
              </div>
            </div>
