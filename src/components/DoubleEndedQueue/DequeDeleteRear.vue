...
const CODES = {
  java: [
    ...
    ['c_sethead', '    Node rear;'], ['', ''],
    ['c_ins', '    void deleteRear() {'],
    ['c_underflow', '        if (rear == null) {'],
    ['c_underflow', '            System.out.println("Deque Underflow");'],
    ['c_underflow', '            return;'],
    ['c_underflow', '        }'],
    ['c_saverear', '        Node temp = rear;'],
    ['c_retreat', '        rear = rear.prev;'],
    ['c_lastcase', '        if (rear == null) {'],
    ['c_lastcase', '            front = null;'],
    ['c_relinkrear', '        } else {'],
    ['c_relinkrear', '            rear.next = null;'],
    ['c_relinkrear', '        }'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ...
    ['c_ins', 'void deleteRear() {'],
    ['c_underflow', '    if (rear == NULL) {'],
    ['c_underflow', '        printf("Deque Underflow\\n");'],
    ['c_underflow', '        return;'],
    ['c_underflow', '    }'],
    ['c_saverear', '    struct Node *temp = rear;'],
    ['c_retreat', '    rear = rear->prev;'],
    ['c_lastcase', '    if (rear == NULL) {'],
    ['c_lastcase', '        front = NULL;'],
    ['c_relinkrear', '    } else {'],
  ...
  cpp: [
    ...
    ['c_ins', 'void deleteRear() {'],
    ['c_underflow', '    if (rear == NULL) {'],
    ['c_underflow', '        cout << "Deque Underflow" << endl;'],
    ['c_underflow', '        return;'],
    ['c_underflow', '    }'],
    ['c_saverear', '    Node *temp = rear;'],
    ['c_retreat', '    rear = rear->prev;'],
  ...
  python: [
    ...
    ['c_ins', '    def delete_rear(self):'],
    ['c_underflow', '        if self.rear is None:'],
    ['c_underflow', '            print("Deque Underflow")'],
    ['c_underflow', '            return'],
    ['c_saverear', '        temp = self.rear'],
    ['c_retreat', '        self.rear = self.rear.prev'],
    ['c_lastcase', '        if self.rear is None:'],
    ['c_lastcase', '            self.front = None'],
    ['c_relinkrear', '        else:'],
    ['c_relinkrear', '            self.rear.next = None'],
  ],
  javascript: [
    ['', 'class Node {'],
    ...
    ['c_ins', '  deleteRear() {'],
    ['c_underflow', '    if (this.rear === null) {'],
    ['c_underflow', '      console.log("Deque Underflow");'],
    ['c_underflow', '      return;'],
    ['c_underflow', '    }'],
    ['c_saverear', '    const temp = this.rear;'],
    ['c_retreat', '    this.rear = this.rear.prev;'],
    ['c_lastcase', '    if (this.rear === null) {'],
    ['c_lastcase', '      this.front = null;'],
    ['c_relinkrear', '    } else {'],
    ['c_relinkrear', '      this.rear.next = null;'],
    ['c_relinkrear', '    }'],
    ['', '  }'], ['', '}'],
  ],
};
...
const PSEUDOCODE = [
  ...
  '    else:',
  '        rear.next = null',
  '    // temp is now removed from the deque',
];

function frame(title, rows) { return { title, rows }; }

// Node phases while it is still on screen:
//   'normal'   - an ordinary, unaffected node
//   'selected' - temp now references it, but it is still fully linked (both
//                the rear pointer and the chain itself still reach it)
//   'detached' - every pointer update needed to unlink it has completed
//                (nothing in the live chain reaches it any more); it is
//                about to be freed but is still drawn for one step so the
//                "reassignment" and "deletion" moments stay visually distinct
function buildSteps(values) {
  const steps = [];
  const phaseStarts = { delete: 0 };

  const n = values.length;
  const addrs = values.map((v, i) => ADDR(i));
  const valOf = {};
  values.forEach((v, i) => { valOf[addrs[i]] = v; });

  // Real pointer state, simulated step by step so every rendered frame
  // reflects the linked list exactly as it truly is at that moment.
  const prevOf = {};
  const nextOf = {};
  addrs.forEach((a, i) => {
    prevOf[a] = i > 0 ? addrs[i - 1] : null;
    nextOf[a] = i < n - 1 ? addrs[i + 1] : null;
  });
  const stateOf = {};
  addrs.forEach(a => { stateOf[a] = 'normal'; });
  const removed = new Set();

  let frontAddr = n > 0 ? addrs[0] : null;
  let rearAddr = n > 0 ? addrs[n - 1] : null;
  let tempAddr;

  const fnLabel = 'deleteRear()';

  function visibleNodes() {
    return addrs
      .filter(a => !removed.has(a))
      .map(a => ({ addr: a, val: valOf[a], prev: prevOf[a], next: nextOf[a], state: stateOf[a] }));
  }

  function pushStep(opts) {
    steps.push({
      nodes: visibleNodes(),
      frontAddr, rearAddr, tempAddr,
      badge: opts.badge,
      code: opts.code,
  ...
  pushStep({
    ...
    code: 'c_ins',
    vars: [frame('main()', [['front', fmt(frontAddr), true], ['rear', fmt(rearAddr), true]])],
  });

  if (n === 0) {
    pushStep({
      badge: 'rear == null \u2192 Deque Underflow, there is nothing to delete',
      code: 'c_underflow', invalid: true,
      vars: [frame('main()', [['front', 'null', true], ['rear', 'null', true]])],
    });
    return { steps, phaseStarts };
  }

  pushStep({
    badge: 'rear != null \u2192 deque is not empty, proceed',
    code: 'c_underflow',
    vars: [frame('main()', [['front', fmt(frontAddr), true], ['rear', fmt(rearAddr), true]])],
  });

  // --- Selection: temp now references the rear node, but nothing about
  // the chain has changed yet, so the node stays fully linked and visible.
  tempAddr = rearAddr;
  stateOf[tempAddr] = 'selected';
  pushStep({
    badge: 'temp = rear \u2192 node @' + tempAddr + ' is selected (still fully linked in the deque)',
    code: 'c_saverear',
    vars: [frame('main()', [['front', fmt(frontAddr), true], ['rear', fmt(rearAddr), true]]), frame(fnLabel, [['temp', fmt(tempAddr), true]])],
  });

  if (n === 1) {
    // rear = rear.prev -> null. The node is still the value of `front`,
    // so it is not orphaned yet - keep it selected, just move the rear pointer.
    rearAddr = null;
    pushStep({
      badge: 'rear = rear.prev \u2192 rear becomes null (it was the only node); temp still holds @' + tempAddr,
      code: 'c_retreat',
      vars: [frame('main()', [['front', fmt(frontAddr), true], ['rear', 'null', true]]), frame(fnLabel, [['temp', fmt(tempAddr), true]])],
    });

    // front = null too - now nothing in the deque references the node, so
    // it becomes truly detached (orphaned, reachable only via temp).
    frontAddr = null;
    stateOf[tempAddr] = 'detached';
    pushStep({
      badge: 'rear == null \u2192 front = null too \u2014 node @' + tempAddr + ' is no longer referenced by front or rear',
      code: 'c_lastcase',
      vars: [frame('main()', [['front', 'null', true], ['rear', 'null', true]]), frame(fnLabel, [['temp', fmt(tempAddr), true]])],
    });

    // Only now, after every pointer update is done, is it actually removed.
    const removedAddr = tempAddr;
    removed.add(tempAddr);
    tempAddr = undefined;
    pushStep({
      badge: 'Node @' + removedAddr + ' removed \u2014 deque is now empty',
      code: 'c_ins',
      vars: [frame('main()', [['front', 'null', true], ['rear', 'null', true]])],
    });
    return { steps, phaseStarts };
  ...
  // --- n >= 2: rear retreats, but the chain link to the old rear node is
  // untouched so far - it is still reachable by traversal, just no longer
  // the official rear. Keep it selected (not yet orphaned).
  const newRearAddr = prevOf[tempAddr];
  rearAddr = newRearAddr;
  pushStep({
    badge: 'rear = rear.prev \u2192 rear retreats to @' + newRearAddr + '; node @' + tempAddr + ' is still linked via @' + newRearAddr + '.next',
    code: 'c_retreat',
    vars: [frame('main()', [['front', fmt(frontAddr), true], ['rear', fmt(newRearAddr), true]]), frame(fnLabel, [['temp', fmt(tempAddr), true]])],
  });

  // --- rear.next = null actually cuts the chain link. Only now is the old
  // rear node orphaned (unreachable by traversal, reachable only via temp).
  nextOf[newRearAddr] = null;
  stateOf[tempAddr] = 'detached';
  pushStep({
    badge: 'rear != null \u2192 rear.next = null \u2192 link to node @' + tempAddr + ' is cut; it is now orphaned, reachable only via temp',
    code: 'c_relinkrear',
    vars: [frame('main()', [['front', fmt(frontAddr), true], ['rear', fmt(newRearAddr), true]]), frame(fnLabel, [['temp', fmt(tempAddr), true]])],
  });

  // --- Only after both pointer updates are complete does the node actually
  // disappear from the diagram.
  const removedAddr = tempAddr;
  removed.add(tempAddr);
  tempAddr = undefined;
  pushStep({
    badge: 'Node @' + removedAddr + ' removed \u2014 deque now has ' + (n - 1) + ' node(s)',
    code: 'c_ins',
    vars: [frame('main()', [['front', fmt(frontAddr), true], ['rear', fmt(rearAddr), true]])],
  });
...
const stepsData = reactive(buildSteps([10, 20, 30]));
...
<template>
  <div class="slide-wrapper">
    ...
    <div class="ll-toolbar">
      ...
      <input type="text" v-model="inpElems" placeholder="Enter numbers, or leave blank for empty deque" class="ll-text-input" />
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
          ...
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.tempAddr !== undefined" class="ll-ptr-chip">
                temp = <b class="ll-c-red">@{{ s.tempAddr }}</b>
              </div>
            </div>

            <div class="ll-stage">
              <template v-for="(n, idx) in s.nodes" :key="n.addr">
                <div class="ll-nwrap" :class="{ 'll-nwrap-pending': n.state === 'detached' }">
                  <div class="ll-who" :class="{ 'll-c-orange': n.state === 'selected', 'll-c-red': n.state === 'detached' }">{{ nodeLabel(n) }}</div>
                  <div class="ll-addr">@{{ n.addr }}</div>
                  <div class="ll-node">
                    <div
                      class="ll-box ll-box-deque"
                      :class="{ 'll-box-cur': n.state === 'selected', 'll-box-del ll-box-pending': n.state === 'detached' }"
                    >
                      <div class="ll-ptr ll-ptr-left"><small>prev</small>{{ fmt(n.prev) }}</div>
                      <div class="ll-data">{{ n.val }}</div>
                      <div class="ll-ptr"><small>next</small>{{ fmt(n.next) }}</div>
                    </div>
                    <span class="ll-arrow" v-if="idx < s.nodes.length - 1 && n.next === s.nodes[idx + 1].addr">&#8644;</span>
                  </div>
                </div>
              </template>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>selected (temp), still linked</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-del"></span>unlinked — about to be removed</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty deque (Underflow)</td><td>O(1)</td><td>just check <code>rear == null</code> and report the error</td></tr>
                <tr><td>Non-empty deque</td><td>O(1)</td><td>the <code>rear</code> pointer gives direct access to the node being removed &mdash; no traversal is ever needed</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only a temporary reference is used, no extra data structures</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              ...
              <code>prev</code> and <code>next</code>, removing from either end never requires walking
              the list. This is what makes a linked-list deque support <b>O(1)</b> insertion and deletion
              at <b>both</b> ends &mdash; unlike a singly linked list, where reaching the rear (without a
              dedicated <code>rear</code> pointer) would require an O(n) traversal just to find the node
              before it. Always check for <b>Deque Underflow</b> (<code>rear == null</code>) before
              deleting &mdash; deleting from an empty deque is undefined behavior otherwise.
            </p>
          </div>
        </div>
      </div>
