...
function buildSteps(values) {
  ...
  if (nodes.length === 0) {
    ...
    steps.push({
      ...
      badge: 'front != null \u2192 return false \u2192 Queue is Not Empty', code: 'c_false',
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <p>
              ...
              a linked-list queue keeps a direct <code>front</code> pointer, the check is just <code>front == null</code>
            ...
            <ol>
              <li><code>isEmpty()</code> is the guard every other operation should call first: <code>dequeue()</code> and <code>front()</code> both rely on it (directly or via the same <code>front == null</code> check) to detect Queue Underflow before touching any pointers.</li>
              ...
              <li>A single-node queue is <b>not</b> empty — only when <code>front</code> itself is <code>null</code> does the queue qualify as empty, and at that point <code>rear</code> must be <code>null</code> too.</li>
            </ol>
            <h3>Edge Cases</h3>
            <ol>
              ...
              <li><b>Any non-empty queue:</b> regardless of how many nodes exist (one or a thousand), <code>isEmpty()</code> returns <code>false</code> the same way — in O(1).</li>
            </ol>
            <p class="ll-note">
              Note: unlike an array-based queue, there is no companion <code>isFull()</code> to pair with
              <code>isEmpty()</code> here — a linked-list queue has no fixed capacity, so only the empty boundary
              is ever meaningful.
            </p>
          </div>
        </div>
