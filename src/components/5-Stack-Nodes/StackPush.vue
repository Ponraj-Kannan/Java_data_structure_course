...
const CODES = {
  java: [
    ...
    ['c_push', '    void push(int data) {'],
    ['c_nodeNew', '        Node newNode = new Node(data);'],
    ['c_link', '        newNode.next = top;'],
    ['c_settop', '        top = newNode;'],
    ['c_size', '        size++;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
  ...
  python: [
    ...
    ['c_push', '    def push(self, data):'],
    ['c_nodeNew', '        new_node = Node(data)'],
    ['c_link', '        new_node.next = self.top'],
    ['c_settop', '        self.top = new_node'],
    ['c_size', '        self.size += 1'],
  ],
  javascript: [
    ...
    ['c_link', '    newNode.next = this.top;'],
    ['c_settop', '    this.top = newNode;'],
    ['c_size', '    this.size++;'],
    ['', '  }'], ['', '}'],
  ],
};
...
function buildSteps(values) {
  ...
  let nodes = []; // stored top-first: nodes[0] is always the current top
  let topAddr = null;
  let size = 0;
  let addrCounter = 0;
  ...
  steps.push({
    ...
    badge: 'Stack is empty \u2192 top = null', code: 'c_sethead',
  ...
  values.forEach((v) => {
    ...
    steps.push({
      ...
      badge: 'read value ' + v + ' \u2192 call push(' + v + ')', code: 'c_push',
      vars: [mainFrame(), frame('push(' + v + ')', [['data', '' + v]])],
    });

    const newAddr = ADDR(addrCounter++);
    steps.push({
      nodes: [...nodes], topAddr, size, neuAddr: newAddr, pendVal: v,
      badge: 'create newNode { data: ' + v + ', next: null } @' + newAddr, code: 'c_nodeNew',
      vars: [mainFrame(), frame('push(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true]])],
    });

    const wasEmpty = topAddr === null;
    const oldTop = topAddr;
    steps.push({
      nodes: [...nodes], topAddr, size, neuAddr: newAddr, pendVal: v, pendLinked: true,
      badge: wasEmpty
        ? 'newNode.next = top \u2192 top is null, stack was empty'
        : 'newNode.next = top \u2192 new node now points to previous top (' + fmt(oldTop) + ')',
      code: 'c_link',
      vars: [mainFrame(), frame('push(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true]])],
    });
    ...
    topAddr = newAddr;
    steps.push({
      nodes: [...nodes], topAddr, size, neuAddr: newAddr,
      badge: 'top = ' + fmt(newAddr) + ' \u2192 new node becomes the top of the stack', code: 'c_settop',
      vars: [mainFrame(), frame('push(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true]])],
    });
    ...
    steps.push({
      nodes: [...nodes], topAddr, size, neuAddr: newAddr,
      badge: 'size++ \u2192 stack size = ' + size, code: 'c_size',
      vars: [frame('main()', [['size', '' + size], ['top', fmt(topAddr), true]])],
    });
  });

  steps.push({
    nodes: [...nodes], topAddr, size, done: true,
    badge: nodes.length
      ? 'All push operations complete \u2192 stack (top \u2192 bottom): ' + nodes.map((n) => n.val).join(', ')
      : 'All push operations complete \u2192 stack is empty',
    code: 'c_push',
    vars: [frame('main()', [['size', '' + size], ['top', fmt(topAddr), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            ...
            <div class="ll-stage">
              <div v-if="s.pendVal !== undefined" class="ll-nwrap">
                ...
                <div class="ll-node">
                  <div class="ll-box ll-box-new">
                    ...
                    <div class="ll-ptr">
                      <small>next</small>{{ s.pendLinked ? fmt(s.topAddr) : 'null' }}
                    </div>
                  </div>
                  <span class="ll-arrow" v-if="s.pendLinked && s.nodes.length">→</span>
                </div>
              </div>

              <template v-for="(n, idx) in s.nodes" :key="n.addr">
                <div class="ll-nwrap">
                  <div class="ll-who" :class="{ 'll-c-orange': n.addr === s.topAddr }">{{ nodeLabel(n) }}</div>
                  <div class="ll-addr">@{{ n.addr }}</div>
                  <div class="ll-node">
                    <div
                      class="ll-box"
                      :class="{ 'll-box-new': n.addr === s.neuAddr, 'll-box-top': n.addr === s.topAddr }"
                    >
              ...
              <div v-if="!s.nodes.length && s.pendVal === undefined" class="ll-empty-note">stack is empty — top = null</div>
            </div>
          </div>
        </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>current top (highlighted)</span>
        </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm — Push</h3>
            <p>
              Push inserts a new node at the <code>top</code> of the stack. Because the stack keeps a direct
              <code>top</code> pointer, no traversal is needed. The new node's <code>next</code> is set to the
              current <code>top</code> (which may be <code>null</code> if the stack is empty), and then
              <code>top</code> is updated to point at the new node — this is exactly the same assignment whether
              the stack was empty or already had nodes, so <code>push()</code> requires no special-case branch.
            </p>
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Best / Average / Worst</td><td>O(1)</td><td>the <code>top</code> pointer gives direct access — no traversal ever required</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only one new node is allocated per call</td></tr>
              </tbody>
            </table>
            <h3>Key Observations</h3>
            <ol>
              <li>Push and Pop both operate exclusively at the <code>top</code> end of the list — this is what makes both O(1).</li>
              <li><code>newNode.next = top</code> works identically whether <code>top</code> is <code>null</code> (empty stack) or an existing node — no if/else is required in the push logic itself.</li>
              <li>A linked-list stack grows one node at a time and dynamically allocates memory as needed — there is no fixed-size buffer to manage or resize.</li>
            </ol>
            <h3>Edge Cases</h3>
            <ol>
              <li><b>Empty stack:</b> first push sets <code>newNode.next = null</code> and <code>top = newNode</code>, so the new node becomes the only node.</li>
              <li><b>Multiple insertions:</b> each call links <code>newNode.next</code> to the previous top, then advances <code>top</code> — the most recently pushed value is always found first.</li>
            </ol>
            <p class="ll-note">
              Note: unlike an array-based stack, a linked-list stack has <b>no fixed capacity</b> and therefore no
              <code>isFull()</code> check — it can keep growing until system memory is exhausted (a boundary
              condition rather than a design limit). <b>Stack Underflow</b> (removing from an empty stack) is a
              <code>pop()</code> concern, covered in the Pop component.
            </p>
          </div>
        </div>
      </div>
