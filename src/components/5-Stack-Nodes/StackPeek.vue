...
const CODES = {
  java: [
    ...
    ['c_peek', '    int peek() {'],
    ['c_empty', '        if (top == null) {'],
    ['c_isempty', '            System.out.println("Stack is Empty");'],
    ['c_isempty', '            return -1;'],
    ['c_empty', '        }'],
    ['c_return', '        return top.data;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ...
    ['c_peek', 'int peek() {'],
    ['c_empty', '    if (top == NULL) {'],
    ['c_isempty', '        printf("Stack is Empty\\n");'],
    ['c_isempty', '        return -1;'],
    ['c_empty', '    }'],
    ['c_return', '    return top->data;'],
    ['', '}'],
  ...
  cpp: [
    ...
    ['c_peek', 'int peek() {'],
    ['c_empty', '    if (top == NULL) {'],
    ['c_isempty', '        cout << "Stack is Empty" << endl;'],
    ['c_isempty', '        return -1;'],
    ['c_empty', '    }'],
    ['c_return', '    return top->data;'],
    ['', '}'],
  ...
  python: [
    ...
    ['c_isempty', '            print("Stack is Empty")'],
    ['c_isempty', '            return None'],
    ['c_return', '        return self.top.data'],
  ],
  javascript: [
    ...
    ['c_isempty', '      console.log("Stack is Empty");'],
    ['c_isempty', '      return null;'],
    ['c_empty', '    }'],
    ['c_return', '    return this.top.data;'],
    ['', '  }'], ['', '}'],
  ],
};
...
function buildSteps(values) {
  ...
  steps.push({
    ...
    badge: nodes.length
      ...
      : 'Stack starts empty', code: 'c_peek',
    vars: [mainFrame()],
  });

  steps.push({
    nodes: [...nodes], topAddr, size,
    badge: 'call peek() \u2192 check if top == null', code: 'c_empty',
    vars: [mainFrame(), frame('peek()', [])],
  });

  if (!nodes.length) {
    steps.push({
      nodes: [], topAddr: null, size: 0, empty: true, done: true,
      badge: 'top == null \u2192 Stack is Empty, nothing to peek', code: 'c_isempty',
      vars: [mainFrame(), frame('peek()', [])],
    });
    return { steps, phaseStarts };
  }

  const topVal = nodes[0].val;

  steps.push({
    nodes: [...nodes], topAddr, size, curAddr: topAddr,
    badge: 'top != null \u2192 read node at ' + fmt(topAddr) + ' (data = ' + topVal + ')', code: 'c_empty',
    vars: [mainFrame(), frame('peek()', [])],
  });

  steps.push({
    nodes: [...nodes], topAddr, size, curAddr: topAddr,
    badge: 'return top.data \u2192 peek() returns ' + topVal, code: 'c_return',
    vars: [mainFrame(), frame('peek()', [['return', '' + topVal]])],
  });

  steps.push({
    nodes: [...nodes], topAddr, size, curAddr: topAddr, done: true,
    badge: 'Peek complete \u2192 top element = ' + topVal + ' (stack unchanged, no nodes added or removed)', code: 'c_peek',
    vars: [mainFrame()],
  });
      ...
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            ...
            <div class="ll-stage">
              <template v-for="(n, idx) in s.nodes" :key="n.addr">
                <div class="ll-nwrap">
                  ...
                  <div v-if="n.addr === s.curAddr" class="ll-peek-label">peeking…</div>
                </div>
              </template>
            ...
            <div v-if="s.empty" class="ll-underflow-banner">⚠ Stack is Empty — nothing to peek</div>
          </div>
        </div>
        ...
        <!-- LEGEND -->
        <div class="ll-legend">
          <span class="ll-leg"><span class="ll-legdot ll-legdot-existing"></span>stack node (unchanged)</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>top node (being read)</span>
        </div>
        ...
        <div class="ll-badge-wrap">
          <div class="ll-badge" :class="{ 'll-badge-underflow': s.empty }">{{ s.badge }}</div>
        </div>
      </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm — Peek</h3>
            <p>
              Peek reads the value stored at the <code>top</code> of the stack without modifying the stack in
              any way — no node is created, removed, or relinked, and the <code>top</code> pointer itself never
              changes. It simply reports <code>top.data</code> back to the caller.
            </p>
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Best / Average / Worst</td><td>O(1)</td><td>the <code>top</code> pointer gives direct access to the node to read</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no new memory is allocated or freed</td></tr>
              </tbody>
            </table>
            <h3>Key Observations</h3>
            <ol>
              <li>Peek is a read-only operation — it is the only core stack operation that never changes <code>top</code>, <code>size</code>, or any node's <code>next</code> pointer.</li>
              <li>Peek is often used before a <code>pop()</code> to inspect a value without committing to removing it.</li>
              <li>Because <code>top</code> gives direct access, Peek never needs to traverse the list, regardless of stack size.</li>
            </ol>
            <h3>Edge Cases</h3>
            <ol>
              <li><b>Empty stack:</b> calling <code>peek()</code> when <code>top == null</code> — there is nothing to read; the operation should fail gracefully (e.g. print a message or return a sentinel/exception) instead of crashing.</li>
              <li><b>Single-node stack:</b> peek simply reads the one existing node — behavior is identical to any other non-empty case.</li>
            </ol>
            <p class="ll-note">
              Note: Peek's "stack is empty" check is a boundary condition just like Pop's <b>Stack Underflow</b>,
              but because Peek never touches any pointers to begin with, there is nothing to undo — the check
              simply prevents reading through a <code>null</code> pointer.
            </p>
          </div>
        </div>
      </div>
