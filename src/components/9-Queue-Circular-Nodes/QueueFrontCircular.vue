...
const CODES = {
  java: [
    ...
    ['c_emptyRet', '            System.out.println("Queue is empty");'],
    ['c_emptyRet', '            return -1;'],
    ['c_empty', '        }'],
    ['c_return', '        return front.data;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['c_sethead', 'struct Node *front = NULL, *rear = NULL;'],
    ['c_sethead', 'int size = 0;'], ['', ''],
    ['c_front', 'int peekFront() {'],
    ['c_empty', '    if (front == NULL) {'],
    ['c_emptyRet', '        printf("Queue is empty\\n");'],
    ['c_emptyRet', '        return -1;'],
    ['c_empty', '    }'],
    ['c_return', '    return front->data;'],
    ['', '}'],
  ...
  cpp: [
    ...
    ['c_front', 'int peekFront() {'],
    ['c_empty', '    if (front == NULL) {'],
    ['c_emptyRet', '        cout << "Queue is empty" << endl;'],
    ['c_emptyRet', '        return -1;'],
    ['c_empty', '    }'],
    ['c_return', '    return front->data;'],
    ['', '}'],
  ...
  python: [
    ...
    ['c_front', '    def front(self):'],
    ['c_empty', '        if self.front is None:'],
    ['c_emptyRet', '            print("Queue is empty")'],
    ['c_emptyRet', '            return None'],
    ['c_return', '        return self.front.data'],
  ],
  javascript: [
    ...
    ['c_emptyRet', '      console.log("Queue is empty");'],
    ['c_emptyRet', '      return null;'],
    ['c_empty', '    }'],
    ['c_return', '    return this.front.data;'],
    ['', '  }'], ['', '}'],
  ],
};
...
const PSEUDOCODE = [
  ...
  '        return null',
  '    return front.data   // no removal, circular structure unchanged',
];
...
function buildSteps(values) {
  ...
  steps.push({
    ...
    badge: nodes.length
      ...
      : 'Queue starts empty', code: 'c_front',
    vars: [mainFrame()],
  });

  steps.push({
    nodes: [...nodes], frontAddr, rearAddr, size,
    badge: 'call front() \u2014 peek at the front element without removing it', code: 'c_front',
    vars: [mainFrame(), frame('front()', [])],
  });

  if (nodes.length === 0) {
    steps.push({
      nodes: [], frontAddr: null, rearAddr: null, size: 0,
      badge: 'front == null \u2192 queue is empty, nothing to peek', code: 'c_empty',
      vars: [mainFrame(), frame('front()', [])],
    });
    steps.push({
      nodes: [], frontAddr: null, rearAddr: null, size: 0, underflow: true, done: true,
      badge: 'print "Queue is empty" \u2192 return (no value to peek)', code: 'c_emptyRet',
      vars: [mainFrame(), frame('front()', [])],
    });
  } else {
    steps.push({
      nodes: [...nodes], frontAddr, rearAddr, size,
      badge: 'front != null \u2192 queue is not empty, proceed', code: 'c_empty',
      vars: [mainFrame(), frame('front()', [])],
    });
    steps.push({
      nodes: [...nodes], frontAddr, rearAddr, size, curAddr: frontAddr, peekVal: nodes[0].val,
      badge: 'return front.data \u2192 peeked value = ' + nodes[0].val, code: 'c_return',
      vars: [mainFrame(), frame('front()', [['front.data', '' + nodes[0].val]])],
    });
    steps.push({
      nodes: [...nodes], frontAddr, rearAddr, size, curAddr: frontAddr, peekVal: nodes[0].val,
      badge: 'front() complete \u2192 circular queue UNCHANGED (still front \u2192 rear: ' + nodes.map((n) => n.val).join(', ') + ', rear still links back to front)', code: 'c_front', done: true,
      vars: [mainFrame()],
    });
  }
      ...
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            ...
            <div class="ll-stage">
              <template v-for="(n, idx) in s.nodes" :key="n.addr">
                <div class="ll-nwrap">
                  ...
                  <div v-if="n.addr === s.curAddr" class="ll-peek-label">peeked (not removed)</div>
                </div>
              </template>
            ...
            <div class="ll-outbox">
              <span class="ll-outlabel">front() result:</span>
              <span v-if="s.peekVal !== undefined" class="ll-outv">{{ s.peekVal }}</span>
              <span v-else class="ll-outv-muted">—</span>
            </div>

            <div v-if="s.underflow" class="ll-underflow-banner">⚠ Queue is empty — there is no front element to peek</div>
          </div>
        </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>front node (peeked, never removed)</span>
        </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm — Front (Peek) on a Circular Queue</h3>
            <p>
              Front (also called Peek) returns the data stored at the <code>front</code> node <b>without removing it</b>.
              The queue's structure, size, and both pointers remain completely unchanged — including the
              <code>rear.next \u2192 front</code> circular link, which is never touched by this operation.
              This is what distinguishes Front from Dequeue.
            </p>
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Best / Average / Worst</td><td>O(1)</td><td>the <code>front</code> pointer gives direct access to the value</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no nodes are created, removed, or traversed</td></tr>
              </tbody>
            </table>
            <h3>Key Observations</h3>
            <ol>
              <li>Front is a <b>read-only</b> operation — it never mutates <code>front</code>, <code>rear</code>, <code>size</code>, or the circular <code>next</code> links.</li>
              <li>It is commonly used before a <code>dequeue()</code> call to inspect the value that would be removed.</li>
              <li>Because the queue keeps a direct <code>front</code> pointer, this is O(1) regardless of queue length — the circular wraparound doesn't add any traversal cost.</li>
            </ol>
            <h3>Edge Cases</h3>
            <ol>
              <li><b>Empty queue:</b> <code>front == null</code> — there is nothing to peek; the operation should fail gracefully (print a message or return a sentinel/exception) rather than dereference a null pointer.</li>
              <li><b>Single-node queue:</b> <code>front</code> and <code>rear</code> point to the same node, and that node's <code>next</code> points to itself; peeking returns that node's data.</li>
              <li><b>Multiple-node queue:</b> <code>rear.next</code> points back to <code>front</code>; peeking still only reads the <code>front</code> node and never walks the circle.</li>
              <li><b>Repeated calls:</b> calling <code>front()</code> multiple times in a row always returns the same value, since nothing is removed or relinked.</li>
            </ol>
            <p class="ll-note">
              Note: Front never triggers Queue Underflow by itself in the sense of corrupting state — but calling it
              on an empty queue is the same boundary condition as an empty-queue Dequeue, so it must be checked the same way.
            </p>
          </div>
        </div>
      </div>
