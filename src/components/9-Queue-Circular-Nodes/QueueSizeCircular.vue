...
const PSEUDOCODE = [
  'function size():',
  '    // size is a counter field, kept in sync',
  '    // by enqueue() (size++) and dequeue() (size--)',
  '    // unaffected by the rear -> front circular link',
  '    return size',
];
...
function buildSteps(values) {
  ...
  // Values are enqueued in order — exactly like Queue Enqueue — so the FIRST
  // value entered is the front of the queue and the LAST is the rear (true FIFO).
  ...
  steps.push({
    ...
    badge: nodes.length
      ? 'Values enqueued in order: ' + values.join(', ') + ' \u2192 circular queue (front \u2192 rear): ' + nodes.map((n) => n.val).join(', ') + ' \u2014 rear links back to front'
      : 'Queue starts empty (size field = 0)', code: 'c_size',
    vars: [mainFrame()],
  });

  steps.push({
    nodes: [...nodes], frontAddr, rearAddr, size,
    badge: 'call size() \u2192 the size field has already been kept in sync by enqueue() (size++) and dequeue() (size--)', code: 'c_size',
    vars: [mainFrame(), frame('size()', [])],
  });

  steps.push({
    nodes: [...nodes], frontAddr, rearAddr, size, reading: true,
    badge: 'return size \u2192 no traversal around the circle from front to rear is needed to answer this', code: 'c_return',
    vars: [mainFrame(), frame('size()', [['return', '' + size]])],
  });

  steps.push({
    nodes: [...nodes], frontAddr, rearAddr, size, done: true,
    badge: 'Size complete \u2192 queue currently has ' + size + (size === 1 ? ' element' : ' elements') + ' (queue unchanged, O(1))', code: 'c_size',
    vars: [mainFrame()],
  });
  ...
  <div class="slide-wrapper">
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip" :class="{ 'll-chip-active': s.code === 'c_return' }">
                size = <b class="ll-c-orange">{{ s.size }}</b>
              </div>
            </div>
            ...
            <div class="ll-stage">
              ...
              <div v-if="!s.nodes.length" class="ll-empty-note">queue is empty — front = rear = null, size = 0</div>
            ...
            <div v-if="s.reading" class="ll-size-banner">no nodes are visited — size is a maintained counter, not computed by traversal (even around the circle)</div>
          </div>
        </div>
        ...
        <!-- LEGEND -->
        <div class="ll-legend">
          <span class="ll-leg"><span class="ll-legdot ll-legdot-existing"></span>queue node (never read or modified)</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-size"></span>size counter (what size() actually reads)</span>
        </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm — Size (Circular Queue)</h3>
            <p>
              Size reports how many elements are currently in the queue. A well-implemented circular linked-list
              queue keeps the same dedicated <code>size</code> field on the queue object as a plain linked-list
              queue, incrementing it inside <code>enqueue()</code> and decrementing it inside <code>dequeue()</code>.
              Because that bookkeeping is done as each element is added or removed at the <code>rear</code> or
              <code>front</code>, <code>size()</code> itself does nothing more than return the already-current
              value of that field — it never has to walk the circle from <code>front</code> back around to
              <code>front</code> again.
            </p>
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Best / Average / Worst</td><td>O(1)</td><td>the <code>size</code> field is read directly — no traversal around the circular chain</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no new memory is allocated; a single counter is reused for the queue's lifetime</td></tr>
              </tbody>
            </table>
            <h3>Key Observations</h3>
            <ol>
              <li>Size is only O(1) because <code>enqueue()</code> and <code>dequeue()</code> maintain the counter as a side effect — <code>size()</code> is "free" precisely because the work was already done elsewhere.</li>
              <li>A circular queue implementation that skips the counter field would instead need to traverse from <code>front</code>, counting nodes until it loops back to <code>front</code> again — that alternative is O(n) instead of O(1), and is also how you'd detect the "end" of a circular list at all, since there's no <code>null</code> to stop at.</li>
              <li>Size is a read-only operation: it never changes <code>front</code>, <code>rear</code>, <code>size</code> itself, or the <code>rear.next \u2192 front</code> circular link.</li>
            </ol>
            <h3>Edge Cases</h3>
            <ol>
              <li><b>Empty queue:</b> <code>size()</code> simply returns <code>0</code> — unlike <code>dequeue()</code>, there is no error case to guard against, since asking "how many?" is always a valid question, even for an empty queue.</li>
              <li><b>Single-node queue:</b> the counter correctly reads <code>1</code>, even though that node's <code>next</code> loops back to itself.</li>
              <li><b>Keeping the counter correct:</b> any code path that changes <code>front</code> or <code>rear</code> directly (bypassing <code>enqueue()</code>/<code>dequeue()</code>) would leave <code>size</code> out of sync with reality — the counter is only as reliable as the discipline used to update it.</li>
            </ol>
            <p class="ll-note">
              Note: unlike Dequeue, Size has no underflow-style boundary condition to check — there is no
              invalid input on which <code>size()</code> can fail, which is what makes it the simplest of the
              core queue operations to reason about, circular or not.
            </p>
          </div>
        </div>
      </div>
...
.ll-chip-active {
  ...
  animation: ll-chip-pulse 1s ease-in-out infinite;
}
