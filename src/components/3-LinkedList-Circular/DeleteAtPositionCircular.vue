...
const CODES = {
  java: [
    ...
    ['c_del', '    void deleteAtIndex(int pos) {'],
    ['c_empty', '        if (head == null) return;'],
    ['c_size', '        int size = 1;'],
    ['c_size', '        Node t = head;'],
    ['c_size', '        while (t.next != head) { t = t.next; size++; }'],
    ['c_invalid', '        if (pos < 1 || pos > size) return;'],
    ['c_pos1', '        if (pos == 1) {'],
    ['c_single', '            if (head.next == head) {'],
    ['c_single', '                head = null;'],
    ['c_single', '                return;'],
    ['c_single', '            }'],
    ['c_findlast', '            Node last = head;'],
    ['c_findlast', '            while (last.next != head) last = last.next;'],
    ['c_newhead', '            head = head.next;'],
    ['c_relinklast', '            last.next = head;'],
    ['c_relinklast', '            return;'],
    ['c_pos1', '        }'],
    ['c_travstart', '        Node prev = head;'],
    ['c_travstart', '        int count = 1;'],
    ['c_travloop', '        while (count < pos - 1) {'],
    ['c_travmove', '            prev = prev.next;'],
    ['c_travmove', '            count++;'],
    ['c_travloop', '        }'],
    ['c_target', '        Node curr = prev.next;'],
    ['c_relink', '        prev.next = curr.next;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ...
    ['c_single', '            return;'],
    ['c_single', '        }'],
    ['c_findlast', '        struct Node *last = head;'],
    ['c_findlast', '        while (last->next != head) last = last->next;'],
    ['c_newhead', '        head = head->next;'],
    ['c_relinklast', '        last->next = head;'],
    ['c_relinklast', '        return;'],
    ...
    ['c_travmove', '        count++;'],
    ['c_travloop', '    }'],
    ['c_target', '    struct Node *curr = prev->next;'],
    ['c_relink', '    prev->next = curr->next;'],
    ['', '}'],
  ],
  cpp: [
  ...
  python: [
    ...
    ['c_del', '    def delete_at_index(self, pos):'],
    ['c_empty', '        if self.head is None: return'],
    ['c_size', '        size = 1'],
    ['c_size', '        t = self.head'],
    ['c_size', '        while t.next != self.head:'],
    ['c_size', '            t = t.next'],
    ['c_size', '            size += 1'],
    ['c_invalid', '        if pos < 1 or pos > size: return'],
    ['c_pos1', '        if pos == 1:'],
    ['c_single', '            if self.head.next == self.head:'],
    ['c_single', '                self.head = None'],
    ['c_single', '                return'],
    ['c_findlast', '            last = self.head'],
    ['c_findlast', '            while last.next != self.head:'],
    ['c_findlast', '                last = last.next'],
    ['c_newhead', '            self.head = self.head.next'],
    ['c_relinklast', '            last.next = self.head'],
    ['c_relinklast', '            return'],
  ...
  javascript: [
    ...
    ['c_del', '  deleteAtIndex(pos) {'],
    ['c_empty', '    if (this.head === null) return;'],
    ['c_size', '    let size = 1;'],
    ['c_size', '    let t = this.head;'],
    ['c_size', '    while (t.next !== this.head) { t = t.next; size++; }'],
    ['c_invalid', '    if (pos < 1 || pos > size) return;'],
    ['c_pos1', '    if (pos === 1) {'],
    ...
    ['c_single', '        return;'],
    ['c_single', '      }'],
    ['c_findlast', '      let last = this.head;'],
    ['c_findlast', '      while (last.next !== this.head) last = last.next;'],
    ['c_newhead', '      this.head = this.head.next;'],
    ['c_relinklast', '      last.next = this.head;'],
    ['c_relinklast', '      return;'],
    ['c_pos1', '    }'],
    ['c_travstart', '    let prev = this.head;'],
    ['c_travstart', '    let count = 1;'],
    ['c_travloop', '    while (count < pos - 1) {'],
    ['c_travmove', '      prev = prev.next;'],
    ['c_travmove', '      count++;'],
    ['c_travloop', '    }'],
    ['c_target', '    const curr = prev.next;'],
    ['c_relink', '    prev.next = curr.next;'],
    ['', '  }'], ['', '}'],
  ],
};
...
function buildSteps(values, pos) {
  ...
  steps.push({
    nodes: [...nodes], headAddr, size, pos,
    badge: 'Initial list with ' + size + ' node(s) \u2014 deleteAtIndex(' + pos + ') called',
    code: 'c_del',
  ...
  if (size === 0) {
    steps.push({
      nodes: [], headAddr: null, size, pos,
      badge: 'head == null \u2192 list is empty, nothing to delete',
      code: 'c_empty',
      vars: [frame('main()', [['head', 'null', true]]), frame('deleteAtIndex(pos)', [])],
    });
  ...
  steps.push({
    ...
    code: 'c_empty',
    vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [])],
  });

  steps.push({
    nodes: [...nodes], headAddr, size, pos,
    badge: 'Traverse once to compute the list size \u2192 size = ' + size,
    code: 'c_size',
    curAddr: nodes[size - 1].addr, curLabel: 't',
    vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false]])],
  });

  const isValid = pos >= 1 && pos <= size;
  if (!isValid) {
    steps.push({
      nodes: [...nodes], headAddr, size, pos, invalid: true,
      badge: 'pos (' + pos + ') is out of range [1, ' + size + '] \u2192 invalid position, deletion aborted. List remains unchanged.',
      code: 'c_invalid',
      vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false]])],
    });
    return { steps, phaseStarts };
  ...
  steps.push({
    nodes: [...nodes], headAddr, size, pos,
    badge: 'pos (' + pos + ') is between 1 and ' + size + ' \u2192 valid, proceed with deletion',
    code: 'c_invalid',
    vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false]])],
  });
  ...
  if (pos === 1) {
    if (size === 1) {
      steps.push({
        nodes: [...nodes], headAddr, size, pos,
        badge: 'pos == 1 and head.next == head \u2192 only one node exists',
        code: 'c_single',
        delAddr: nodes[0].addr, delLabel: 'head',
        vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false]])],
      });
      const deletedAddr = nodes[0].addr;
      nodes = []; headAddr = null;
      steps.push({
        nodes: [], headAddr: null, size: 0, pos,
      ...
      steps.push({
        nodes: [...nodes], headAddr, size, pos,
        badge: 'pos == 1 \u2192 deleting the head node @' + headAddr,
        code: 'c_pos1',
        delAddr: nodes[0].addr, delLabel: 'old head',
        vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false]])],
      });

      steps.push({
        nodes: [...nodes], headAddr, size, pos,
        badge: 'last = head \u2192 traverse to find the last node (its next must point to the new head)',
        code: 'c_findlast',
        curAddr: nodes[0].addr, curLabel: 'last',
        delAddr: nodes[0].addr, delLabel: 'old head',
        vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['last', fmt(nodes[0].addr), true]])],
      });
      ...
      for (let t = 0; t < size - 1; t++) {
        steps.push({
          ...
          code: 'c_findlast',
          curAddr: nodes[t + 1].addr, curLabel: 'last',
          delAddr: nodes[0].addr, delLabel: 'old head',
          vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['last', fmt(nodes[t + 1].addr), true]])],
        });
      }
      ...
      steps.push({
        nodes: [...nodes], headAddr, size, pos,
        badge: 'last.next == head \u2192 last node found @' + nodes[lastIdx].addr,
        code: 'c_findlast',
        curAddr: nodes[lastIdx].addr, curLabel: 'last',
        delAddr: nodes[0].addr, delLabel: 'old head',
        vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['last', fmt(nodes[lastIdx].addr), true]])],
      });

      const newHeadAddr = nodes[1].addr;
      steps.push({
        nodes: [...nodes], headAddr, size, pos,
        badge: 'head = head.next \u2192 new head becomes @' + newHeadAddr,
        code: 'c_newhead',
        curAddr: nodes[lastIdx].addr, curLabel: 'last',
        delAddr: nodes[0].addr, delLabel: 'old head',
        vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['last', fmt(nodes[lastIdx].addr), true]])],
      ...
      steps.push({
        nodes: [...nodes], headAddr, size, pos,
        badge: 'last.next = head \u2192 circular link now points to the new head @' + newHeadAddr,
        code: 'c_relinklast',
        curAddr: nodes[lastIdx].addr, curLabel: 'last',
        delAddr: nodes[0].addr, delLabel: 'old head',
        vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['last', fmt(nodes[lastIdx].addr), true]])],
      ...
      nodes = nodes.slice(1);
      steps.push({
        nodes: [...nodes], headAddr, size: nodes.length, pos,
        badge: 'Node @' + deletedAddr + ' deleted \u2192 list now has ' + nodes.length + ' node(s), head updated',
        code: 'c_del',
        vars: [frame('main()', [['head', fmt(headAddr), true]])],
      });
    }
    ...
    steps.push({
      nodes: [...nodes], headAddr, size, pos,
      badge: 'prev = head, count = 1 \u2192 begin traversal toward position ' + (pos - 1),
      code: 'c_travstart',
      curAddr: nodes[0].addr, curLabel: 'prev',
      vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['prev', fmt(nodes[0].addr), true], ['count', 1, false]])],
    });
    ...
    for (let t = 0; t < prevIdx; t++) {
      steps.push({
        nodes: [...nodes], headAddr, size, pos,
        badge: 'count (' + (t + 1) + ') < pos - 1 \u2192 prev = prev.next, count = ' + (t + 2),
        code: 'c_travmove',
        curAddr: nodes[t + 1].addr, curLabel: 'prev',
        vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['prev', fmt(nodes[t + 1].addr), true], ['count', t + 2, false]])],
      });
    }

    steps.push({
      nodes: [...nodes], headAddr, size, pos,
      badge: 'count == pos - 1 \u2192 prev found at position ' + (pos - 1) + ' (@' + nodes[prevIdx].addr + ')',
      code: 'c_travloop',
      curAddr: nodes[prevIdx].addr, curLabel: 'prev',
      vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['prev', fmt(nodes[prevIdx].addr), true], ['count', pos - 1, false]])],
    });

    const targetIdx = pos - 1;
    steps.push({
      nodes: [...nodes], headAddr, size, pos,
      badge: 'curr = prev.next \u2192 target node at position ' + pos + ' found (@' + nodes[targetIdx].addr + ')',
      code: 'c_target',
      curAddr: nodes[prevIdx].addr, curLabel: 'prev',
      delAddr: nodes[targetIdx].addr, delLabel: 'curr (target)',
      vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['prev', fmt(nodes[prevIdx].addr), true], ['curr', fmt(nodes[targetIdx].addr), true]])],
    });

    steps.push({
      nodes: [...nodes], headAddr, size, pos,
      badge: 'prev.next = curr.next \u2192 bypass node @' + nodes[targetIdx].addr,
      code: 'c_relink',
      curAddr: nodes[prevIdx].addr, curLabel: 'prev',
      delAddr: nodes[targetIdx].addr, delLabel: 'curr (target)',
      vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtIndex(pos)', [['size', size, false], ['prev', fmt(nodes[prevIdx].addr), true], ['curr', fmt(nodes[targetIdx].addr), true]])],
    ...
    const deletedAddr = nodes[targetIdx].addr;
  ...
  steps.push({
    nodes: [...nodes], headAddr, size: nodes.length, pos,
    badge: 'deleteAtIndex(' + pos + ') complete \u2014 final circular list has ' + nodes.length + ' node(s)',
    code: 'c_del',
    vars: [frame('main()', [['head', fmt(headAddr), true]])],
  });
...
const stepsData = reactive(buildSteps([10, 20, 30, 40], 3));
...
function applyInput() {
  ...
  const pos = parseInt(inpPosition.value, 10);
  if (isNaN(pos)) { alert('Enter a valid position.'); return; }
  inpPosition.value = pos;
  playing.value = false;
...
function nodeLabel(n) {
  ...
  if (n.addr === s.value.curAddr) labels.push((s.value.curLabel || 'prev') + ' \u2192');
  if (n.addr === s.value.delAddr && n.addr !== s.value.curAddr) labels.push((s.value.delLabel || 'target') + ' \u2192');
  return labels.join(' ');
}
...
<template>
  <div class="slide-wrapper">
    ...
    <div class="ll-toolbar">
      ...
      <input type="number" v-model="inpPosition" class="ll-num-input" title="1-indexed position to delete" />
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip">size = <b class="ll-c-green">{{ s.size }}</b></div>
              <div class="ll-ptr-chip">pos = <b class="ll-c-orange">{{ s.pos }}</b></div>
              <div v-if="s.curAddr !== undefined" class="ll-ptr-chip">
                {{ s.curLabel || 'prev' }} = <b class="ll-c-purple">{{ fmt(s.curAddr) }}</b>
              </div>
              <div v-if="s.delAddr !== undefined" class="ll-ptr-chip">
                {{ s.delLabel || 'target' }} = <b class="ll-c-red">@{{ s.delAddr }}</b>
              </div>
            </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>prev / last pointer</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-del"></span>node to delete (target)</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Best / Average / Worst</td><td>O(n)</td><td>must traverse to compute the size and to reach the target position</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra data structures needed, only a few pointers (prev / curr / last)</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Tip: unlike a singly <b>linear</b> list, deleting the <b>head</b> (position 1) of a circular
              list is still <b>O(n)</b> &mdash; the last node's <code>next</code> pointer must be updated
              to point at the new head, which means traversing all the way around the list (unless a
              <code>tail</code> pointer is maintained, which drops this case to <b>O(1)</b>).
            </p>
          </div>
        </div>
      </div>
