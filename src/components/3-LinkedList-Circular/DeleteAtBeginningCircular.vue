...
const CODES = {
  ...
  python: [
    ...
    ['c_del', '    def delete_at_beginning(self):'],
...
function buildSteps(values) {
  ...
  steps.push({
    ...
    badge: 'Initial list with ' + nodes.length + ' node(s) — deleteAtBeginning() called',
    code: 'c_del',
  ...
  while (nodes.length > 0) {
    steps.push({
      ...
      code: 'c_empty',
      vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtBeginning()', [])],
    });
    ...
    if (nodes.length === 1) {
      ...
      const oldHeadAddr = nodes[0].addr;

      steps.push({
        nodes: [...nodes], headAddr,
        badge: 'head.next != head \u2192 multiple nodes exist, @' + oldHeadAddr + ' will be removed',
        code: 'c_single',
        delAddr: oldHeadAddr,
        vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtBeginning()', [['last', fmt(nodes[0].addr), true]])],
      });
      ...
      for (let t = 0; t < lastIdx; t++) {
        steps.push({
          ...
          delAddr: oldHeadAddr,
          vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtBeginning()', [['last', fmt(nodes[t + 1].addr), true]])],
        });
      }
      ...
      steps.push({
        ...
        code: 'c_travloop',
        curAddr: nodes[lastIdx].addr,
        delAddr: oldHeadAddr,
        vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtBeginning()', [['last', fmt(nodes[lastIdx].addr), true]])],
      });

      const newHeadAddr = nodes[1].addr;
      steps.push({
        nodes: [...nodes], headAddr: newHeadAddr,
        badge: 'head = head.next \u2192 head pointer moves to @' + newHeadAddr,
        code: 'c_newhead',
        curAddr: nodes[lastIdx].addr,
        delAddr: oldHeadAddr,
        vars: [frame('main()', [['head', fmt(newHeadAddr), true]]), frame('deleteAtBeginning()', [['last', fmt(nodes[lastIdx].addr), true]])],
      });

      steps.push({
        nodes: [...nodes], headAddr: newHeadAddr,
        badge: 'last.next = head \u2192 @' + nodes[lastIdx].addr + ' now points to new head @' + newHeadAddr + ' (circular structure preserved)',
        code: 'c_relink',
        curAddr: nodes[lastIdx].addr,
        delAddr: oldHeadAddr,
        vars: [frame('main()', [['head', fmt(newHeadAddr), true]]), frame('deleteAtBeginning()', [['last', fmt(nodes[lastIdx].addr), true]])],
      ...
      steps.push({
        nodes: [...nodes], headAddr,
        badge: 'Node @' + oldHeadAddr + ' deleted \u2192 list now has ' + nodes.length + ' node(s)',
        code: 'c_del',
        vars: [frame('main()', [['head', fmt(headAddr), true]])],
      });
    }
...
function nodeLabel(n) {
  ...
  if (n.addr === s.value.delAddr && n.addr !== s.value.headAddr) labels.push('delete \u2192');
  return labels.join(' ');
}
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
          ...
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.delAddr !== undefined" class="ll-ptr-chip">
                delete = <b class="ll-c-red">@{{ s.delAddr }}</b>
              </div>
            </div>
            ...
            <div class="ll-stage">
              <template v-for="(n, idx) in s.nodes" :key="n.addr">
                <div class="ll-nwrap">
                  <div class="ll-who" :class="{ 'll-c-red': n.addr === s.delAddr && n.addr !== s.headAddr }">{{ nodeLabel(n) }}</div>
              ...
              <div v-if="s.nodes.length > 0" class="ll-wraparrow" :class="{ 'll-wraparrow-hot': s.code === 'c_newhead' || s.code === 'c_relink' }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>last pointer</span>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Best / Average / Worst</td><td>O(n)</td><td>must traverse to the last node so its <code>next</code> pointer can be repointed to the new head</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra data structures needed, only a <code>last</code> pointer</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Tip: if the list keeps a <code>tail</code> pointer (common optimization), deletion
              at the beginning drops to <b>O(1)</b> &mdash; you can update <code>tail.next</code>
              and <code>head</code> directly without any traversal at all.
            </p>
          </div>
        </div>
      </div>
