...
const props = defineProps({
  topic:    { type: String, default: 'Binary Tree — Postorder Traversal' },
  subTopic: { type: String, default: 'Left → Right → Root Recursive Traversal' },
});
...
const CODES = {
  java: [
    ...
    ['', '    Node root;'],
    ['', ''],
    ['c_call',       '    void postorder(Node node) {'],
    ['c_nullCheck',  '        if (node == null) {'],
    ['c_nullReturn', '            return;'],
    ['',             '        }'],
    ['c_recurLeft',  '        postorder(node.left);'],
    ['c_recurRight', '        postorder(node.right);'],
    ...
    ['c_main',       '        tree.postorder(tree.root);'],
    ['c_main',       '    }'],
    ['', '}'],
  ],
  c: [
    ['c_call',       'void postorder(struct Node* node) {'],
    ['c_nullCheck',  '    if (node == NULL) {'],
    ['c_nullReturn', '        return;'],
    ['',             '    }'],
    ['c_recurLeft',  '    postorder(node->left);'],
...
function buildSteps(nodes, edges, rootId) {
  ...
  if (rootId === null) {
    snap(null, 'Tree is empty — postorder traversal cannot run.', 'c_main');
    return steps;
  }

  snap(null, 'main(): Start Postorder Traversal by calling postorder(root).', 'c_main');

  function traverse(nodeId) {
    const isNull = nodeId === null || nodeId === undefined;
    const node   = isNull ? null : nodesMap[nodeId];

    const frameTitle = `postorder(node = ${isNull ? 'null' : fmt(ADDR(nodeId))})`;
    const frameRows  = [
    ...
    snap(nodeId, `Recurse LEFT: call postorder(node.left = ${childAddrHelper(node.left)})`, 'c_recurLeft');
    traverse(node.left);

    // Step 4: Recurse Right Subtree
    snap(nodeId, `Left subtree done for node ${node.val}. Recurse RIGHT: call postorder(node.right = ${childAddrHelper(node.right)})`, 'c_recurRight');
    traverse(node.right);

    // Step 5: Visit Node (Print Root Data - ONLY after both subtrees done!)
    visitedList.push(node.val);
    snap(nodeId, `Both subtrees completed for node @${node.addr} → Visit & process node data: ${node.val}. Output: [${visitedList.join(', ')}]`, 'c_visit');

    // Step 6: Function Return
    snap(nodeId, `Completed postorder traversal for node ${node.val} (@${node.addr}). Pop stack frame & return.`, 'c_return');
    stackFrames.pop();
  }

  traverse(rootId);
  snap(null, `Postorder Traversal Complete! Final Output: [${visitedList.join(', ')}]`, 'c_main');
          ...
          <div class="ll-main" ref="mainRef">
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              ...
              <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
                <div class="ll-perm-area" ref="vizSvgRef">
                  <div class="ll-ptrs">
                    ...
                    <div class="ll-ptr-chip pso-result-chip">Postorder: <b>[ {{ s.visitedList ? s.visitedList.join(', ') : '' }} ]</b></div>
                  </div>
              ...
              <div class="ll-legend">
                ...
                <span class="ll-leg"><span class="ll-legdot ll-legdot-visited"></span>visited (printed in postorder)</span>
              </div>
            ...
            <div class="ll-right-col">
              <div class="ll-code-panel">
                ...
                <div v-else class="ll-info-scroll">
                  <h3>Time &amp; Space Complexity &mdash; Postorder Traversal</h3>
                  ...
                  <p class="ll-note">
                    <b>Postorder Pattern: Left &rarr; Right &rarr; Root</b><br/>
                    Root is processed <i>last</i>, only after both left and right subtrees have been completely traversed. Essential for deleting/freeing tree nodes safely or evaluating postfix (RPN) expressions.
                  </p>
                  <h3>Algorithm Steps</h3>
                  <p>1. Check base case: if <code>node == null</code>, return.<br/>
                     2. Recursively traverse left subtree: <code>postorder(node.left)</code>.<br/>
                     3. Recursively traverse right subtree: <code>postorder(node.right)</code>.<br/>
                     4. Visit current node (print/process <code>node.data</code>).</p>
                </div>
              </div>
            </div>
          </div>
...
.pso-result-chip { background: var(--green-light); border-color: var(--green); color: #15803d; font-weight: 600; }
