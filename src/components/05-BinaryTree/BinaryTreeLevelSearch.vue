...
const props = defineProps({
  topic:    { type: String, default: 'Binary Tree — Search (Level Order / BFS)' },
  subTopic: { type: String, default: 'Queue-based Level Order Search' },
});
...
const CODES = {
  java: [
    ...
    ['', '    Node root;'],
    ['', ''],
    ['c_call',        '    boolean search(int target) {'],
    ...
    ['c_checkEqual',  '            if (curr.data == target) {'],
    ['c_found',       '                return true;'],
    ['',              '            }'],
    ...
    ['c_main',        '        boolean result = tree.search(target);'],
    ['c_main',        '    }'],
    ['', '}'],
  ],
  c: [
    ['c_call',        'int search(struct Node* root, int target) {'],
    ['c_nullCheck',   '    if (root == NULL) {'],
    ...
    ['c_main',        'int main() {'],
    ['c_main',        '    // ... build tree level order ...'],
    ['c_main',        '    int result = search(root, target);'],
    ['c_main',        '    return 0;'],
    ['',              '}'],
  ],
  cpp: [
    ['c_call',        'bool search(Node* root, int target) {'],
    ...
    ['c_main',        'int main() {'],
    ['c_main',        '    // ... build tree level order ...'],
    ['c_main',        '    bool result = search(root, target);'],
    ['c_main',        '    return 0;'],
    ['',              '}'],
  ],
  python: [
    ['c_call',        'def search(root, target):'],
    ...
    ['', ''],
    ['c_main',        '# build tree level order ...'],
    ['c_main',        'result = search(root, target)'],
  ],
};
...
function buildSteps(nodes, edges, rootId, target) {
  ...
  function snap(currId, foundId, badge, code) {
    steps.push({
      nodes:        nodes.map(n => ({ ...n })),
      edges:        edges.map(e => ({ ...e })),
      rootId,
  ...
  if (rootId === null) {
    snap(null, null, 'Tree is empty — search cannot run.', 'c_notFound');
    return steps;
  }

  snap(null, null, `search(target = ${target}) called. Starting Level Order (BFS) Search.`, 'c_main');
  snap(null, null, `Call search(root, target = ${target})`, 'c_call');

  const rootNull = rootId === null;
  snap(null, null, `Check: root == null? → ${rootNull ? 'TRUE (empty tree)' : 'FALSE'}`, 'c_nullCheck');

  if (rootNull) {
    snap(null, null, 'root == null → Target not found. return false.', 'c_nullReturn');
    return steps;
  }
  ...
  queue.push(rootId);
  snap(null, null, `q.add(root) → Enqueue root (@${ADDR(rootId)}, val=${nodesMap[rootId].val}) into queue.`, 'c_enqueueRoot');

  let isFound = false;

  while (queue.length > 0) {
    snap(null, null, `while (!q.isEmpty()) → ${queue.length} node(s) in queue. Continue search.`, 'c_whileLoop');

    const currId = queue.shift();
    const curr   = nodesMap[currId];
    snap(currId, null, `Dequeue: curr = q.poll() → curr = @${curr.addr} (val=${curr.val}).`, 'c_dequeue');

    const isEqual = curr.val === target;
    snap(currId, null, `Compare: if (${curr.val} == ${target}) → ${isEqual ? 'TRUE — Target Found!' : 'FALSE — No match'}`, 'c_checkEqual');

    if (isEqual) {
      snap(currId, currId, `Target ${target} FOUND at node @${curr.addr}! return true.`, 'c_found');
      isFound = true;
      break;
    }
    ...
    snap(currId, null, `Check: curr.left != null? → ${leftExist ? `TRUE (@${ADDR(curr.left)}, val=${nodesMap[curr.left].val})` : 'FALSE'}`, 'c_leftCheck');
    if (leftExist) {
      queue.push(curr.left);
      snap(currId, null, `q.add(curr.left) → Enqueue left child @${ADDR(curr.left)} (val=${nodesMap[curr.left].val}).`, 'c_enqueueLeft');
    }

    const rightExist = curr.right !== null;
    snap(currId, null, `Check: curr.right != null? → ${rightExist ? `TRUE (@${ADDR(curr.right)}, val=${nodesMap[curr.right].val})` : 'FALSE'}`, 'c_rightCheck');
    if (rightExist) {
      queue.push(curr.right);
      snap(currId, null, `q.add(curr.right) → Enqueue right child @${ADDR(curr.right)} (val=${nodesMap[curr.right].val}).`, 'c_enqueueRight');
    }
  }

  if (!isFound) {
    snap(null, null, `while (!q.isEmpty()) → Queue is EMPTY! Target ${target} NOT FOUND in Binary Tree. return false.`, 'c_notFound');
  }
...
const currentStep = computed(() => steps.value[si.value] || {
  nodes: [], edges: [], vars: [], badge: '', rootId: null, currId: null, foundId: null, visitedArr: [], queueIds: [],
});
...
const treeLayout = computed(() => {
  ...
  if (!step || !step.nodes || !step.nodes.length) {
    return { positions: {}, edges: [], viewBox: FIXED_VIEWBOX,
             nodeW: NODE_W, nodeBoxH: NODE_BOX_H, nodeH: NODE_H };
  }
            ...
            <div class="ll-right-col">
              <div class="ll-code-panel">
                ...
                <div v-else class="ll-info-scroll">
                  <h3>Time &amp; Space Complexity &mdash; Binary Tree Level Order Search</h3>
                  <table class="ll-complexity-table">
                    <thead><tr><th>Case</th><th>Time</th><th>Why</th></tr></thead>
                    <tbody>
                      <tr><td>Best Case</td><td>O(1)</td><td>Target is at the root node &mdash; found on the first comparison.</td></tr>
                      <tr><td>Average Case</td><td>O(N)</td><td>Unsorted tree: must search level by level until target is found.</td></tr>
                      <tr><td>Worst Case</td><td>O(N)</td><td>Target is at the last node or not present in the tree.</td></tr>
                      <tr><td>Space (Queue)</td><td>O(W) &approx; O(N)</td><td>Queue holds at most W nodes at the widest level (up to N/2 nodes).</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    <b>Why BFS Search for Unsorted Binary Trees?</b><br/>
                    Unlike Binary Search Trees (BST), a general Binary Tree does not satisfy the BST ordering property. Thus, any search algorithm (BFS or DFS) must potentially inspect all N nodes in the worst case.
                  </p>
                  ...
                  <p>1. If <code>root == null</code>, return false.<br/>
                     ...
                     3. While <code>q</code> is not empty:<br/>
                        ...
                        &nbsp;&nbsp;&bull; If <code>curr.data == target</code> &rarr; return <b>true</b> (Target Found!).<br/>
