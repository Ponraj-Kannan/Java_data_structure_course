...
const props = defineProps({
  topic:    { type: String, default: 'Binary Tree — Total Number of Nodes' },
  subTopic: { type: String, default: 'Recursive Traversal Approach (Left + Right + 1)' },
});
...
const CODES = {
  java: [
    ...
    ['',              '    Node root;'],
    ['',              ''],
    ['c_call',        '    int countNodes(Node node) {'],
    ['c_nullCheck',   '        if (node == null) {'],
    ['c_nullReturn',  '            return 0;'],
    ['',              '        }'],
    ['c_recurLeft',   '        int leftCount  = countNodes(node.left);'],
    ['c_recurRight',  '        int rightCount = countNodes(node.right);'],
    ['c_compute',     '        int count = leftCount + rightCount + 1;'],
    ...
    ['c_main',        '        int total = tree.countNodes(tree.root);'],
    ['c_main',        '        System.out.println("Total Nodes: " + total);'],
    ['c_main',        '    }'],
    ['',              '}'],
  ],
  c: [
    ['c_call',        'int countNodes(struct Node* node) {'],
    ['c_nullCheck',   '    if (node == NULL) {'],
    ['c_nullReturn',  '        return 0;'],
    ['',              '    }'],
    ['c_recurLeft',   '    int leftCount  = countNodes(node->left);'],
    ...
    ['c_main',        'int main() {'],
    ['c_main',        '    // ... build tree level order ...'],
    ['c_main',        '    int total = countNodes(root);'],
    ['c_main',        '    printf("Total Nodes: %d\\n", total);'],
    ['c_main',        '    return 0;'],
    ['',              '}'],
  ],
  cpp: [
    ...
    ['c_main',        '    // ... build tree level order ...'],
    ['c_main',        '    int total = countNodes(root);'],
    ['c_main',        '    cout << "Total Nodes: " << total << endl;'],
    ['c_main',        '    return 0;'],
    ['',              '}'],
  ],
  python: [
    ['c_call',        'def count_nodes(node):'],
    ['c_nullCheck',   '    if node is None:'],
    ['c_nullReturn',  '        return 0'],
    ['c_recurLeft',   '    left_count  = count_nodes(node.left)'],
    ['c_recurRight',  '    right_count = count_nodes(node.right)'],
    ['c_compute',     '    count = left_count + right_count + 1'],
    ['c_return',      '    return count'],
    ['',              ''],
    ['c_main',        '# build tree level order ...'],
    ['c_main',        'total = count_nodes(root)'],
    ['c_main',        'print("Total Nodes:", total)'],
  ],
};
...
const PSEUDOCODE = [
  ...
  '    return count',
  '',
  '// Total Nodes = 1 + left subtree count + right subtree count',
];
...
function buildSteps(nodes, edges, rootId) {
  ...
  if (rootId === null) {
    totalCount = 0;
    snap(null, 'Tree is empty — countNodes returns 0.', 'c_main', {});
    return steps;
  }

  const nodeCounts = {};
  snap(null, 'main(): Start counting nodes by calling countNodes(root).', 'c_main', nodeCounts);

  function countNodes(nodeId) {
    const isNull = nodeId === null || nodeId === undefined;
    const node   = isNull ? null : nodesMap[nodeId];
    const nodeStr = isNull ? 'null' : fmt(ADDR(nodeId));

    stackFrames.push({
      title: 'countNodes(node = ' + nodeStr + ')',
      rows:  [['node', nodeStr, true], ...(isNull ? [] : [['node.data', '' + node.val]])],
    });

    snap(nodeId, 'Call countNodes(node = ' + nodeStr + ')', 'c_call', nodeCounts);
    ...
    snap(nodeId, 'Recurse LEFT: call countNodes(node.left = ' + childAddrStr(node.left) + ')', 'c_recurLeft', nodeCounts);
    const leftCount = countNodes(node.left);

    const topFrame = stackFrames[stackFrames.length - 1];
    topFrame.rows = [['node', fmt(ADDR(nodeId)), true], ['node.data', '' + node.val], ['leftCount', '' + leftCount]];

    snap(nodeId,
      'Left subtree node count of node ' + node.val + ' is ' + leftCount + '. Recurse RIGHT: call countNodes(node.right = ' + childAddrStr(node.right) + ')',
      'c_recurRight', nodeCounts);
    const rightCount = countNodes(node.right);

    topFrame.rows = [['node', fmt(ADDR(nodeId)), true], ['node.data', '' + node.val], ['leftCount', '' + leftCount], ['rightCount', '' + rightCount]];

    const cnt = leftCount + rightCount + 1;
    topFrame.rows = [['node', fmt(ADDR(nodeId)), true], ['node.data', '' + node.val], ['leftCount', '' + leftCount], ['rightCount', '' + rightCount], ['count', '' + cnt]];
    nodeCounts[nodeId] = cnt;

    snap(nodeId, 'count = leftCount(' + leftCount + ') + rightCount(' + rightCount + ') + 1 = ' + cnt, 'c_compute', nodeCounts);
    snap(nodeId, 'return ' + cnt + ' from countNodes(node ' + node.val + ' @' + node.addr + ')', 'c_return', nodeCounts);
  ...
  totalCount = result;
  snap(null, 'countNodes complete! Total number of nodes in the tree = ' + result, 'c_main', nodeCounts);
  return steps;
}
...
function nodeBoxClass(n) {
  ...
  if ((step.nodeCounts || {})[n.id] !== undefined) return 'bt-box-resolved';
  return '';
}
          ...
          <div class="ll-main" ref="mainRef">
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
                <div class="ll-perm-area" ref="vizSvgRef">
                  <div class="ll-ptrs">
                    ...
                    <div v-if="s.totalCount !== null" class="ll-ptr-chip count-result-chip">Total Nodes = <b>{{ s.totalCount }}</b></div>
                    <div v-else class="ll-ptr-chip count-calculating-chip">Counting Nodes…</div>
                  </div>
                      ...
                      <div xmlns="http://www.w3.org/1999/xhtml" class="ll-node-wrap">
                        <div class="ll-box" :class="nodeBoxClass(n)">
                          <div class="ll-node-top">
                            ...
                            <div class="ll-data">
                              {{ n.val }}
                              <small v-if="s.nodeCounts[n.id] !== undefined" class="node-c-tag">n={{ s.nodeCounts[n.id] }}</small>
                            </div>
              ...
              <div class="ll-legend">
                ...
                <span class="ll-leg"><span class="ll-legdot ll-legdot-current"></span>active node (counting nodes)</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-resolved"></span>nodes computed (subtrees done)</span>
              </div>
            ...
            <div class="ll-right-col">
              <div class="ll-code-panel">
                ...
                <div v-else class="ll-info-scroll">
                  <h3>Time &amp; Space Complexity &mdash; Count Total Nodes</h3>
                  <table class="ll-complexity-table">
                    <thead><tr><th>Metric</th><th>Complexity</th><th>Why</th></tr></thead>
                    <tbody>
                      <tr><td>Time Complexity</td><td>O(N)</td><td>Every node in the tree is visited once to count.</td></tr>
                  ...
                  <p class="ll-note">
                    ...
                    <code>count(node) = count(node.left) + count(node.right) + 1</code><br/>
                    The total node count of a tree node is 1 plus the node count of its left and right subtrees. An empty subtree (null node) has 0 nodes.
                  </p>
                  <h3>Algorithm Steps</h3>
                  <p>1. Base case: if <code>node == null</code>, return <code>0</code>.<br/>
                     2. Recurse left: <code>leftCount = countNodes(node.left)</code>.<br/>
                     3. Recurse right: <code>rightCount = countNodes(node.right)</code>.<br/>
                     4. Compute and return: <code>leftCount + rightCount + 1</code>.</p>
                </div>
              </div>
            </div>
          </div>
