...
const CODES = {
  java: [
    ...
    ['c_insert', '        for (int i = 0; i < size; i++)'],
    ['c_insert', '            list.insertEnd(sc.nextInt());'],
  ...
  c: [
    ...
    ['c_main', '    int sz, v;'],
    ['c_main', '    scanf("%d", &sz);'],
    ['c_insert', '    for (int i = 0; i < sz; i++) {'],
    ['c_insert', '        scanf("%d", &v);'],
    ['c_insert', '        insertEnd(v);'],
    ['c_insert', '    }'],
    ['c_display', '    display();'],
    ['c_main', '    return 0;'],
    ['}', ''],
  ...
  cpp: [
    ...
    ['c_nodeNew', '    newNode->next = nullptr;'],
    ['c_sethead', '    if (!head) {'],
    ['c_sethead', '        head = newNode;'],
    ['c_sethead', '        return;'],
    ['c_sethead', '    }'],
    ['c_walk', '    Node *temp = head;'],
    ['c_walk', '    while (temp->next)'],
    ['c_walk', '        temp = temp->next;'],
    ['c_attach', '    temp->next = newNode;'],
    ['c_attach', '    newNode->prev = temp;'],
    ['}', ''],
  ...
  python: [
    ...
    ['', ''],
    ['c_insert', '    def insert_end(self, data):'],
    ['c_nodeNew', '        n = Node(data)'],
    ['c_sethead', '        if not self.head:'],
    ['c_sethead', '            self.head = n'],
    ['c_sethead', '            return'],
    ['c_walk', '        temp = self.head'],
    ['c_walk', '        while temp.next:'],
    ['c_walk', '            temp = temp.next'],
    ['c_attach', '        temp.next = n'],
    ['c_attach', '        n.prev = temp'],
    ['', ''],
    ...
    ['c_main', 'lst = DLL()'],
    ['c_main', 'sz = int(input())'],
    ['c_insert', 'for _ in range(sz):'],
    ['c_insert', '    lst.insert_end(int(input()))'],
...
function buildSteps(values) {
  ...
  values.forEach((v, li) => {
    ...
    const tailAddr = nodes.length ? nodes[nodes.length - 1].addr : null;
    const ins = () => frame('insertEnd(data=' + v + ')', [['data', '' + v]]);
    const mf = () => frame('main()', [['size', sz], ['i', '' + li], ['head', fmt(headAddr()), true]]);
    steps.push({
      nodes: nodes.slice(), head: headAddr(), pending: { val: v, addr }, pendPrev: null, pendNext: null,
      badge: 'create newNode(' + v + ') @' + addr, code: 'c_nodeNew',
      vars: [mf(), ins()],
    });
    if (!nodes.length) {
      nodes = [{ val: v, addr }];
      steps.push({
        ...
        badge: 'head is null → head = newNode @' + addr, code: 'c_sethead',
        vars: [mf(), ins()],
      });
    } else {
      for (let t = 0; t < nodes.length - 1; t++) {
        steps.push({
          ...
          badge: 'walk: temp = @' + nodes[t].addr + ' → temp.next = @' + nodes[t + 1].addr, code: 'c_walk',
          vars: [mf(), frame('insertEnd(data=' + v + ')', [['data', '' + v], ['temp', fmt(nodes[t].addr), true]])],
        });
      }
      const lastIdx = nodes.length - 1;
      steps.push({
        nodes: nodes.slice(), head: headAddr(), tempAddr: nodes[lastIdx].addr, curAddr: nodes[lastIdx].addr,
        badge: 'temp.next = null → reached tail @' + nodes[lastIdx].addr, code: 'c_walk',
        vars: [mf(), frame('insertEnd(data=' + v + ')', [['data', '' + v], ['temp', fmt(nodes[lastIdx].addr), true]])],
      });
      nodes = [...nodes, { val: v, addr }];
      steps.push({
        nodes: nodes.slice(), head: headAddr(), tempAddr: nodes[lastIdx].addr, neuAddr: addr,
        badge: 'attach: temp.next=newNode, newNode.prev=temp', code: 'c_attach',
        vars: [mf(), frame('insertEnd(data=' + v + ')', [['data', '' + v], ['temp', fmt(nodes[lastIdx].addr), true]])],
      });
    }
  });
  ...
  steps.push({
    nodes: nodes.slice(), head: headAddr(), badge: 'display(): temp=null → done', code: 'c_display', out: printed.slice(),
    vars: [frame('main()', [['head', fmt(headAddr()), true]]), frame('display()', [['temp', 'null', true]])],
  });
            ...
            <div class="ll-stage">
              <div v-for="(n, idx) in s.nodes" :key="n.addr" class="ll-nwrap">
                ...
                <div class="ll-node">
                  ...
                  <span
                    class="ll-arrow"
                    :class="{ 'll-arrow-cut': nextAddrOf(idx) && nextAddrOf(idx) === s.delAddr }"
                  >{{ nextAddrOf(idx) ? '⇄' : '' }}</span>
                </div>
              </div>

              <div v-if="s.pending" class="ll-nwrap" style="margin-left: 22px">
