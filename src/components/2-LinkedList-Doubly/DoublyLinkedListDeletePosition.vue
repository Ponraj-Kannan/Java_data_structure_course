...
const CODES = {
  java: [
    ...
    ['', '}'],
    ['', 'class DLL {'],
    ['c_build', '    Node head = null;'],
    ['', ''],
    ['c_build', '    void insertEnd(int d) { /* ... */ }'],
    ['', ''],
    ['c_delete', '    void deleteAt(int pos) {'],
    ['c_check', '        if (head == null) return;'],
    ['c_movehead', '        if (pos == 1) {'],
    ['c_movehead', '            Node t = head;'],
    ['c_movehead', '            head = head.next;'],
    ['c_movehead', '            if (head != null) head.prev = null;'],
    ['c_movehead', '            return;'],
    ['c_movehead', '        }'],
    ['c_walk', '        Node temp = head;'],
    ['c_walk', '        int i = 1;'],
    ['c_walk', '        while (temp != null && i < pos) {'],
    ['c_walk', '            temp = temp.next;'],
    ['c_walk', '            i++;'],
    ['c_walk', '        }'],
    ['c_relink', '        if (temp == null) return;'],
    ['c_relink', '        if (temp.prev != null)'],
    ['c_relink', '            temp.prev.next = temp.next;'],
    ['c_relink', '        if (temp.next != null)'],
    ['c_relink', '            temp.next.prev = temp.prev;'],
    ['', '    }'],
    ['', ''],
    ...
    ['c_build', '            list.insertEnd(sc.nextInt());'],
    ['c_main', '        int pos = sc.nextInt();'],
    ['c_delete', '        list.deleteAt(pos);'],
  ...
  c: [
    ...
    ['', '};'],
    ['', ''],
    ['c_build', 'struct Node *head = NULL;'],
    ['', ''],
    ['c_build', 'void insertEnd(int d) { /* ... */ }'],
    ['', ''],
    ['c_delete', 'void deleteAt(int pos) {'],
    ['c_check', '    if (!head) return;'],
    ['c_movehead', '    if (pos == 1) {'],
    ['c_movehead', '        struct Node *t = head;'],
    ['c_movehead', '        head = head->next;'],
    ['c_movehead', '        if (head) head->prev = NULL;'],
    ['c_movehead', '        free(t);'],
    ['c_movehead', '        return;'],
    ['c_movehead', '    }'],
    ['c_walk', '    struct Node *temp = head;'],
    ['c_walk', '    int i = 1;'],
    ['c_walk', '    while (temp && i < pos) {'],
    ['c_walk', '        temp = temp->next;'],
    ['c_walk', '        i++;'],
    ['c_walk', '    }'],
    ['c_relink', '    if (!temp) return;'],
    ['c_relink', '    if (temp->prev)'],
    ['c_relink', '        temp->prev->next = temp->next;'],
    ['c_relink', '    if (temp->next)'],
    ['c_relink', '        temp->next->prev = temp->prev;'],
    ['c_relink', '    free(temp);'],
    ['}', ''],
    ['', ''],
  ...
  cpp: [
    ...
    ['c_movehead', '        Node *t = head;'],
    ['c_movehead', '        head = head->next;'],
    ['c_movehead', '        if (head) head->prev = nullptr;'],
    ['c_movehead', '        delete t;'],
    ['c_movehead', '        return;'],
    ['c_movehead', '    }'],
    ['c_walk', '    Node *temp = head;'],
  ...
  python: [
    ...
    ['', ''],
    ['', 'class DLL:'],
    ['c_build', '    def __init__(self):'],
    ['c_build', '        self.head = None'],
    ['', ''],
    ['c_build', '    def insert_end(self, d): ...'],
    ['', ''],
    ['c_delete', '    def delete_at(self, pos):'],
    ['c_check', '        if not self.head:'],
    ['c_check', '            return'],
    ['c_movehead', '        if pos == 1:'],
    ['c_movehead', '            t = self.head'],
    ['c_movehead', '            self.head = self.head.next'],
    ['c_movehead', '            if self.head:'],
    ['c_movehead', '                self.head.prev = None'],
    ['c_movehead', '            return'],
    ['c_walk', '        temp = self.head'],
    ['c_walk', '        i = 1'],
    ['c_walk', '        while temp and i < pos:'],
    ['c_walk', '            temp = temp.next'],
    ['c_walk', '            i += 1'],
    ['c_relink', '        if not temp: return'],
    ['c_relink', '        if temp.prev:'],
    ['c_relink', '            temp.prev.next = temp.next'],
    ['c_relink', '        if temp.next:'],
    ['c_relink', '            temp.next.prev = temp.prev'],
...
function buildSteps(values, pos) {
  ...
  if (!nodes.length) return { steps, phaseStarts };
  phaseStarts.delete = steps.length;
  const safePos = Math.min(pos, nodes.length);
  const oldHead = headAddr();
  steps.push({
    nodes: nodes.slice(), head: oldHead,
    badge: 'deleteAt(pos=' + pos + '): check head != null', code: 'c_check',
    vars: [frame('deleteAt(' + pos + ')', [['pos', '' + pos], ['head', fmt(oldHead), true]])],
  });
  if (pos === 1) {
    const delAddr = oldHead;
    const newHead = nodes.length > 1 ? nodes[1].addr : null;
    steps.push({
      nodes: nodes.slice(), head: oldHead, delAddr,
      badge: 'pos=1 → deleting head @' + delAddr, code: 'c_movehead',
      vars: [frame('deleteAt(' + pos + ')', [['temp', fmt(delAddr), true]])],
    });
    nodes.shift();
    steps.push({
      nodes: nodes.slice(), head: newHead,
      badge: 'head=head.next (' + fmt(newHead) + '), head.prev=null', code: 'c_movehead',
      vars: [frame('deleteAt(' + pos + ')', [['head', fmt(newHead), true]])],
    });
  } else {
    const delIdx = safePos - 1;
    for (let t = 0; t < delIdx; t++) {
      steps.push({
        nodes: nodes.slice(), head: oldHead, tempAddr: nodes[t].addr, curAddr: nodes[t].addr,
        badge: 'walk: temp @' + nodes[t].addr + ', i=' + (t + 1) + ' < pos=' + pos, code: 'c_walk',
        vars: [frame('deleteAt(' + pos + ')', [['temp', fmt(nodes[t].addr), true], ['i', '' + (t + 1)]])],
      });
    }
    const delAddr = nodes[delIdx].addr;
    const prevAddr = delIdx > 0 ? nodes[delIdx - 1].addr : null;
    const nextAddr = delIdx + 1 < nodes.length ? nodes[delIdx + 1].addr : null;
    steps.push({
      nodes: nodes.slice(), head: oldHead, tempAddr: delAddr, delAddr,
      badge: 'temp = @' + delAddr + ' (position ' + pos + ')', code: 'c_walk',
      vars: [frame('deleteAt(' + pos + ')', [['temp', fmt(delAddr), true]])],
    });
    nodes.splice(delIdx, 1);
    steps.push({
      nodes: nodes.slice(), head: headAddr(),
      badge: 'relink: prev.next=next (' + fmt(nextAddr) + '), next.prev=prev (' + fmt(prevAddr) + ')', code: 'c_relink',
      vars: [frame('deleteAt(' + pos + ')', [['temp', fmt(delAddr)], ['prev', fmt(prevAddr), true], ['next', fmt(nextAddr)]])],
    });
  }
