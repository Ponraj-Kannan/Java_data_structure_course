...
const CODES = {
  java: [
    ...
    ['c_nodeNew', '    Node prev;'],
    ['c_nodeNew', '    Node next;'],
    ['c_nodeNew', '    Node(int data) {'],
    ['c_nodeNew', '        this.data = data;'],
    ['c_nodeNew', '        this.prev = null;'],
    ...
    ['c_setprev', '        if (head != null) {'],
    ['c_setprev', '            head.prev = newNode;'],
    ['c_setprev', '        }'],
    ...
    ['c_main', '        DoublyLinkedList list = new DoublyLinkedList();'],
    ['c_main', '        int size = sc.nextInt();'],
    ['c_main', '        for (int i = 0; i < size; i++)'],
    ['c_insert', '            list.insertAtBeginning(sc.nextInt());'],
  ...
  c: [
    ...
    ['c_insert', '        insertAtBeginning(value);'],
    ['c_main', '    }'],
    ['c_display', '    display();'],
    ['c_main', '    return 0;'],
    ['}', ''],
  ...
  cpp: [
    ...
    ['c_nodeNew', '    newNode->data = data;'],
    ['c_nodeNew', '    newNode->prev = nullptr;'],
    ['c_setnext', '    newNode->next = head;'],
    ['c_setprev', '    if (head != nullptr) {'],
    ['c_setprev', '        head->prev = newNode;'],
    ['c_setprev', '    }'],
    ['c_sethead', '    head = newNode;'],
    ['}', ''],
    ...
    ['c_display', 'void display() {'],
    ['c_display', '    Node *temp = head;'],
    ['c_display', '    while (temp != nullptr) {'],
    ['c_display', '        cout << temp->data << " ";'],
    ['c_display', '        temp = temp->next;'],
    ['c_display', '    }'],
    ['}', ''],
    ['', ''],
  ...
  python: [
    ['', 'class Node:'],
    ['c_nodeNew', '    def __init__(self, data):'],
    ['c_nodeNew', '        self.data = data'],
    ...
    ['c_setprev', '        if self.head is not None:'],
    ['c_setprev', '            self.head.prev = new_node'],
    ...
    ['c_main', 'lst = DoublyLinkedList()'],
    ['c_main', 'for _ in range(size):'],
    ['c_insert', '    lst.insert_at_beginning(int(input()))'],
...
function buildSteps(values) {
  ...
  steps.push({
    nodes: [], head: null, badge: 'Read input → size = ' + sz, code: 'c_main',
    vars: [frame('main()', [['size', sz], ['head', 'null', true]])],
  });
  values.forEach((v, li) => {
    const addr = ADDR(li);
    const oldHead = headAddr();
    const df = (rows) => frame('insertAtBeginning(data=' + v + ')', rows);
    const mainF = (h) => frame('main()', [['size', sz], ['i', '' + li], ['head', fmt(h), true]]);
    steps.push({
      nodes: nodes.slice(), head: oldHead, pending: { val: v, addr: addr }, pendPrev: null, pendNext: null,
      badge: 'read value ' + v + ' → new node @' + addr + ' (prev=null, next=null)', code: 'c_nodeNew',
      vars: [mainF(oldHead), df([['data', '' + v], ['newNode', fmt(addr), true]])],
    });
    steps.push({
      nodes: nodes.slice(), head: oldHead, pending: { val: v, addr: addr }, pendPrev: null, pendNext: oldHead,
      badge: 'newNode.next = head (' + fmt(oldHead) + ')', code: 'c_setnext',
      vars: [mainF(oldHead), df([['data', '' + v], ['newNode', fmt(addr), true]])],
    });
    if (oldHead !== null) {
      steps.push({
        nodes: nodes.slice(), head: oldHead, pending: { val: v, addr: addr }, pendPrev: null, pendNext: oldHead, headPrevOverride: addr, curAddr: oldHead,
        badge: 'head.prev = newNode (@' + addr + ')', code: 'c_setprev',
        vars: [mainF(oldHead), df([['data', '' + v], ['newNode', fmt(addr), true]])],
      });
    }
    nodes = [{ val: v, addr: addr }].concat(nodes);
    steps.push({
      nodes: nodes.slice(), head: addr, neuAddr: addr,
      badge: 'head = newNode → @' + addr, code: 'c_sethead',
      vars: [mainF(addr), df([['data', '' + v], ['newNode', fmt(addr), true]])],
    });
  });
  phaseStarts.display = steps.length;
  let printed = [];
  steps.push({
    nodes: nodes.slice(), head: headAddr(), tempAddr: nodes[0] && nodes[0].addr, badge: 'display(): start at head', code: 'c_display', out: [],
    vars: [frame('main()', [['size', sz], ['head', fmt(headAddr()), true]]), frame('display()', [['temp', fmt(nodes[0] && nodes[0].addr), true]])],
  });
  for (let t = 0; t < nodes.length; t++) {
    printed = printed.concat(nodes[t].val);
    steps.push({
      ...
      vars: [frame('main()', [['size', sz], ['head', fmt(headAddr()), true]]), frame('display()', [['temp', fmt(nodes[t].addr), true]])],
    });
  }
  steps.push({
    nodes: nodes.slice(), head: headAddr(), tempAddr: null, badge: 'display(): temp = null → done', code: 'c_display', out: printed.slice(), done: true,
    vars: [frame('main()', [['size', sz], ['head', fmt(headAddr()), true]]), frame('display()', [['temp', 'null', true]])],
  });
...
function prevAddrOf(idx) {
  let prevAddr = idx > 0 ? s.value.nodes[idx - 1].addr : null;
  if (idx === 0 && s.value.headPrevOverride !== undefined) prevAddr = s.value.headPrevOverride;
  return prevAddr;
}

function nextAddrOf(idx) {
  return idx < s.value.nodes.length - 1 ? s.value.nodes[idx + 1].addr : null;
}

function nextIsDel(idx) {
  const nextAddr = nextAddrOf(idx);
  return !!(nextAddr && nextAddr === s.value.delAddr);
}
            ...
            <div class="ll-stage">
              <div v-for="(n, idx) in s.nodes" :key="n.addr" class="ll-nwrap">
                ...
                <div class="ll-node">
                  ...
                  >
                    <div class="ll-ptr ll-ptr-prev"><small>prev</small>{{ fmt(prevAddrOf(idx)) }}</div>
                    <div class="ll-data">{{ n.val }}</div>
                    <div class="ll-ptr"><small>next</small>{{ fmt(nextAddrOf(idx)) }}</div>
                  </div>
                  <span class="ll-arrow" :class="{ 'll-arrow-cut': nextIsDel(idx) }">{{ nextAddrOf(idx) ? '⇄' : '' }}</span>
                </div>
              </div>
