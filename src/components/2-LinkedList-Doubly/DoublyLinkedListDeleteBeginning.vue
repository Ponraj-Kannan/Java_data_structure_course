...
const CODES = {
  java: [
    ...
    ['', ''],
    ['c_delete', '    void deleteBeginning() {'],
    ['c_check', '        if (head == null) return;'],
    ['c_temp', '        Node temp = head;'],
    ['c_movehead', '        head = head.next;'],
    ['c_free', '        if (head != null)'],
    ['c_free', '            head.prev = null;'],
    ['c_free', '        // temp is now unreferenced'],
    ['', '    }'],
    ['', ''],
  ...
  cpp: [
    ...
    ['c_temp', '    Node *temp = head;'],
    ['c_movehead', '    head = head->next;'],
    ['c_free', '    if (head != nullptr)'],
    ['c_free', '        head->prev = nullptr;'],
    ['c_free', '    delete temp;'],
    ['}', ''],
    ['', ''],
    ['c_display', 'void display() {'],
  ...
  python: [
    ...
    ['', ''],
    ['c_delete', '    def delete_beginning(self):'],
    ['c_check', '        if not self.head:'],
    ['c_check', '            return'],
    ['c_temp', '        temp = self.head'],
    ['c_movehead', '        self.head = self.head.next'],
    ['c_free', '        if self.head:'],
    ['c_free', '            self.head.prev = None'],
    ['', ''],
...
function buildSteps(values) {
  ...
  const oldHead = nodes[0].addr;
  const newHead = nodes.length > 1 ? nodes[1].addr : null;
  steps.push({
    nodes: nodes.slice(), head: oldHead,
    badge: 'deleteBeginning(): check head != null', code: 'c_check',
    vars: [frame('main()', [['head', fmt(oldHead), true]]), frame('deleteBeginning()', [['head', fmt(oldHead), true]])],
  });
  steps.push({
    nodes: nodes.slice(), head: oldHead, tempAddr: oldHead, delAddr: oldHead,
    badge: 'temp = head (@' + oldHead + ')', code: 'c_temp',
    vars: [frame('deleteBeginning()', [['temp', fmt(oldHead), true]])],
  });
  steps.push({
    nodes: nodes.slice(), head: newHead, tempAddr: oldHead, delAddr: oldHead,
    badge: 'head = head.next (head = ' + fmt(newHead) + ')', code: 'c_movehead',
    vars: [frame('deleteBeginning()', [['temp', fmt(oldHead)], ['head', fmt(newHead), true]])],
  });
  nodes.shift();
  if (nodes.length) nodes[0] = { ...nodes[0] };
  steps.push({
    nodes: nodes.slice(), head: newHead,
    badge: 'head.prev = null, free temp (@' + oldHead + ')', code: 'c_free',
    vars: [frame('deleteBeginning()', [['temp', fmt(oldHead)], ['head', fmt(newHead), true]])],
  });
