...
const CODES = {
  java: [
    ...
    ['c_ctor', '    }'], ['', ''],
    ['c_size', '    int size() {'],
    ['c_empty', '        if (front == -1) return 0;'],
    ['c_formula', '        return ((rear - front + capacity) % capacity) + 1;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['', '#define MAX 100'], ['', ''],
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_size', 'int size(struct CircularQueue *q) {'],
    ['c_empty', '    if (q->front == -1) return 0;'],
    ['c_formula', '    return ((q->rear - q->front + q->capacity) % q->capacity) + 1;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class CircularQueue {'],
  ...
  python: [
    ...
    ['c_ctor', '        self.front = -1'],
    ['c_ctor', '        self.rear = -1'], ['', ''],
    ['c_size', '    def size(self):'],
    ['c_empty', '        if self.front == -1:'],
    ['c_empty', '            return 0'],
    ['c_formula', '        return ((self.rear - self.front + self.capacity) % self.capacity) + 1'],
  ],
  javascript: [
    ['', 'class CircularQueue {'],
    ...
    ['c_ctor', '  }'], ['', ''],
    ['c_size', '  size() {'],
    ['c_empty', '    if (this.front === -1) return 0;'],
    ['c_formula', '    return ((this.rear - this.front + this.capacity) % this.capacity) + 1;'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'normal'  - an occupied, settled slot, not (yet) counted
//   'counted' - an occupied slot that has been included in the running
//               tally while size() walks from front around to rear
function buildSteps(capacity, frontIdx, values) {
  const steps = [];
  const phaseStarts = { size: 0 };
  ...
  function pushStep(opts) {
    steps.push({
      cells: cells(),
      front, rear, capacity,
      size: opts.runningCount != null ? opts.runningCount : (front === -1 ? 0 : ((rear - front + capacity) % capacity) + 1),
      badge: opts.badge,
      code: opts.code,
  ...
  pushStep({
    badge: 'Initial queue \u2014 capacity ' + capacity + ' \u2014 ' + fnLabel + ' called',
    code: 'c_size',
  ...
  if (front === -1) {
    pushStep({
      badge: 'front == -1 \u2192 size() returns 0 \u2014 queue is empty',
      code: 'c_empty',
      success: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['returns', '0', true]])],
    });
    return { steps, phaseStarts };
  }

  pushStep({
    badge: 'front != -1 \u2192 compute size = ((rear - front + capacity) % capacity) + 1',
    code: 'c_formula',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [])],
  });

  const totalSize = ((rear - front + capacity) % capacity) + 1;
  for (let i = 0; i < totalSize; i++) {
    const idx = (front + i) % capacity;
    stateAt[idx] = 'counted';
    pushStep({
      badge: 'counting index ' + idx + (idx === front ? ' (front)' : idx === rear ? ' (rear)' : '') + ' \u2192 running count = ' + (i + 1),
      code: 'c_formula',
      runningCount: i + 1,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['count', String(i + 1), true]])],
    });
  }
  ...
  pushStep({
    badge: fnLabel + ' returns ' + totalSize + ' \u2014 the queue currently holds ' + totalSize + ' element(s)',
    code: 'c_formula',
    success: true,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['returns', String(totalSize), true]])],
  });
...
const inpElems = ref('40 50 60 70');
const lang = ref('java');
const speed = ref(650);
const si = ref(0);
...
const stepsData = reactive(buildSteps(5, 3, [40, 50, 60, 70]));
  ...
  <div class="slide-wrapper">
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
            ...
            <div class="ll-arr-track">
              <template v-for="cell in s.cells" :key="cell.idx">
                <div class="ll-arr-cell-wrap">
                  <div class="ll-who" :class="{ 'll-c-purple': cell.state === 'counted' }">{{ cellLabel(cell) }}</div>
                  <div
                    class="ll-arr-box"
                    :class="{
                      'll-box-empty': cell.state === 'empty',
                      'll-box-counted': cell.state === 'counted'
                    }"
                  >
        ...
        <div class="ll-legend">
          <span class="ll-leg"><span class="ll-legdot ll-legdot-existing"></span>occupied cell (not yet counted)</span>
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-counted"></span>counted toward size (walking front &#8594; rear, with wrap)</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty queue</td><td>O(1)</td><td>single comparison <code>front == -1</code>, returns 0 immediately</td></tr>
                <tr><td>Non-empty queue</td><td>O(1)</td><td>one subtraction, one modulo, and one addition &mdash; <code>((rear - front + capacity) % capacity) + 1</code> &mdash; regardless of how many elements are present or whether they wrap</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no traversal or extra storage is needed; the counting shown in the visualization is for teaching purposes only &mdash; the real formula never actually loops</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              How the formula handles wrap-around: <code>rear - front</code> alone would be negative whenever
              <code>rear</code> has wrapped past <code>front</code> (for example <code>front = 3</code>,
              <code>rear = 1</code>). Adding <code>capacity</code> before taking <code>% capacity</code> corrects
              exactly for that one wrap, turning a negative "distance" into the true number of steps from
              <code>front</code> to <code>rear</code> going forward through the array. The final <code>+ 1</code>
              accounts for <code>front</code> itself being an element, not just a boundary.
            </p>
            <h3>Advantages</h3>
            <p>Reports the exact element count in O(1) time with no traversal, and remains correct whether or not the stored elements are split across the wrap point &mdash; a plain <code>rear - front + 1</code> would be wrong the moment wrap-around occurs.</p>
            <h3>Limitations</h3>
            <p>Depends on <code>front</code> and <code>rear</code> being kept perfectly consistent by every <code>enqueue()</code> and <code>dequeue()</code>; if either pointer is updated incorrectly elsewhere, this formula will silently report the wrong size rather than detect the inconsistency.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p>In a <b>Simple (Linear) Queue</b>, size is simply <code>rear - front + 1</code>, since <code>rear</code> never wraps and is always <code>&ge; front</code>. In a <b>Circular Queue</b>, <code>rear</code> can be numerically smaller than <code>front</code> after wrapping, so the plain subtraction would go negative &mdash; the <code>+ capacity</code> before the modulo is what makes the Circular Queue's formula correct in every case, wrapped or not.</p>
            <h3>Key Observations</h3>
            <p>When there is no wrap (<code>rear &ge; front</code>), <code>(rear - front + capacity) % capacity</code> simplifies to just <code>rear - front</code>, since adding a full <code>capacity</code> and taking the modulo cancels back out &mdash; so the same formula quietly handles both the wrapped and non-wrapped cases without needing a separate branch.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> <code>front == -1</code> short-circuits straight to 0 without touching the formula. <b>Single-element queue:</b> <code>front == rear</code>, so the formula reduces to <code>0 + 1 = 1</code>. <b>No wrap:</b> elements sit contiguously between <code>front</code> and <code>rear</code>; the formula behaves like a simple range count. <b>Wrapped:</b> elements span from <code>front</code> to the end of the array and continue from index 0 up to <code>rear</code>; the <code>+ capacity</code> correction still yields the exact right count. <b>Full queue:</b> the formula correctly returns <code>capacity</code>, matching the total number of array cells.</p>
          </div>
        </div>
      </div>
    </div>
...
.ll-root {
  ...
  --node: #1d4ed8; --nodeNew: #15803d; --nodeCur: #c2410c; --nodeDel: #991b1b; --nodeCounted: #7e22ce;
...
.ll-box-counted { border-color: var(--purple) !important; background: var(--nodeCounted) !important; box-shadow: 0 0 0 3px rgba(147,51,234,.25) !important; }
...
.ll-legdot-counted { background: var(--nodeCounted); border: 1.5px solid var(--purple); }
