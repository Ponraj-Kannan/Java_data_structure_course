...
const CODES = {
  java: [
    ...
    ['c_capzero', '        if (capacity == 0) return true;'],
    ['c_check', '        return (rear + 1) % capacity == front;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['', '#define MAX 100'], ['', ''],
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_isfull', 'int isFull(struct CircularQueue *q) {'],
    ['c_capzero', '    if (q->capacity == 0) return 1;'],
    ['c_check', '    return (q->rear + 1) % q->capacity == q->front;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class CircularQueue {'],
  ...
  python: [
    ...
    ['c_capzero', '        if self.capacity == 0:'],
    ['c_capzero', '            return True'],
    ['c_check', '        return (self.rear + 1) % self.capacity == self.front'],
  ],
  javascript: [
    ['', 'class CircularQueue {'],
    ...
    ['c_ctor', '  }'], ['', ''],
    ['c_isfull', '  isFull() {'],
    ['c_capzero', '    if (this.capacity === 0) return true;'],
    ['c_check', '    return (this.rear + 1) % this.capacity === this.front;'],
    ['', '  }'], ['', '}'],
  ],
};
...
// This operation never mutates the array or the pointers - every cell stays
// 'normal' (occupied) or 'empty'. The one extra thing worth showing is
// nextPos = (rear + 1) % capacity, the slot rear would move to on the very
// next enqueue - highlighted with 'next ->' so it's visually obvious
// whether that slot is free or already occupied by front.
function buildSteps(capacity, frontIdx, values) {
  ...
  function pushStep(opts) {
    steps.push({
      ...
      nextPos: opts.nextPos != null ? opts.nextPos : null,
      vars: opts.vars,
    });
  }
  ...
  if (capacity === 0) {
    pushStep({
      badge: 'capacity == 0 \u2192 isFull() returns true \u2014 Queue is Full (no array cells exist)',
      code: 'c_capzero',
      success: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['returns', 'true', true]])],
    });
    return { steps, phaseStarts };
  }

  const nextPos = ((rear + 1) % capacity + capacity) % capacity;

  pushStep({
    badge: 'checking (rear + 1) % capacity == front \u2026 next slot after rear is index ' + nextPos,
    code: 'c_check',
    showNext: true,
    nextPos,
  ...
  const isFull = front !== -1 && nextPos === front;

  if (isFull) {
    pushStep({
      badge: '(rear + 1) % capacity == front \u2192 isFull() returns true \u2014 Queue is Full (index ' + nextPos + ' is already occupied by front)',
      code: 'c_check',
      success: true,
      showNext: true,
      nextPos,
    ...
    pushStep({
      badge: '(rear + 1) % capacity != front \u2192 isFull() returns false \u2014 Queue is Not Full (index ' + nextPos + ' is still free)',
      code: 'c_check',
      success: false,
      showNext: true,
      nextPos,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['returns', 'false', true]])],
    });
  }
...
const inpCapacity = ref(4);
const inpFront = ref(1);
...
const stepsData = reactive(buildSteps(4, 1, [10, 20, 30, 40]));
...
function cellLabel(cell) {
  const labels = [];
  if (s.value.front !== -1 && cell.idx === s.value.front) labels.push('front');
  if (s.value.rear !== -1 && cell.idx === s.value.rear) labels.push('rear');
  if (s.value.showNext && cell.idx === s.value.nextPos) labels.push('next');
...
<template>
  <div class="slide-wrapper">
    ...
    <div class="ll-toolbar">
      ...
      <input type="text" v-model="inpElems" style="width: 185px;" placeholder="Enter numbers, or leave blank for an empty queue" class="ll-text-input" />
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
            ...
            <div class="ll-arr-track">
              <template v-for="cell in s.cells" :key="cell.idx">
                <div class="ll-arr-cell-wrap">
                  <div class="ll-who" :class="{ 'll-c-orange': s.showNext && cell.idx === s.nextPos }">{{ cellLabel(cell) }}</div>
                  <div
                    class="ll-arr-box"
                    :class="{
                      'll-box-empty': cell.state === 'empty',
                      'll-box-next': s.showNext && cell.idx === s.nextPos
                    }"
                  >
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-next"></span>next slot after rear &mdash; full only if this equals front</span>
        </div>
        ...
        <div class="ll-badge-wrap">
          <div class="ll-badge" :class="{ 'll-badge-success': s.success === true }">{{ s.badge }}</div>
        </div>
      </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Capacity 0</td><td>O(1)</td><td>a single check <code>capacity == 0</code> reports full immediately, avoiding division by zero</td></tr>
                <tr><td>Not full</td><td>O(1)</td><td>one modulo and one comparison, <code>(rear + 1) % capacity == front</code>, regardless of how many elements are present</td></tr>
                <tr><td>Full</td><td>O(1)</td><td>same single comparison &mdash; no traversal is ever needed to decide fullness</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only reads the existing <code>capacity</code>, <code>front</code>, and <code>rear</code> fields</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Why the queue is full when <code>(rear + 1) % capacity == front</code>: that expression computes
              exactly the index <code>rear</code> would move to on the <em>next</em> <code>enqueue()</code>.
              If that index is already occupied by <code>front</code> &mdash; the oldest remaining element &mdash;
              then writing there would overwrite live data, so the queue must refuse the insertion instead.
              This is also why a Circular Queue implemented this way can only ever hold
              <code>capacity</code> elements, never more: one comparison keeps <code>rear</code> from ever lapping <code>front</code>.
            </p>
            <h3>Advantages</h3>
            <p>A single O(1) check that <code>enqueue()</code> relies on to safely avoid a Queue Overflow, without needing to maintain a separate element counter.</p>
            <h3>Limitations</h3>
            <p>Distinguishing full from empty this way requires the sentinel <code>front == -1</code> to remain reserved for "empty" &mdash; if <code>front</code> and <code>rear</code> both started at <code>0</code> instead, a queue with 0 elements and a queue with <code>capacity</code> elements could look identical (<code>front == rear</code>). Some implementations avoid this altogether by tracking a separate <code>size</code> counter instead of relying on pointer relationships.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p>A <b>Simple (Linear) Queue</b> considers itself full the moment <code>rear</code> reaches <code>capacity - 1</code>, even if slots before <code>front</code> have since been freed by dequeues &mdash; a false "full" that wastes space. A <b>Circular Queue</b> only reports full when every slot, including ones freed and reused via wrap-around, is genuinely occupied &mdash; captured precisely by <code>(rear + 1) % capacity == front</code>.</p>
            <h3>Key Observations</h3>
            <p>The condition compares the <em>would-be next</em> position of <code>rear</code> against <code>front</code>, not <code>rear</code> against <code>front</code> directly &mdash; because <code>front == rear</code> on its own is ambiguous (it also describes a queue with exactly one element). Requiring <code>front != -1</code> is implicit in the formula, since an empty queue has <code>front == -1</code>, which can never equal a valid array index produced by the modulo.</p>
            <h3>Edge Cases</h3>
            <p><b>Capacity 0:</b> handled as a special case up front, since <code>% 0</code> is undefined; such a queue is trivially always full. <b>Just below capacity:</b> <code>isFull()</code> correctly returns false right up until the very last free slot is filled. <b>Exactly at capacity:</b> the next slot after <code>rear</code> coincides with <code>front</code>, so <code>isFull()</code> returns true and any further <code>enqueue()</code> must be rejected. <b>After a dequeue:</b> once <code>front</code> moves forward (freeing its old slot), <code>isFull()</code> immediately returns false again, even if that freed slot is only reachable by wrapping around.</p>
          </div>
        </div>
      </div>
    </div>
...
.ll-box-next { outline: 2px dashed var(--orange); outline-offset: 2px; }
...
.ll-legdot-next { background: transparent; border: 2px dashed var(--orange); }
...
.ll-badge-success { border-left-color: var(--green); background: var(--green-light); color: #15803d; }
