...
const CODES = {
  java: [
    ...
    ['c_underflow', '        }'],
    ['c_read', '        int value = arr[front];'],
    ['c_reset', '        if (front == rear) {'],
    ['c_reset', '            front = -1;'],
    ['c_reset', '            rear = -1;'],
    ['c_reset', '        } else {'],
    ['c_wrap', '            front = (front + 1) % capacity;'],
    ['c_reset', '        }'],
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_dequeue', 'int dequeue(struct CircularQueue *q) {'],
    ['c_underflow', '    if (q->front == -1) {'],
    ['c_underflow', '        printf("Queue Underflow\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_read', '    int value = q->arr[q->front];'],
    ['c_reset', '    if (q->front == q->rear) {'],
    ['c_reset', '        q->front = -1;'],
    ['c_reset', '        q->rear = -1;'],
    ['c_reset', '    } else {'],
    ['c_wrap', '        q->front = (q->front + 1) % q->capacity;'],
    ['c_reset', '    }'],
    ['c_return', '    return value;'],
    ['', '}'],
  ],
  ...
  python: [
    ...
    ['c_read', '        value = self.arr[self.front]'],
    ['c_reset', '        if self.front == self.rear:'],
    ['c_reset', '            self.front = -1'],
    ['c_reset', '            self.rear = -1'],
    ['c_reset', '        else:'],
    ['c_wrap', '            self.front = (self.front + 1) % self.capacity'],
    ['c_return', '        return value'],
  ],
  javascript: [
    ['', 'class CircularQueue {'],
    ...
    ['c_underflow', '    }'],
    ['c_read', '    const value = this.arr[this.front];'],
    ['c_reset', '    if (this.front === this.rear) {'],
    ['c_reset', '      this.front = -1;'],
    ['c_reset', '      this.rear = -1;'],
    ['c_reset', '    } else {'],
    ['c_wrap', '      this.front = (this.front + 1) % this.capacity;'],
    ['c_reset', '    }'],
    ['c_return', '    return value;'],
    ['', '  }'], ['', '}'],
  ],
...
//   'empty'  - unused slot, drawn dashed and blank (includes a slot right
//              after it has been dequeued and settled)
//   'normal' - an occupied, settled slot
//   'cur'    - the front slot, selected and read into `value`, still fully
//              part of the queue
//   'del'    - front has already moved past it (possibly wrapping around);
//              it is logically removed even though the value briefly
//              lingers on screen
function buildSteps(capacity, frontIdx, values) {
  ...
  if (front === -1) {
    pushStep({
      badge: 'front == -1 \u2192 Queue Underflow, nothing to dequeue',
  ...
  pushStep({
    badge: 'front != -1 \u2192 queue is not empty, proceed',
    code: 'c_underflow',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [])],
  });

  // --- value = arr[front]: read the front element. Nothing has been
  // removed yet, so the slot stays fully part of the queue - just selected.
  const dequeuedVal = valAt[front];
  stateAt[front] = 'cur';
  pushStep({
    badge: 'value = arr[front] \u2192 reads ' + dequeuedVal + ' from index ' + front,
    code: 'c_read',
    dequeuedVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(dequeuedVal), true]])],
  });

  // --- front is about to move past this slot (directly to -1, or by one
  // position, possibly wrapping) - it is now logically unreachable, even
  // though its value briefly lingers on screen.
  const oldFrontIdx = front;
  stateAt[oldFrontIdx] = 'del';

  if (front === rear) {
    front = -1;
    rear = -1;
    pushStep({
      badge: 'front == rear \u2192 only one element was left, reset front = rear = -1',
      code: 'c_reset',
      dequeuedVal,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(dequeuedVal), true]])],
    });
    ...
    const wrapped = newFront < prevFront;
    front = newFront;
    pushStep({
      badge: wrapped
        ? 'front = (front + 1) % capacity \u2192 wraps around from index ' + prevFront + ' back to index ' + front
        : 'front = (front + 1) % capacity \u2192 front advances to index ' + front,
      code: 'c_wrap',
      dequeuedVal,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(dequeuedVal), true]])],
    });
  ...
  // --- return value: the operation hands the dequeued value back. Only now
  // does the slot visually clear to an ordinary empty slot.
  delete stateAt[oldFrontIdx];
  delete valAt[oldFrontIdx];
  pushStep({
    badge: 'return value \u2192 ' + fnLabel + ' returns ' + dequeuedVal,
    code: 'c_return',
    dequeuedVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
...
const inpFront = ref(4);
const inpElems = ref('70 10 20');
const lang = ref('java');
const speed = ref(650);
const si = ref(0);
...
const stepsData = reactive(buildSteps(5, 4, [70, 10, 20]));
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
            ...
            <div class="ll-arr-track">
              <template v-for="cell in s.cells" :key="cell.idx">
                <div class="ll-arr-cell-wrap">
                  <div class="ll-who" :class="{ 'll-c-orange': cell.state === 'cur', 'll-c-red': cell.state === 'del' }">{{ cellLabel(cell) }}</div>
                  <div
                    class="ll-arr-box"
                    :class="{
                      'll-box-empty': cell.state === 'empty',
                      'll-box-cur': cell.state === 'cur',
                      'll-box-del ll-box-pending': cell.state === 'del'
                    }"
                  >
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>selected (front), still in the queue</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-del"></span>logically removed &mdash; about to clear</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty queue (underflow)</td><td>O(1)</td><td>a single comparison, <code>front == -1</code>, detects it and nothing is read</td></tr>
                <tr><td>Single-element queue</td><td>O(1)</td><td><code>front == rear</code> is checked directly, then both reset to -1</td></tr>
                <tr><td>Normal dequeue</td><td>O(1)</td><td>direct read from <code>arr[front]</code> followed by one modulo operation &mdash; no shifting of other elements</td></tr>
                <tr><td>Wrap-around dequeue</td><td>O(1)</td><td><code>(front + 1) % capacity</code> jumps front from <code>capacity - 1</code> back to <code>0</code> in one step, same cost as any other dequeue</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used or freed per dequeue &mdash; the array stays the same fixed size</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: unlike a Simple (Linear) Queue, dequeue here never leaves a permanently
              "dead" prefix of the array. The freed slot at the old <code>front</code> becomes available
              again the moment <code>rear</code> wraps back around to it on a future enqueue, because
              <code>front</code> itself also wraps via <code>(front + 1) % capacity</code>.
            </p>
            <h3>Advantages of Circular Queue</h3>
            <p>Dequeue is still O(1), and the freed slot is immediately reusable by a later enqueue once <code>rear</code> wraps around &mdash; there is no permanent loss of capacity the way there is in a Simple Queue.</p>
            <h3>Limitations</h3>
            <p>Because <code>front</code> and <code>rear</code> can both wrap, code must consistently use modulo arithmetic and the sentinel <code>front == -1</code> for "empty" rather than assuming <code>front &lt;= rear</code> always holds; a Circular Queue is slightly more error-prone to implement correctly than a Simple Queue for this reason.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p>In a <b>Simple (Linear) Queue</b>, dequeue only ever moves <code>front</code> forward towards <code>rear</code>; once <code>front &gt; rear</code> the queue is empty and every earlier index below the old <code>front</code> is permanently unusable until the queue is reset. In a <b>Circular Queue</b>, dequeue moves <code>front</code> forward using <code>(front + 1) % capacity</code>, so once it passes the last index it wraps back to <code>0</code> &mdash; letting those freed slots be reused by later enqueues instead of being wasted.</p>
            <h3>Key Observations</h3>
            <p>The check <code>front == rear</code> (rather than a range check) is what identifies "only one element left", because after removing it both pointers must reset to <b>-1</b> to mark the queue empty again. On every other dequeue, only <code>front</code> moves, using the wrap-aware formula <code>(front + 1) % capacity</code>, while <code>rear</code> stays exactly where it was.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> <code>front == -1</code> immediately signals <b>Queue Underflow</b>, and neither pointer nor the array is touched. <b>Single-element queue:</b> after reading the value, <code>front == rear</code> is true, so both reset to -1. <b>Multiple-element queue:</b> only <code>front</code> advances, via <code>(front + 1) % capacity</code>, leaving <code>rear</code> untouched. <b>Wrap-around deletion:</b> when <code>front</code> is at <code>capacity - 1</code>, the next dequeue sends it back to index <code>0</code> instead of running off the end of the array.</p>
          </div>
        </div>
      </div>
    </div>
