...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_isempty', 'int isEmpty(struct CircularQueue *q) {'],
    ['c_check', '    return q->front == -1;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class CircularQueue {'],
...
// This operation never touches the array or the pointers, so every cell
// simply stays 'normal' (occupied) or 'empty' throughout - there is no
// 'cur' / 'new' / 'del' phase for isEmpty().
function buildSteps(capacity, frontIdx, values) {
  ...
  if (front === -1) {
    pushStep({
      badge: 'front == -1 \u2192 isEmpty() returns true \u2014 Queue is Empty',
      code: 'c_check',
      success: true,
    ...
    pushStep({
      badge: 'front != -1 \u2192 isEmpty() returns false \u2014 Queue is Not Empty',
      code: 'c_check',
      success: false,
...
const stepsData = reactive(buildSteps(5, 2, [30, 40]));
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg">isEmpty() only inspects <code>front</code> &mdash; it never reads the array</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty queue</td><td>O(1)</td><td>single comparison <code>front == -1</code>, no array access</td></tr>
                <tr><td>Non-empty queue</td><td>O(1)</td><td>same single comparison, regardless of how many elements are present</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory &mdash; only reads the existing <code>front</code> field</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>isEmpty()</code> depends entirely on the sentinel value <code>front == -1</code>,
              not on comparing <code>front</code> and <code>rear</code>. This sentinel is exactly what
              lets a Circular Queue tell "empty" apart from "full" even though both <code>enqueue()</code>
              and <code>dequeue()</code> can leave <code>front</code> and <code>rear</code> pointing at the same index.
            </p>
            <h3>Advantages</h3>
            <p>A single O(1) check that calling code can use before <code>dequeue()</code> or <code>front()</code> to safely avoid triggering a Queue Underflow.</p>
            <h3>Limitations</h3>
            <p>Only reports empty vs. not-empty &mdash; it says nothing about how many elements are present. For that, <code>size()</code> is needed instead.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p><code>isEmpty()</code> is written identically for a Simple (Linear) Queue and a Circular Queue &mdash; both use the same <code>front == -1</code> sentinel, set once at construction and restored whenever the last element is dequeued. The circular wrap-around behavior of <code>enqueue()</code> and <code>dequeue()</code> does not change this check at all.</p>
            <h3>Key Observations</h3>
            <p>Because <code>front</code> is reset to <b>-1</b> both at construction and whenever the queue is fully drained, <code>front == -1</code> is a reliable, constant-time way to detect emptiness without needing to track a separate element count.</p>
            <h3>Edge Cases</h3>
            <p><b>Freshly constructed queue:</b> <code>front</code> starts at -1, so <code>isEmpty()</code> correctly returns true before any enqueue happens. <b>After the last dequeue:</b> <code>front</code> and <code>rear</code> are reset to -1 together, so <code>isEmpty()</code> becomes true again immediately. <b>Full queue:</b> even when the queue holds <code>capacity</code> elements, <code>front</code> is a valid index (never -1), so <code>isEmpty()</code> correctly returns false.</p>
          </div>
        </div>
      </div>
    </div>
