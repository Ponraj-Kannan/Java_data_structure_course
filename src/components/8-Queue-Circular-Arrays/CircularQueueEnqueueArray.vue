...
const CODES = {
  java: [
    ...
    ['c_fields', '    int front;'],
    ['c_fields', '    int rear;'], ['', ''],
    ['c_ctor', '    CircularQueue(int size) {'],
    ['c_ctor', '        capacity = size;'],
    ['c_ctor', '        arr = new int[capacity];'],
    ['c_ctor', '        front = -1;'],
    ['c_ctor', '        rear = -1;'],
    ['c_ctor', '    }'], ['', ''],
    ['c_enqueue', '    void enqueue(int value) {'],
    ['c_full', '        if ((rear + 1) % capacity == front) {'],
    ['c_full', '            System.out.println("Queue Overflow");'],
    ['c_full', '            return;'],
    ['c_full', '        }'],
    ['c_emptyinit', '        if (front == -1) {'],
    ['c_emptyinit', '            front = 0;'],
    ['c_emptyinit', '            rear = 0;'],
    ['c_emptyinit', '        } else {'],
    ['c_wrap', '            rear = (rear + 1) % capacity;'],
    ['c_emptyinit', '        }'],
    ['c_insert', '        arr[rear] = value;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ['', '#define MAX 100'], ['', ''],
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_enqueue', 'void enqueue(struct CircularQueue *q, int value) {'],
    ['c_full', '    if ((q->rear + 1) % q->capacity == q->front) {'],
    ['c_full', '        printf("Queue Overflow\\n");'],
    ['c_full', '        return;'],
    ['c_full', '    }'],
    ['c_emptyinit', '    if (q->front == -1) {'],
    ['c_emptyinit', '        q->front = 0;'],
    ['c_emptyinit', '        q->rear = 0;'],
    ['c_emptyinit', '    } else {'],
    ['c_wrap', '        q->rear = (q->rear + 1) % q->capacity;'],
    ['c_emptyinit', '    }'],
    ['c_insert', '    q->arr[q->rear] = value;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class CircularQueue {'],
    ...
    ['c_enqueue', '    void enqueue(int value) {'],
    ['c_full', '        if ((rear + 1) % capacity == front) {'],
    ['c_full', '            cout << "Queue Overflow" << endl;'],
    ['c_full', '            return;'],
    ['c_full', '        }'],
    ['c_emptyinit', '        if (front == -1) {'],
    ['c_emptyinit', '            front = 0;'],
  ...
  python: [
    ...
    ['c_ctor', '        self.front = -1'],
    ['c_ctor', '        self.rear = -1'], ['', ''],
    ['c_enqueue', '    def enqueue(self, value):'],
    ['c_full', '        if (self.rear + 1) % self.capacity == self.front:'],
    ['c_full', '            print("Queue Overflow")'],
    ['c_full', '            return'],
    ['c_emptyinit', '        if self.front == -1:'],
    ['c_emptyinit', '            self.front = 0'],
    ['c_emptyinit', '            self.rear = 0'],
    ['c_emptyinit', '        else:'],
    ['c_wrap', '            self.rear = (self.rear + 1) % self.capacity'],
    ['c_insert', '        self.arr[self.rear] = value'],
  ],
  javascript: [
    ['', 'class CircularQueue {'],
    ...
    ['c_ctor', '    this.rear = -1;'],
    ['c_ctor', '  }'], ['', ''],
    ['c_enqueue', '  enqueue(value) {'],
    ['c_full', '    if ((this.rear + 1) % this.capacity === this.front) {'],
    ['c_full', '      console.log("Queue Overflow");'],
    ['c_full', '      return;'],
    ['c_full', '    }'],
    ['c_emptyinit', '    if (this.front === -1) {'],
    ['c_emptyinit', '      this.front = 0;'],
    ['c_emptyinit', '      this.rear = 0;'],
    ['c_emptyinit', '    } else {'],
    ['c_wrap', '      this.rear = (this.rear + 1) % this.capacity;'],
    ['c_emptyinit', '    }'],
    ['c_insert', '    this.arr[this.rear] = value;'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'normal' - an occupied, settled slot (includes the enqueued value once
//              the operation has fully completed)
//   'cur'    - the slot rear has just moved onto (possibly after wrapping),
//              before a value is written
//   'new'    - the slot that just received the enqueued value, until the
//              operation finishes
function isFullQueue(front, rear, capacity) {
  if (capacity === 0) return true;
  return front !== -1 && (rear + 1) % capacity === front;
}

function buildSteps(capacity, frontIdx, values, enqueueValue) {
  const steps = [];
  const phaseStarts = { enqueue: 0 };
  ...
  const fnLabel = 'enqueue(' + enqueueValue + ')';
  ...
  if (capacity === 0) {
    pushStep({
      badge: 'capacity == 0 \u2192 Queue Overflow, no array cells exist to hold ' + enqueueValue,
      code: 'c_full', invalid: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
    });
    return { steps, phaseStarts };
  }

  if (isFullQueue(front, rear, capacity)) {
    pushStep({
      badge: '(rear + 1) % capacity == front \u2192 Queue Overflow, ' + enqueueValue + ' cannot be inserted',
      code: 'c_full', invalid: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
    });
    return { steps, phaseStarts };
  ...
  pushStep({
    badge: '(rear + 1) % capacity != front \u2192 queue is not full, proceed',
    code: 'c_full',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
  });

  // --- if front == -1: front = rear = 0. Only happens the very first time
  // an element is inserted into a completely empty queue.
  if (front === -1) {
    front = 0;
    rear = 0;
    stateAt[rear] = 'cur';
    pushStep({
      badge: 'front == -1 \u2192 queue was empty, set front = rear = 0',
      code: 'c_emptyinit',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
    });
  } else {
    ...
    const wrapped = newRear < prevRear;
    rear = newRear;
    stateAt[rear] = 'cur';
    pushStep({
      badge: wrapped
        ? 'rear = (rear + 1) % capacity \u2192 wraps around from index ' + prevRear + ' back to index ' + rear
        : 'rear = (rear + 1) % capacity \u2192 rear now points to index ' + rear + ' (still empty)',
      code: 'c_wrap',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
    });
  }

  // --- arr[rear] = value: the value is actually written into that slot.
  valAt[rear] = enqueueValue;
  stateAt[rear] = 'new';
  pushStep({
    badge: 'arr[rear] = ' + enqueueValue + ' \u2192 index ' + rear + ' now holds ' + enqueueValue,
    code: 'c_insert',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(enqueueValue), true]])],
  });
...
const inpElems = ref('40 50');
const inpValue = ref(60);
const lang = ref('java');
const speed = ref(650);
const si = ref(0);
...
const stepsData = reactive(buildSteps(5, 3, [40, 50], 60));
...
function applyInput() {
  ...
  if (isNaN(val)) { alert('Enter a valid number to enqueue.'); return; }

  playing.value = false;
  const built = buildSteps(cap, frontIdx, arr, val);
...
<template>
  <div class="slide-wrapper">
    ...
    <div class="ll-toolbar">
      ...
      <input type="text" v-model="inpElems" style="width: 80px;" placeholder="Enter numbers, or leave blank for an empty queue" class="ll-text-input" />
      <label>Value</label>
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
            ...
            <div class="ll-arr-track">
              <template v-for="cell in s.cells" :key="cell.idx">
                <div class="ll-arr-cell-wrap">
                  <div class="ll-who" :class="{ 'll-c-orange': cell.state === 'cur', 'll-c-green': cell.state === 'new' }">{{ cellLabel(cell) }}</div>
                  <div
                    class="ll-arr-box"
                    :class="{
                      'll-box-empty': cell.state === 'empty',
                      'll-box-cur ll-box-pending': cell.state === 'cur',
                      'll-box-new': cell.state === 'new'
                    }"
                  >
                    {{ cell.state === 'empty' || cell.state === 'cur' ? '' : cell.val }}
                  </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>rear's target slot (wraps via % capacity)</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-new"></span>newly enqueued value</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty queue / first insertion</td><td>O(1)</td><td>just set <code>front = rear = 0</code> and write the value</td></tr>
                <tr><td>Normal enqueue</td><td>O(1)</td><td>direct write to <code>arr[rear]</code> after a single modulo operation &mdash; no traversal or shifting</td></tr>
                <tr><td>Wrap-around enqueue</td><td>O(1)</td><td><code>(rear + 1) % capacity</code> jumps rear from <code>capacity - 1</code> back to <code>0</code> in one step, same cost as any other enqueue</td></tr>
                <tr><td>Full queue (overflow)</td><td>O(1)</td><td>a single comparison, <code>(rear + 1) % capacity == front</code>, detects it and nothing is written</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is allocated per enqueue &mdash; the array is pre-allocated once, size <code>capacity</code></td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: the modulo operation <code>(rear + 1) % capacity</code> is what makes the
              queue "circular" &mdash; once <code>rear</code> passes the last index (<code>capacity - 1</code>),
              it wraps back to index <code>0</code> instead of stopping. This lets a Circular Queue reuse
              slots that were freed by earlier dequeues, something a Simple (Linear) Queue can never do.
            </p>
            <h3>Advantages of Circular Queue</h3>
            <p>Reclaims freed-up space at the front of the array, so a fixed-size buffer can be reused indefinitely as long as the number of elements present never exceeds <code>capacity</code>. Still O(1) for enqueue and dequeue, and remains cache-friendly contiguous memory.</p>
            <h3>Limitations</h3>
            <p>The capacity is still fixed at creation time &mdash; a Circular Queue does not grow. Index arithmetic requires the modulo operator, which is slightly less intuitive than a Simple Queue's plain increment. Distinguishing "full" from "empty" also needs care: both states can otherwise look like <code>front == rear</code>, which is why this implementation uses the sentinel <code>front == -1</code> to mean "empty" and the formula <code>(rear + 1) % capacity == front</code> to mean "full".</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p>A <b>Simple (Linear) Queue</b> only ever advances <code>rear</code> forward from <code>0</code> towards <code>capacity - 1</code> and then reports <b>Queue Overflow</b> for good, even if slots before <code>front</code> have since been freed by dequeues. A <b>Circular Queue</b> wraps both <code>front</code> and <code>rear</code> back to index <code>0</code> using modulo arithmetic once they pass the last index, so those freed slots get reused and the queue only overflows once it is genuinely full of <code>capacity</code> elements.</p>
            <h3>Key Observations</h3>
            <p><code>front</code> and <code>rear</code> both start at <b>-1</b> for an empty queue. The very first enqueue sets <b>both</b> <code>front</code> and <code>rear</code> to <b>0</b>; every enqueue after that only advances <code>rear</code> via <code>(rear + 1) % capacity</code>, leaving <code>front</code> untouched. The current size is always <code>((rear - front + capacity) % capacity) + 1</code> (or 0 when the queue is empty), a formula that stays correct even when <code>rear</code> has wrapped past <code>front</code>.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> the first enqueue sets <code>front</code> and <code>rear</code> from -1 to 0 together, in addition to placing the value. <b>Multiple insertions:</b> each subsequent enqueue only advances <code>rear</code> and writes the new value, leaving <code>front</code> unchanged. <b>Wrap-around insertion:</b> when <code>rear</code> is at <code>capacity - 1</code>, the next enqueue sends it back to index <code>0</code> instead of overflowing, provided that slot isn't occupied by <code>front</code>. <b>Full queue:</b> when <code>(rear + 1) % capacity == front</code>, enqueue must detect overflow and leave the queue completely unchanged &mdash; no write happens and neither pointer moves.</p>
          </div>
        </div>
      </div>
    </div>
