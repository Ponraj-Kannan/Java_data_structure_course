...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_peek', 'int peek(struct CircularQueue *q) {'],
    ['c_underflow', '    if (q->front == -1) {'],
    ['c_underflow', '        printf("Queue Underflow\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_read', '    return q->arr[q->front];'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class CircularQueue {'],
...
const PSEUDOCODE = [
  'function peek():          // the "front" operation',
  '    if front == -1:',
...
//   'cur'    - the front slot, highlighted while its value is being read;
//              reverts to 'normal' once the operation completes, since
//              front() never modifies the queue
function buildSteps(capacity, frontIdx, values) {
  const steps = [];
  const phaseStarts = { front: 0 };
  ...
  if (front === -1) {
    pushStep({
      badge: 'front == -1 \u2192 Queue Underflow, there is no front element',
  ...
  // --- return arr[front]: highlight the front slot while its value is read.
  // Nothing is removed or written - the queue's contents and both pointers
  // stay exactly as they were.
  const frontVal = valAt[front];
  stateAt[front] = 'cur';
  pushStep({
    badge: 'arr[front] \u2192 index ' + front + ' holds ' + frontVal + ' (no structural change)',
    code: 'c_read',
    frontVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(frontVal), true]])],
  });

  // --- settle back to normal: front() is non-destructive, so the slot
  // simply returns to its ordinary occupied appearance.
  stateAt[front] = 'normal';
  pushStep({
    badge: fnLabel + ' complete \u2014 front element is ' + frontVal + ', queue unchanged (size still ' + size() + ' / ' + capacity + ')',
    code: 'c_peek',
    frontVal,
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>front &mdash; highlighted, not removed</span>
        </div>
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                ...
                <tr><td>Peek at front</td><td>O(1)</td><td>direct array read <code>arr[front]</code> &mdash; no traversal, no pointer movement</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used &mdash; the queue's array, front, and rear are all left exactly as they were</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>front()</code> is purely a read. Unlike <code>dequeue()</code>, it never
              changes <code>front</code>, never changes <code>rear</code>, and never clears a slot &mdash;
              it only reports the value currently sitting at whatever index <code>front</code> points to,
              even if that index has already wrapped around past <code>capacity - 1</code> from earlier operations.
            </p>
            <h3>Advantages</h3>
            <p>Lets code inspect the next element that <code>dequeue()</code> would remove, without committing to actually removing it &mdash; useful for lookahead logic, conditional processing, or simply confirming the queue has the expected next value.</p>
            <h3>Limitations</h3>
            <p>Only exposes the single element at <code>front</code>; to inspect the entire contents of a Circular Queue, code still has to walk <code>size()</code> elements starting from <code>front</code> and applying <code>% capacity</code> at each step, since the values may be split across the wrap point.</p>
            <h3>Simple Queue vs Circular Queue</h3>
            <p>The <code>front()</code> operation itself is identical in cost and logic for a Simple (Linear) Queue and a Circular Queue &mdash; both simply return <code>arr[front]</code> after checking <code>front == -1</code>. The only difference is what <code>front</code> can be: in a Circular Queue it may point anywhere from <code>0</code> to <code>capacity - 1</code> as a result of earlier wrap-arounds, whereas in a Simple Queue it only ever increases from <code>0</code> towards <code>rear</code>.</p>
            <h3>Key Observations</h3>
            <p><code>front()</code> and <code>dequeue()</code> read the exact same value &mdash; <code>arr[front]</code> &mdash; but only <code>dequeue()</code> advances the pointer afterwards. This makes <code>front()</code> safe to call any number of times in a row without changing the queue at all.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty queue:</b> <code>front == -1</code> immediately signals <b>Queue Underflow</b>, and no array access happens. <b>Single-element queue:</b> <code>front</code> and <code>rear</code> are the same index, and that one value is returned. <b>Multiple-element queue:</b> only the value at <code>front</code> is returned, regardless of how many elements follow it. <b>Wrapped front:</b> if earlier dequeues have already moved <code>front</code> past index <code>capacity - 1</code> and back to <code>0</code>, <code>front()</code> still works exactly the same way &mdash; it simply reads whatever index <code>front</code> currently holds.</p>
          </div>
        </div>
      </div>
    </div>
