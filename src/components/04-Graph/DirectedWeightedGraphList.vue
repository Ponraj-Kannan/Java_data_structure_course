...
const CODES = {
  ...
  c: [
    ...
    ['c_set_uv', '    newNode->weight = weight;'],
...
const PSEUDOCODE = [
  ...
  '        adjList.get(source).add(Pair(destination, weight))  // directed (source -> destination)',
  '',
  'main():',
  '    read vertices',
...
function buildSteps(numV, edgeList) {
  ...
  for (let i = 0; i < edgeList.length; i++) {
    ...
    if (isValid) {
      ...
      const existingEdge = processedEdges.find(e => e.u === u && e.v === v);
      if (existingEdge) existingEdge.weight = weight;
      else processedEdges.push({ u, v, weight });

      steps.push({
        badge: `adjList.get(${u}).add(new Pair(${v}, ${weight})); → Added Pair(${v}, ${weight}) to source vertex ${u}'s list (directed u → v)`,
        code: 'c_set_uv',
  ...
  steps.push({
    badge: `main() finished: All ${E} directed weighted edges added to Graph Adjacency List.`,
    code: 'c_main_done',
      ...
      <div class="row-main">
        <div class="ll-root">
          ...
          <div class="ll-toolbar">
            ...
            <label>Edges</label>
            <input type="text" v-model="edgesInputStr" style="width: 250px;" class="ll-text-input" placeholder="e.g. [[0,1,5],[0,2,4],[1,2,3]]" @keyup.enter="applySetup" />
          ...
          <div class="ll-main" ref="mainRef">
            ...
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              ...
              <div class="ll-legend">
                ...
                <span class="ll-leg"><span class="ll-legdot ll-legdot-has-edge"></span>Neighbor Pair(v, w)</span>
                ...
                <div v-else class="ll-info-scroll">
                  ...
                  <table class="ll-complexity-table">
                    ...
                    <tbody>
                      ...
                      <tr><td>Add Edge <code>addEdge(u, v, weight)</code></td><td>O(1)</td><td>Appends <code>Pair(v, weight)</code> to <code>adj[u]</code> (directed source bucket).</td></tr>
                      ...
                      <tr><td>Space Complexity</td><td>O(V + E)</td><td>Stores V vertex heads and E directed neighbor Pair nodes.</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    Key feature: <b>Adjacency List Collections</b> for directed weighted graphs store directed edges as <code>Pair(destination, weight)</code> in source list <code>adj[u]</code>. Uses efficient <code>O(V + E)</code> memory.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div class="graph-modal-body">
          ...
          <svg v-else class="graph-modal-svg" viewBox="0 0 720 470">
            ...
            >
              ...
              <!-- Visible Directed Edge Line with Arrow -->
              <path
                :d="getEdgeGeo(edge).pathD"
                fill="none"
                class="ll-edge-line"
...
/* Neighbor Box (Soft Emerald/Teal, Pair v (w)) */
.ll-adj-chip {
  height: 32px;
  background: #ecfdf5;
...
.ll-chip-weight { font-size: 11px; font-weight: 700; color: #475569; margin-left: 3px; }
.ll-adj-chip-hovered {
...
.ll-adj-chip-hovered .ll-chip-weight { color: #065f46 !important; }
.ll-adj-chip-added {
...
.ll-adj-chip-added .ll-chip-weight { color: #14532d !important; }
