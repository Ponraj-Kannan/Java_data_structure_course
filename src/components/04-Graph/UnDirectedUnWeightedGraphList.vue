...
const CODES = {
  java: [
    ...
    ['c_set_vu', '        adjList.get(destination).add(source);  // symmetric'],
    ['c_add_edge_done', '    }'],
    ['', '}'],
    ['', ''],
    ['', 'public class Main {'],
  ...
  cpp: [
    ...
    ['c_set_vu', '        adjList[destination].push_back(source);  // symmetric'],
    ['c_add_edge_done', '    }'],
    ['', '};'],
    ['', ''],
    ['c_main_entry', 'int main() {'],
  ...
  python: [
    ...
    ['c_set_vu', '        self.adjList[destination].append(source)  # symmetric'],
  ...
  javascript: [
    ...
    ['c_set_vu', '        this.adjList[destination].push(source);  // symmetric'],
    ['c_add_edge_done', '    }'],
    ['', '}'],
    ['', ''],
...
const PSEUDOCODE = [
  ...
  '        adjList.get(destination).add(source)  // symmetric (undirected)',
  '',
  'main():',
  '    read vertices',
...
function buildSteps(numV, edgeList) {
  ...
  for (let i = 0; i < V; i++) {
    steps.push({
      ...
      V, adjList: cloneAdj(initialAdj), activeU: -1, activeV: -1, activeK: -1, curI: i, curJ: -1, edges: []
    });
  }
  ...
  for (let i = 0; i < edgeList.length; i++) {
    ...
    if (isValid) {
      if (!currentAdj[u].includes(v)) currentAdj[u].push(v);

      steps.push({
        badge: `adjList.get(${u}).add(${v}); → Added neighbor ${v} to vertex ${u}'s list`,
        code: 'c_set_uv',
        vars: [frame('main()', [['i', String(i)]]), frame('addEdge()', [['source', String(u)], ['destination', String(v)], [`adjList[${u}]`, JSON.stringify(currentAdj[u])]])],
        V, adjList: cloneAdj(currentAdj), activeU: u, activeV: v, activeK: i, curI: u, curJ: v, edges: [...processedEdges, { u, v }]
      });

      if (!currentAdj[v].includes(u)) currentAdj[v].push(u);
      processedEdges.push({ u, v });

      steps.push({
        badge: `adjList.get(${v}).add(${u}); → Added symmetric neighbor ${u} to vertex ${v}'s list (undirected)`,
        code: 'c_set_vu',
        vars: [frame('main()', [['i', String(i)]]), frame('addEdge()', [['source', String(u)], ['destination', String(v)], [`adjList[${v}]`, JSON.stringify(currentAdj[v])]])],
        V, adjList: cloneAdj(currentAdj), activeU: v, activeV: u, activeK: i, curI: v, curJ: u, edges: [...processedEdges]
      });
    }
  ...
  steps.push({
    badge: `main() finished: All ${E} undirected unweighted edges added to Graph Adjacency List.`,
    code: 'c_main_done',
...
const undirectedEdgesForRender = computed(() => {
  ...
  if (s.value && s.value.activeU >= 0 && s.value.activeV >= 0) {
    ...
    if (au === av) {
      const key = `loop-${au}`;
      if (!seen.has(key)) { seen.add(key); result.push({ u: au, v: av }); }
    } else {
      const key = au < av ? `${au}-${av}` : `${av}-${au}`;
      if (!seen.has(key)) { seen.add(key); result.push({ u: Math.min(au, av), v: Math.max(au, av) }); }
    }
  }
  return result;
});
                          ...
                          <div class="ll-adj-items">
                            <template v-if="s.adjList && s.adjList[uIdx - 1] && s.adjList[uIdx - 1].length">
                              <span
                                ...
                                class="ll-adj-chip"
                                :class="{
                                  'll-adj-chip-hovered': isEdgeHovered(uIdx - 1, neighbor),
                                  'll-adj-chip-added': ((uIdx - 1) === s.curI && neighbor === s.curJ) || ((uIdx - 1) === s.curJ && neighbor === s.curI)
                                }"
                                @mouseenter="hoveredEdge = { u: uIdx - 1, v: neighbor }"
                ...
                <div v-else class="ll-info-scroll">
                  ...
                  <table class="ll-complexity-table">
                    ...
                    <tbody>
                      ...
                      <tr><td>Add Edge <code>addEdge(u, v)</code></td><td>O(1)</td><td>Appends <code>v</code> to <code>adj[u]</code> and <code>u</code> to <code>adj[v]</code>.</td></tr>
                      <tr><td>Query Edge <code>getEdge(u, v)</code></td><td>O(deg(u))</td><td>Scans list <code>adj[u]</code> for neighbor <code>v</code>.</td></tr>
                      <tr><td>Space Complexity</td><td>O(V + E)</td><td>Stores V vertex heads and 2E neighbor nodes.</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    Key feature: <b>Adjacency List Collections</b> for undirected unweighted graphs store edges symmetrically (<code>adj[u].add(v)</code> and <code>adj[v].add(u)</code>). Uses efficient <code>O(V + E)</code> memory.
                  </p>
                </div>
              </div>
            </div>
...
/* Floating Modal Dialog Container */
...
.ll-legdot-has-edge { background: #ecfdf5; border: 1.5px solid #34d399; }
.ll-legdot-u { background: #dbeafe; border: 1.5px solid #3b82f6; }
.ll-legdot-v { background: #f3e8ff; border: 1.5px solid #9333ea; }
.ll-legdot-active { background: #bbf7d0; border: 1.5px solid #22c55e; }
