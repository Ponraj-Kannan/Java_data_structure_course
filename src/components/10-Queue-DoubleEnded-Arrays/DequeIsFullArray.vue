...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_isfull', 'int isFull(struct Deque *dq) {'],
    ['c_check', '    return (dq->rear + 1) % dq->capacity == dq->front;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Deque {'],
...
function buildSteps(capacity, frontIdx, values) {
  ...
  if (capacity === 0) {
    result = true;
    pushStep({
      badge: 'capacity == 0 \u2192 Deque is Full (there are no array cells at all)',
      code: 'c_check',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['result', String(result), true]])],
    });
  } else {
    nextSlot = (rear + 1) % capacity;
    result = nextSlot === front;
    if (result) stateAt[nextSlot] = stateAt[nextSlot] || 'peek';
    pushStep({
      badge: result
        ? '(rear + 1) % capacity \u2192 index ' + nextSlot + ', which equals front \u2192 Deque is Full'
        : '(rear + 1) % capacity \u2192 index ' + nextSlot + ', which is free (front is at ' + front + ') \u2192 Deque is Not Full',
      code: 'c_check',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['result', String(result), true]])],
    });
  }
  ...
  pushStep({
    badge: fnLabel + ' returned ' + result + ' \u2014 deque size ' + size() + ' / ' + capacity,
    code: 'c_isfull',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
...
const stepsData = reactive(buildSteps(5, 2, [10, 20, 30, 40, 50]));
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip">isFull = <b :class="s.result === null ? 'll-c-purple' : (s.result ? 'll-c-red' : 'll-c-green')">{{ s.result === null ? '\u2014' : s.result }}</b></div>
            </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-peek"></span>(rear + 1) % capacity slot being checked</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Any deque state</td><td>O(1)</td><td>one modulo operation plus one comparison, regardless of capacity or size</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used; the deque is left completely unchanged</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation, and why the deque becomes full exactly when <code>(rear + 1) % capacity == front</code>:
              inserting one more element would need to place it at slot <code>(rear + 1) % capacity</code> (the very next
              circular position after rear). If that slot is already <code>front</code>, writing there would overwrite
              the oldest element still in the deque and corrupt it &mdash; so the deque must instead refuse the insertion
              and report itself as full. In other words, one slot's worth of "room" is what separates a full deque from
              one that still has space, and this check is what detects that exact boundary.
            </p>
            <h3>Advantages of Deque</h3>
            <p>Checking fullness in O(1) lets insertFront and insertRear both guard themselves cheaply before writing,
              without ever needing to count how many elements are currently stored.</p>
            <h3>Limitations</h3>
            <p><code>isFull</code> only tells you whether there is room for <b>one more</b> element; it says nothing
              about how much other capacity is free. A deque with capacity 100 and 99 elements is exactly as "full" by
              this check as a deque with capacity 1 and 1 element.</p>
            <h3>Difference between Queue, Circular Queue, and Deque</h3>
            <p>A <b>Simple Queue</b> is considered full once <code>rear == capacity - 1</code>, even if slots before
              <code>front</code> are actually free, since it never wraps. A <b>Circular Queue</b> fixes that by using the
              same <code>(rear + 1) % capacity == front</code> check as a Deque. A <b>Deque</b> uses the identical check
              &mdash; the difference is only that a Deque can insert at either end, so this single fullness condition
              guards both <code>insertFront</code> and <code>insertRear</code>, whereas a Circular Queue only needs it
              for insertion at the rear.</p>
            <h3>Key Observations</h3>
            <p>A fixed-size circular deque with capacity <code>n</code> can hold at most <code>n</code> elements, but the
              full check deliberately leaves no way to distinguish "completely full" from "completely empty" using
              <code>front</code> and <code>rear</code> alone if all <code>n</code> slots were allowed to fill &mdash;
              which is exactly why the check <code>(rear + 1) % capacity == front</code> naturally caps real usable
              capacity at <code>n</code> elements while keeping the empty state (<code>front == -1</code>) unambiguous.
            </p>
            <h3>Edge Cases</h3>
            <p><b>Capacity 0:</b> there are no array cells at all, so the deque is always considered full and no
              insertion can ever succeed. <b>Empty deque:</b> <code>front == -1</code>, so <code>(rear + 1) % capacity</code>
              can never equal <code>front</code> and isFull correctly reports false. <b>Single free slot left:</b>
              isFull correctly reports false right up until that last slot is filled. <b>Completely full deque:</b>
              isFull reports true regardless of whether the occupied region wraps around the end of the array or not.</p>
          </div>
        </div>
      </div>
    </div>
