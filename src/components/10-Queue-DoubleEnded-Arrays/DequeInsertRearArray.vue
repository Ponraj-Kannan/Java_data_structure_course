...
const CODES = {
  java: [
    ...
    ['c_ctor', '        rear = -1;'],
    ['c_ctor', '    }'], ['', ''],
    ['c_insertrear', '    void insertRear(int value) {'],
    ['c_overflow', '        if ((rear + 1) % capacity == front) {'],
    ['c_overflow', '            System.out.println("Deque Overflow");'],
    ['c_overflow', '            return;'],
    ['c_overflow', '        }'],
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_insertrear', 'void insertRear(struct Deque *dq, int value) {'],
    ['c_overflow', '    if ((dq->rear + 1) % dq->capacity == dq->front) {'],
    ['c_overflow', '        printf("Deque Overflow\\n");'],
    ['c_overflow', '        return;'],
    ['c_overflow', '    }'],
    ['c_emptyinit', '    if (dq->front == -1) {'],
    ['c_emptyinit', '        dq->front = 0;'],
    ['c_emptyinit', '        dq->rear = 0;'],
    ['c_emptyinit', '    } else {'],
    ['c_wrap', '        dq->rear = (dq->rear + 1) % dq->capacity;'],
    ['c_emptyinit', '    }'],
    ['c_write', '    dq->arr[dq->rear] = value;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Deque {'],
    ...
    ['c_insertrear', '    void insertRear(int value) {'],
    ['c_overflow', '        if ((rear + 1) % capacity == front) {'],
    ['c_overflow', '            cout << "Deque Overflow" << endl;'],
    ['c_overflow', '            return;'],
    ['c_overflow', '        }'],
    ['c_emptyinit', '        if (front == -1) {'],
    ['c_emptyinit', '            front = 0;'],
  ...
  python: [
    ...
    ['c_ctor', '        self.front = -1'],
    ['c_ctor', '        self.rear = -1'], ['', ''],
    ['c_insertrear', '    def insert_rear(self, value):'],
    ['c_overflow', '        if (self.rear + 1) % self.capacity == self.front:'],
    ['c_overflow', '            print("Deque Overflow")'],
    ['c_overflow', '            return'],
    ...
    ['c_write', '        self.arr[self.rear] = value'],
  ],
  javascript: [
    ['', 'class Deque {'],
    ...
    ['c_ctor', '    this.rear = -1;'],
    ['c_ctor', '  }'], ['', ''],
    ['c_insertrear', '  insertRear(value) {'],
    ['c_overflow', '    if ((this.rear + 1) % this.capacity === this.front) {'],
    ['c_overflow', '      console.log("Deque Overflow");'],
    ['c_overflow', '      return;'],
    ['c_overflow', '    }'],
    ...
    ['c_emptyinit', '    }'],
    ['c_write', '    this.arr[this.rear] = value;'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'normal' - an occupied, settled slot (includes the inserted value once
//              the operation has fully completed)
//   'cur'    - the slot rear has just moved onto (possibly after wrapping),
//              before a value is written
//   'new'    - the slot that just received the inserted value, until the
//              operation finishes
function isFullDeque(front, rear, capacity) {
  if (capacity === 0) return true;
  return front !== -1 && (rear + 1) % capacity === front;
}

function buildSteps(capacity, frontIdx, values, insertValue) {
  const steps = [];
  const phaseStarts = { insertRear: 0 };
  ...
  if (capacity === 0) {
    pushStep({
      badge: 'capacity == 0 \u2192 Deque Overflow, no array cells exist to hold ' + insertValue,
      code: 'c_overflow', invalid: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(insertValue), true]])],
    });
    return { steps, phaseStarts };
  }

  if (isFullDeque(front, rear, capacity)) {
    pushStep({
      badge: '(rear + 1) % capacity == front \u2192 Deque Overflow, ' + insertValue + ' cannot be inserted',
      code: 'c_overflow', invalid: true,
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(insertValue), true]])],
    });
    return { steps, phaseStarts };
  ...
  pushStep({
    badge: '(rear + 1) % capacity != front \u2192 deque is not full, proceed',
    code: 'c_overflow',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(insertValue), true]])],
  });

  // --- if front == -1: front = rear = 0. Only happens the very first time
  // an element is inserted into a completely empty deque.
  if (front === -1) {
    front = 0;
    rear = 0;
    stateAt[rear] = 'cur';
    pushStep({
      badge: 'front == -1 \u2192 deque was empty, set front = rear = 0',
      code: 'c_emptyinit',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(insertValue), true]])],
    });
  } else {
  ...
  valAt[rear] = insertValue;
  stateAt[rear] = 'new';
  pushStep({
    badge: 'arr[rear] = ' + insertValue + ' \u2192 index ' + rear + ' now holds ' + insertValue,
    code: 'c_write',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(insertValue), true]])],
  });
  ...
  stateAt[rear] = 'normal';
  pushStep({
    badge: fnLabel + ' complete \u2014 deque size now ' + size() + ' / ' + capacity,
    code: 'c_insertrear',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
...
const stepsData = reactive(buildSteps(5, 0, [10, 20, 30], 40));
...
<template>
  <div class="slide-wrapper">
    ...
    <div class="ll-toolbar">
      ...
      <input type="text" v-model="inpElems" style="width: 80px;" placeholder="Enter numbers, or leave blank for an empty deque" class="ll-text-input" />
      <label>Value</label>
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>rear's target slot</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-new"></span>newly inserted value</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty deque / first insertion</td><td>O(1)</td><td>just set <code>front = rear = 0</code> and write the value</td></tr>
                <tr><td>Normal insertRear</td><td>O(1)</td><td>direct write to <code>arr[rear]</code> after one modulo increment &mdash; no traversal or shifting</td></tr>
                <tr><td>Wrap-around insertion</td><td>O(1)</td><td>the modulo operation folds index <code>capacity</code> back to <code>0</code> in constant time</td></tr>
                <tr><td>Full deque (overflow)</td><td>O(1)</td><td>a single comparison detects it, nothing is written</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is allocated per insertion &mdash; the array is pre-allocated once</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: because <code>rear</code> is advanced with <code>(rear + 1) % capacity</code> instead of a
              plain <code>rear + 1</code>, the pointer wraps from index <code>capacity - 1</code> straight back to
              <code>0</code> instead of running off the end of the array. This circular indexing is what lets a Deque
              reuse the slots freed up at the front end, and is the key difference from a simple linear array, where
              inserting once <code>rear</code> reaches the last index would simply fail even if earlier slots are free.
            </p>
            <h3>Advantages of Deque</h3>
            <p>Insertion and deletion are O(1) from <b>both</b> ends, unlike a simple array where inserting at the rear
              once the array is logically full would require resizing or shifting. The fixed-size circular array is
              cache-friendly and needs no per-element pointer overhead, unlike a linked-list based deque.</p>
            <h3>Limitations</h3>
            <p>The capacity is fixed at creation time. Once <code>(rear + 1) % capacity == front</code>, the deque is full
              and further insertions from either end fail with <b>Deque Overflow</b> until an element is removed.</p>
            <h3>Difference between Queue, Circular Queue, and Deque</h3>
            <p>A <b>Simple Queue</b> only allows insertion at the rear and removal at the front, and <code>rear</code>
              never wraps &mdash; once it hits <code>capacity - 1</code> it stops for good even if front slots are free.
              A <b>Circular Queue</b> fixes that by wrapping both pointers with modulo arithmetic, but still restricts
              insertion to the rear and removal to the front. A <b>Deque (Double-Ended Queue)</b> goes a step further:
              using the same circular indexing, it allows insertion <b>and</b> removal at <b>both</b> the front and the
              rear, making it strictly more flexible than either a simple or circular queue.</p>
            <h3>Key Observations</h3>
            <p><code>front</code> and <code>rear</code> both start at <b>-1</b> for an empty deque. The very first
              <code>insertRear</code> (or <code>insertFront</code>) sets both <code>front</code> and <code>rear</code> to
              <b>0</b>. Every insertRear after that only moves <code>rear</code> forward circularly, leaving
              <code>front</code> untouched. The current size is always <code>((rear - front + capacity) % capacity) + 1</code>
              (or 0 when the deque is empty), which correctly accounts for wrap-around.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty deque:</b> the first insertRear sets <code>front</code> and <code>rear</code> from -1 to 0 in
              addition to placing the value. <b>First insertion:</b> identical to the empty-deque case above. <b>Multiple
              insertions:</b> each subsequent insertRear only advances <code>rear</code> circularly and writes the new
              value, leaving <code>front</code> unchanged. <b>Wrap-around insertion:</b> when <code>rear</code> is at
              index <code>capacity - 1</code>, the next insertRear wraps it to <code>0</code> instead of running past the
              end of the array. <b>Full deque:</b> when <code>(rear + 1) % capacity == front</code>, insertRear must
              detect overflow and leave the deque completely unchanged &mdash; no write happens and neither pointer
              moves.</p>
          </div>
        </div>
      </div>
