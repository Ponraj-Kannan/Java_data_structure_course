...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_peekrear', 'int peekRear(struct Deque *dq) {'],
    ['c_underflow', '    if (dq->front == -1) {'],
    ['c_underflow', '        printf("Deque is Empty\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_read', '    return dq->arr[dq->rear];'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Deque {'],
  ...
  python: [
    ...
    ['c_ctor', '        self.front = -1'],
    ['c_ctor', '        self.rear = -1'], ['', ''],
    ['c_peekrear', '    def peek_rear(self):'],
    ['c_underflow', '        if self.front == -1:'],
    ['c_underflow', '            print("Deque is Empty")'],
    ['c_underflow', '            return -1'],
    ['c_read', '        return self.arr[self.rear]'],
  ],
  javascript: [
    ['', 'class Deque {'],
    ...
    ['c_underflow', '      return -1;'],
    ['c_underflow', '    }'],
    ['c_read', '    return this.arr[this.rear];'],
    ['', '  }'], ['', '}'],
  ],
};
...
//   'peek'   - the rear slot currently being viewed (purple highlight);
//              no value is removed and no pointer moves
function buildSteps(capacity, frontIdx, values) {
  const steps = [];
  const phaseStarts = { peekRear: 0 };
  ...
  // --- highlight arr[rear]; nothing is removed and no pointer moves.
  peeked = valAt[rear];
  stateAt[rear] = 'peek';
  pushStep({
    badge: 'return arr[rear] \u2192 index ' + rear + ' holds ' + peeked + ' (deque is unchanged)',
    code: 'c_read',
  ...
  stateAt[rear] = 'normal';
  pushStep({
    badge: fnLabel + ' complete \u2014 rear element is ' + peeked + ' \u2014 deque size still ' + size() + ' / ' + capacity,
    code: 'c_peekrear',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-peek"></span>rear cell being viewed</span>
        </div>
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                ...
                <tr><td>Non-empty deque</td><td>O(1)</td><td>direct read of <code>arr[rear]</code> &mdash; no traversal, no pointer movement, no write</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used; the deque is left completely unchanged</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>peekRear</code> is purely a read operation. Unlike <code>deleteRear</code>, it never
              touches <code>front</code>, <code>rear</code>, or the contents of the array &mdash; it only reports what is
              stored at <code>arr[rear]</code>. Calling it any number of times in a row always returns the same value and
              never changes the deque's size.
            </p>
            ...
            <h3>Limitations</h3>
            <p>peekRear only ever reports the rear element; it cannot be used to inspect elements deeper inside the deque
              without repeatedly removing and reinserting them, which would be inefficient and would change the deque's
              order.</p>
            ...
            <p>A <b>Simple Queue</b> and a <b>Circular Queue</b> both only expose the front element for peeking, since
              they only ever remove from the front &mdash; neither has a <code>peekRear</code>. A <b>Deque
              (Double-Ended Queue)</b> additionally exposes the rear element for peeking, reflecting the fact that it
              supports operations at both ends.</p>
            <h3>Key Observations</h3>
            <p><code>peekRear</code> and <code>deleteRear</code> share the same underflow check and read the exact same
              value from <code>arr[rear]</code> &mdash; the only difference is that peek stops right there, while delete
              goes on to clear the slot and move <code>rear</code>. This makes peek a safe, side-effect-free way to
              inspect the deque.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty deque:</b> peekRear must detect <code>front == -1</code> and report that the deque is empty
              instead of reading an invalid index. <b>Single-element deque:</b> front and rear point to the same index,
              so peekFront and peekRear would return the same value. <b>Multiple-element deque:</b> peekRear always
              reports the element at <code>rear</code>, regardless of how many elements exist or whether the deque has
              wrapped around.</p>
          </div>
        </div>
      </div>
