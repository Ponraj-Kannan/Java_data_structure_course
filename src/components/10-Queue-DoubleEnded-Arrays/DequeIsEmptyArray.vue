...
const CODES = {
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_isempty', 'int isEmpty(struct Deque *dq) {'],
    ['c_check', '    return dq->front == -1;'],
    ['', '}'],
  ],
  cpp: [
    ['', 'class Deque {'],
...
function buildSteps(capacity, frontIdx, values) {
  ...
  pushStep({
    badge: result
      ...
      : 'front != -1 \u2192 Deque is Not Empty (front points to a valid index)',
    code: 'c_check',
...
const stepsData = reactive(buildSteps(5, 0, [10, 20, 30]));
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip">isEmpty = <b :class="s.result === null ? 'll-c-purple' : (s.result ? 'll-c-green' : 'll-c-red')">{{ s.result === null ? '\u2014' : s.result }}</b></div>
            </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Any deque state</td><td>O(1)</td><td>a single comparison of <code>front</code> against -1, regardless of capacity or size</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used; the deque is left completely unchanged</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>isEmpty</code> never inspects the array contents or <code>rear</code> at all
              &mdash; a single sentinel value, <code>front == -1</code>, is enough to distinguish an empty deque from a
              non-empty one. This is only reliable because every operation that empties the deque (deleteFront or
              deleteRear removing the last element) explicitly resets <b>both</b> <code>front</code> and <code>rear</code>
              to -1, rather than leaving a stale index behind.
            </p>
            <h3>Advantages of Deque</h3>
            <p>Checking emptiness in O(1) lets calling code safely guard every other operation &mdash; deleteFront,
              deleteRear, peekFront, and peekRear can all call <code>isEmpty</code> first to avoid reading or removing
              from invalid indices, without paying any extra cost.</p>
            <h3>Limitations</h3>
            <p><code>isEmpty</code> only tells you whether the deque has zero elements; it says nothing about how close
              the deque is to being full. That is the job of the separate <code>isFull</code> check.</p>
            <h3>Difference between Queue, Circular Queue, and Deque</h3>
            <p>A <b>Simple Queue</b>, a <b>Circular Queue</b>, and a <b>Deque</b> all use the exact same emptiness check
              &mdash; <code>front == -1</code> &mdash; since all three represent "no elements" the same way. The
              difference between them only shows up in how <code>front</code> and <code>rear</code> move during
              insertion and removal, not in how emptiness is detected.</p>
            <h3>Key Observations</h3>
            <p><code>front == -1</code> and <code>rear == -1</code> always go together: a deque is never in a state
              where one is -1 and the other is not. This invariant is what makes checking only <code>front</code>
              sufficient. The size formula <code>((rear - front + capacity) % capacity) + 1</code> would actually be
              wrong for an empty deque if evaluated directly, which is exactly why <code>isEmpty</code> (or an explicit
              <code>front == -1</code> check inside the size calculation) must be handled as a special case.</p>
            <h3>Edge Cases</h3>
            <p><b>Freshly created deque:</b> <code>front</code> and <code>rear</code> both start at -1, so isEmpty
              correctly reports true before any insertion happens. <b>After the last element is removed:</b>
              deleteFront/deleteRear reset both pointers to -1, so isEmpty correctly reports true again. <b>Single-element
              deque:</b> <code>front == rear</code> and both are valid indices (not -1), so isEmpty correctly reports
              false. <b>Full deque:</b> isEmpty still simply checks <code>front == -1</code>, which is false, regardless
              of how full the deque is.</p>
          </div>
        </div>
      </div>
    </div>
