...
const CODES = {
  java: [
    ...
    ['c_ctor', '        rear = -1;'],
    ['c_ctor', '    }'], ['', ''],
    ['c_deleterear', '    int deleteRear() {'],
    ['c_underflow', '        if (front == -1) {'],
    ['c_underflow', '            System.out.println("Deque Underflow");'],
    ['c_underflow', '            return -1;'],
    ['c_underflow', '        }'],
    ['c_capture', '        int value = arr[rear];'],
    ['c_rearupdate', '        if (front == rear) {'],
    ['c_rearupdate', '            front = -1;'],
    ['c_rearupdate', '            rear = -1;'],
    ['c_rearupdate', '        } else {'],
    ['c_rearupdate', '            rear = (rear - 1 + capacity) % capacity;'],
    ['c_rearupdate', '        }'],
  ...
  c: [
    ...
    ['c_fields', '    int rear;'],
    ['', '};'], ['', ''],
    ['c_deleterear', 'int deleteRear(struct Deque *dq) {'],
    ['c_underflow', '    if (dq->front == -1) {'],
    ['c_underflow', '        printf("Deque Underflow\\n");'],
    ['c_underflow', '        return -1;'],
    ['c_underflow', '    }'],
    ['c_capture', '    int value = dq->arr[dq->rear];'],
    ['c_rearupdate', '    if (dq->front == dq->rear) {'],
    ['c_rearupdate', '        dq->front = -1;'],
    ['c_rearupdate', '        dq->rear = -1;'],
    ['c_rearupdate', '    } else {'],
    ['c_rearupdate', '        dq->rear = (dq->rear - 1 + dq->capacity) % dq->capacity;'],
    ['c_rearupdate', '    }'],
    ['c_return', '    return value;'],
    ['', '}'],
  ],
  ...
  python: [
    ...
    ['c_ctor', '        self.front = -1'],
    ['c_ctor', '        self.rear = -1'], ['', ''],
    ['c_deleterear', '    def delete_rear(self):'],
    ['c_underflow', '        if self.front == -1:'],
    ['c_underflow', '            print("Deque Underflow")'],
    ['c_underflow', '            return -1'],
    ['c_capture', '        value = self.arr[self.rear]'],
    ['c_rearupdate', '        if self.front == self.rear:'],
    ['c_rearupdate', '            self.front = -1'],
    ['c_rearupdate', '            self.rear = -1'],
    ['c_rearupdate', '        else:'],
    ['c_rearupdate', '            self.rear = (self.rear - 1 + self.capacity) % self.capacity'],
    ['c_return', '        return value'],
  ],
  javascript: [
    ['', 'class Deque {'],
    ...
    ['c_ctor', '    this.rear = -1;'],
    ['c_ctor', '  }'], ['', ''],
    ['c_deleterear', '  deleteRear() {'],
    ['c_underflow', '    if (this.front === -1) {'],
    ['c_underflow', '      console.log("Deque Underflow");'],
    ['c_underflow', '      return -1;'],
    ['c_underflow', '    }'],
    ['c_capture', '    const value = this.arr[this.rear];'],
    ['c_rearupdate', '    if (this.front === this.rear) {'],
    ['c_rearupdate', '      this.front = -1;'],
    ['c_rearupdate', '      this.rear = -1;'],
    ['c_rearupdate', '    } else {'],
    ['c_rearupdate', '      this.rear = (this.rear - 1 + this.capacity) % this.capacity;'],
    ['c_rearupdate', '    }'],
    ['c_return', '    return value;'],
    ['', '  }'], ['', '}'],
  ],
...
//   'del'    - the rear slot currently being removed (red pulse), before
//              it is cleared
function buildSteps(capacity, frontIdx, values) {
  const steps = [];
  const phaseStarts = { deleteRear: 0 };
  ...
  if (front === -1) {
    pushStep({
      badge: 'front == -1 \u2192 Deque Underflow, nothing to remove',
  ...
  // --- value = arr[rear]: capture the value before it is removed.
  const targetIdx = rear;
  removed = valAt[targetIdx];
  stateAt[targetIdx] = 'del';
  pushStep({
    badge: 'value = arr[rear] \u2192 capturing ' + removed + ' from index ' + targetIdx + ' before removing it',
    code: 'c_capture',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(removed), true]])],
  });

  // --- clear the slot and move rear (or reset both pointers if this was
  // the last remaining element).
  delete valAt[targetIdx];
  delete stateAt[targetIdx];

  if (front === rear) {
    front = -1;
    rear = -1;
    pushStep({
      badge: 'front == rear \u2192 only one element left, front = rear = -1 \u2014 deque is now empty',
      code: 'c_rearupdate',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(removed), true]])],
    });
  } else {
    const oldRear = rear;
    rear = (rear - 1 + capacity) % capacity;
    const wrapped = rear > oldRear;
    pushStep({
      badge: wrapped
        ? 'rear = (rear - 1 + capacity) % capacity \u2192 wraps around from index ' + oldRear + ' to index ' + rear
        : 'rear = (rear - 1 + capacity) % capacity \u2192 rear moves to index ' + rear,
      code: 'c_rearupdate',
      vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]]), frame(fnLabel, [['value', String(removed), true]])],
    });
  }
  ...
  pushStep({
    badge: fnLabel + ' complete \u2014 removed ' + removed + ' \u2014 deque size now ' + size() + ' / ' + capacity,
    code: 'c_return',
    vars: [frame('main()', [['capacity', String(capacity), true], ['front', String(front), true], ['rear', String(rear), true]])],
  });
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div class="ll-ptr-chip">removed = <b class="ll-c-red">{{ s.removed === null ? '\u2014' : s.removed }}</b></div>
            </div>

            <div class="ll-arr-track">
              <template v-for="cell in s.cells" :key="cell.idx">
                <div class="ll-arr-cell-wrap">
                  <div class="ll-who" :class="{ 'll-c-red': cell.state === 'del' }">{{ cellLabel(cell) }}</div>
                  <div
                    class="ll-arr-box"
                    :class="{
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-del"></span>cell being removed</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty deque (underflow)</td><td>O(1)</td><td>a single comparison detects it, nothing is read or removed</td></tr>
                <tr><td>Single-element deque</td><td>O(1)</td><td>the value is captured, then <code>front</code> and <code>rear</code> both reset to -1</td></tr>
                <tr><td>Normal deleteRear</td><td>O(1)</td><td>direct read from <code>arr[rear]</code> followed by one modulo decrement &mdash; no traversal or shifting</td></tr>
                <tr><td>Wrap-around deletion</td><td>O(1)</td><td>the modulo operation folds index -1 back to <code>capacity - 1</code> in constant time</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used &mdash; the slot is simply marked free</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <code>deleteRear</code> never shifts any other elements to fill the gap. It only reads
              <code>arr[rear]</code>, then decrements <code>rear</code> with <code>(rear - 1 + capacity) % capacity</code>
              so the pointer wraps from index 0 back to <code>capacity - 1</code> instead of going negative. The special
              case <code>front == rear</code> means only one element remained, so after removing it both pointers are
              reset to <b>-1</b> to correctly represent an empty deque &mdash; without this reset the deque would
              incorrectly believe a stale slot is still occupied.
            </p>
            <h3>Advantages of Deque</h3>
            <p>Deletion is O(1) from <b>both</b> ends, unlike a simple array where removing the last logical element can
              still be cheap, but removing from the front requires shifting every remaining element left. The fixed-size
              circular array is cache-friendly and needs no per-element pointer overhead, unlike a linked-list based
              deque.</p>
            <h3>Limitations</h3>
            <p>Because the array is fixed-size, repeatedly inserting and deleting only frees up circular slots &mdash; it
              never grows the underlying storage. If <code>deleteFront</code> or <code>deleteRear</code> is called on an
              empty deque, it must report <b>Deque Underflow</b> rather than reading garbage data.</p>
            <h3>Difference between Queue, Circular Queue, and Deque</h3>
            <p>A <b>Simple Queue</b> does not support removal from the rear at all &mdash; only insertion. A
              <b>Circular Queue</b> wraps its pointers with modulo arithmetic but still restricts removal to the front
              only. A <b>Deque (Double-Ended Queue)</b> goes further: using the same circular indexing, it allows removal
              <b>and</b> insertion at <b>both</b> the front and the rear, making it strictly more flexible than either a
              simple or circular queue.</p>
            <h3>Key Observations</h3>
            <p>Deleting the last remaining element (<code>front == rear</code>) is a special case that resets both
              <code>front</code> and <code>rear</code> to <b>-1</b>, rather than just decrementing <code>rear</code>
              &mdash; this is what correctly signals an empty deque to future operations. Every other deleteRear only
              moves <code>rear</code> backward circularly, leaving <code>front</code> untouched. The current size is
              always <code>((rear - front + capacity) % capacity) + 1</code> (or 0 when empty), which correctly accounts
              for wrap-around.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty deque:</b> deleteRear must detect <code>front == -1</code> and report Deque Underflow without
              touching the array. <b>Single-element deque:</b> after capturing the value, both <code>front</code> and
              <code>rear</code> reset to -1. <b>Multiple-element deque:</b> only <code>rear</code> moves backward
              circularly; <code>front</code> and every other element stay exactly where they are. <b>Wrap-around
              deletion:</b> when <code>rear</code> is at index 0, the next deleteRear wraps it to
              <code>capacity - 1</code> instead of going negative.</p>
          </div>
        </div>
      </div>
    </div>
