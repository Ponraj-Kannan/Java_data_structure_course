...
const CODES = {
  ...
  c: [
    ...
    ['c_sethead', '        head = newNode;'],
    ['c_empty', '        return;'], ['c_empty', '    }'],
    ['c_travstart', '    struct Node *last = head;'],
    ['c_travloop', '    while (last->next != head) {'],
    ['c_travmove', '        last = last->next;'],
    ['c_travloop', '    }'],
    ['c_setnext', '    newNode->next = head;'],
    ...
    ['', '}'], ['', ''],
    ['c_display', 'void display() {'], ['c_display', '    if (head == NULL) return;'],
    ['c_display', '    struct Node *temp = head;'],
    ['c_display', '    do {'], ['c_display', '        printf("%d ", temp->data);'],
    ['c_display', '        temp = temp->next;'],
    ['c_display', '    } while (temp != head);'], ['', '}'],
  ],
  cpp: [
    ...
    ['c_display', 'void display() {'], ['c_display', '    if (head == NULL) return;'],
    ['c_display', '    Node *temp = head;'],
    ['c_display', '    do {'], ['c_display', '        cout << temp->data << " ";'],
    ['c_display', '        temp = temp->next;'],
    ['c_display', '    } while (temp != head);'], ['', '}'],
  ],
  python: [
    ...
    ['', 'class CircularLinkedList:'], ['c_sethead', '    def __init__(self):'],
    ['c_sethead', '        self.head = None'], ['', ''],
    ['c_insert', '    def insert_at_end(self, data):'],
    ['c_nodeNew', '        new_node = Node(data)'],
    ['c_empty', '        if self.head is None:'],
    ['c_selfloop', '            new_node.next = new_node'],
    ['c_sethead', '            self.head = new_node'],
    ['c_empty', '            return'],
    ['c_travstart', '        last = self.head'],
    ['c_travloop', '        while last.next != self.head:'],
    ['c_travmove', '            last = last.next'],
    ['c_setnext', '        new_node.next = self.head'],
    ['c_setlast', '        last.next = new_node'], ['', ''],
    ['c_display', '    def display(self):'], ['c_display', '        if self.head is None: return'],
    ['c_display', '        temp = self.head'],
    ['c_display', '        while True:'],
...
function buildSteps(values) {
  ...
  steps.push({
    nodes: [], headAddr: null, badge: 'Read input \u2192 size = ' + values.length, code: 'c_insert',
    vars: [frame('main()', [['size', '' + values.length], ['head', 'null', true]])],
  });

  values.forEach((v, li) => {
    const newAddr = ADDR(addrCounter++);
    const mainFrame = () => frame('main()', [['i', '' + li], ['value', '' + v], ['head', fmt(headAddr), true]]);

    steps.push({
      nodes: [...nodes], headAddr, badge: 'read value ' + v + ' \u2192 create newNode @' + newAddr, code: 'c_nodeNew',
      neuAddr: newAddr, pendVal: v,
      vars: [mainFrame(), frame('insertAtEnd(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true]])],
    });

    if (nodes.length === 0) {
      steps.push({
        nodes: [...nodes], headAddr, badge: 'list is empty \u2192 newNode.next = newNode (self loop)', code: 'c_selfloop',
        neuAddr: newAddr, pendVal: v, pendNext: newAddr, selfLoop: true,
        vars: [mainFrame(), frame('insertAtEnd(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true]])],
      });
      nodes = [{ val: v, addr: newAddr }];
      headAddr = newAddr;
      steps.push({
        nodes: [...nodes], headAddr, badge: 'head = @' + newAddr + ' \u2192 single circular node', code: 'c_sethead',
        neuAddr: newAddr,
        vars: [mainFrame(), frame('insertAtEnd(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true]])],
      });
    } else {
      ...
      steps.push({
        nodes: [...nodes], headAddr, badge: 'last = head \u2192 start search for tail node', code: 'c_travstart',
        neuAddr: newAddr, pendVal: v, curAddr: nodes[0].addr,
        vars: [mainFrame(), frame('insertAtEnd(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true], ['last', fmt(nodes[0].addr), true]])],
      });
      for (let t = 0; t < lastIdx; t++) {
        steps.push({
          nodes: [...nodes], headAddr, badge: 'last.next != head \u2192 last = last.next', code: 'c_travmove',
          neuAddr: newAddr, pendVal: v, curAddr: nodes[t + 1].addr,
          vars: [mainFrame(), frame('insertAtEnd(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true], ['last', fmt(nodes[t + 1].addr), true]])],
        });
      }
      steps.push({
        nodes: [...nodes], headAddr, badge: 'last.next == head \u2192 tail found @' + nodes[lastIdx].addr, code: 'c_travloop',
        neuAddr: newAddr, pendVal: v, curAddr: nodes[lastIdx].addr, tailFound: true,
        vars: [mainFrame(), frame('insertAtEnd(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true], ['last', fmt(nodes[lastIdx].addr), true]])],
      });
      steps.push({
        nodes: [...nodes], headAddr, badge: 'newNode.next = head = ' + fmt(headAddr) + ' \u2192 link new node to head', code: 'c_setnext',
        neuAddr: newAddr, pendVal: v, pendNext: headAddr, curAddr: nodes[lastIdx].addr,
        vars: [mainFrame(), frame('insertAtEnd(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true], ['last', fmt(nodes[lastIdx].addr), true]])],
      });
      steps.push({
        nodes: [...nodes], headAddr, badge: 'last.next = newNode \u2192 tail now points to @' + newAddr + ' (new tail)', code: 'c_setlast',
        neuAddr: newAddr, pendVal: v, pendNext: headAddr, curAddr: nodes[lastIdx].addr, tailRelink: newAddr,
        vars: [mainFrame(), frame('insertAtEnd(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true], ['last', fmt(nodes[lastIdx].addr), true]])],
      });
      nodes = [...nodes, { val: v, addr: newAddr }];
      steps.push({
        nodes: [...nodes], headAddr, badge: 'head unchanged = ' + fmt(headAddr) + ' \u2192 @' + newAddr + ' is now the new tail!', code: 'c_setlast',
        neuAddr: newAddr,
        vars: [mainFrame(), frame('insertAtEnd(' + v + ')', [['data', '' + v], ['newNode', fmt(newAddr), true]])],
      });
    }
  ...
  steps.push({
    ...
    vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('display()', [['temp', fmt(headAddr), true]])],
  });
  for (let t = 0; t < nodes.length; t++) {
    printed = [...printed, nodes[t].val];
    steps.push({
      nodes: [...nodes], headAddr, tempAddr: nodes[t].addr, curAddr: nodes[t].addr,
      badge: 'display(): print ' + nodes[t].val + ' \u2192 temp = temp.next', code: 'c_display', out: [...printed],
      vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('display()', [['temp', fmt(nodes[t].addr), true]])],
    });
  }
  steps.push({
    nodes: [...nodes], headAddr, tempAddr: headAddr, badge: 'temp == head again \u2192 stop (circular, never null)',
...
function resetAll() { playing.value = false; si.value = 0; }
function nodeLabel(n) {
  const labels = [];
  if (n.addr === s.value.headAddr && !s.value.pendVal) labels.push('head \u2192');
  if (n.addr === s.value.curAddr) labels.push('last \u2192');
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.tempAddr !== undefined" class="ll-ptr-chip">temp = <b class="ll-c-orange">{{ fmt(s.tempAddr) }}</b></div>
              <div v-if="s.curAddr !== undefined" class="ll-ptr-chip">last = <b class="ll-c-purple">{{ fmt(s.curAddr) }}</b></div>
            ...
            <div class="ll-stage">
              <template v-for="(n, idx) in s.nodes" :key="n.addr">
                <div class="ll-nwrap">
                  ...
                  <div class="ll-node">
                    <div class="ll-box" :class="{ 'll-box-new': n.addr === s.neuAddr, 'll-box-cur': n.addr === s.curAddr || n.addr === s.tempAddr }">
              ...
              <div v-if="s.pendVal !== undefined" class="ll-nwrap">
                ...
                <div class="ll-node">
                  <div class="ll-box ll-box-new">
                    ...
                    <div class="ll-ptr"><small>next</small>{{ s.selfLoop ? '\u21BAself' : fmt(s.pendNext !== undefined ? s.pendNext : null) }}</div>
                  </div>
                  <span class="ll-arrow" v-if="!s.selfLoop && s.pendNext === undefined"></span>
                  <span class="ll-arrow" v-else-if="!s.selfLoop">&rarr;</span>
                </div>
              </div>

              <div v-if="s.nodes.length" class="ll-wraparrow" :class="{ 'll-wraparrow-hot': s.tailRelink }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>current / last pointer</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Best / Average / Worst</td><td>O(n)</td><td>must traverse to find the tail node to append after it</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>only one new node is allocated, no extra structures</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Tip: if the list keeps a <code>tail</code> pointer (common optimization), insertion
              at the end drops to <b>O(1)</b> since traversal is no longer needed.
            </p>
          </div>
        </div>
      </div>
...
.ll-wraparrow-hot { color: #fff; background: var(--purple); border-style: solid; animation: none; }
.ll-outbox { margin: 4px 16px 8px; background: var(--surface2); border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 7px 14px; font-family: 'Consolas', monospace; font-size: 15px; min-height: 36px; display: flex; align-items: center; gap: 4px; width: calc(100% - 32px); }
.ll-outlabel { color: var(--muted); font-size: 11px; margin-right: 6px; font-weight: 500; }
.ll-outv { color: var(--text); font-weight: 700; margin-right: 8px; animation: ll-pop .3s ease; }
