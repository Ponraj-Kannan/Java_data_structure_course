...
const CODES = {
  java: [
    ...
    ['c_build', '    }'],
    ['', ''],
    ['c_delete', '    void deleteAtBeginning() {'],
    ['c_check', '        if (head == null) {'],
    ['c_check', '            System.out.println("Empty list");'],
    ['c_check', '            return;'],
    ['c_check', '        }'],
    ['c_temp', '        Node temp = head;        // save old head'],
    ['c_movehead', '        head = head.next;         // advance head'],
    ['c_free', '        temp = null;              // free old head'],
    ['c_delete', '    }'],
    ['', ''],
    ...
    ['c_delete', '        list.deleteAtBeginning();'],
  ...
  c: [
    ...
    ['', ''],
    ['c_delete', 'void deleteAtBeginning() {'],
    ['c_check', '    if (head == NULL) { printf("Empty\\n"); return; }'],
    ['c_temp', '    struct Node *temp = head;'],
  ...
  cpp: [
    ...
    ['', ''],
    ['c_delete', 'void deleteAtBeginning() {'],
    ['c_check', '    if (!head) { cout << "Empty" << endl; return; }'],
    ['c_temp', '    Node *temp = head;'],
    ['c_movehead', '    head = head->next;'],
    ['c_free', '    delete temp;'],
    ['c_delete', '}'],
    ['', ''],
  ...
  python: [
    ...
    ['c_build', '            t.next = n'],
    ['', ''],
    ['c_delete', '    def delete_at_beginning(self):'],
    ['c_check', '        if self.head is None:'],
    ['c_check', '            print("Empty list")'],
    ['c_check', '            return'],
    ['c_temp', '        temp = self.head      # save old head'],
    ['c_movehead', '        self.head = self.head.next  # advance'],
    ['c_free', '        temp = None           # free old head'],
    ['', ''],
    ...
    ['c_delete', 'lst.delete_at_beginning()'],
...
function buildSteps(values) {
  ...
  steps.push({
    nodes: [...nodes], headAddr, badge: 'deleteAtBeginning(): check if empty', code: 'c_check',
    vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtBeginning()', [['head', fmt(headAddr), true]])],
  });
  if (nodes.length > 0) {
    const delAddr = headAddr;
    steps.push({
      nodes: [...nodes], headAddr, tempAddr: delAddr, delAddr, badge: 'temp = head = @' + delAddr + ' (save old head)', code: 'c_temp',
      vars: [frame('deleteAtBeginning()', [['temp', fmt(delAddr), true], ['head', fmt(headAddr), true]])],
    });
    const newHead = nodes.length > 1 ? nodes[1].addr : null;
    steps.push({
      nodes: [...nodes], headAddr: newHead, tempAddr: delAddr, delAddr, badge: 'head = head.next = ' + fmt(newHead), code: 'c_movehead',
      vars: [frame('deleteAtBeginning()', [['temp', fmt(delAddr), true], ['head', fmt(newHead), true]])],
    });
    nodes.shift();
    headAddr = newHead;
    steps.push({
      nodes: [...nodes], headAddr, badge: 'free(temp) → node deleted', code: 'c_free',
      vars: [frame('deleteAtBeginning()', [['temp', 'null', true], ['head', fmt(headAddr), true]])],
    });
  }
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>temp (to delete)</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-del"></span>freed node</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>deleteAtBeginning() &mdash; empty list</td><td>O(1)</td><td>O(1)</td></tr>
                <tr><td>deleteAtBeginning() &mdash; non-empty list</td><td>O(1)</td><td>O(1)</td></tr>
                <tr><td>display()</td><td>O(n)</td><td>O(1) auxiliary</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              <b>Why deletion is O(1):</b> the node to remove is always <code>head</code> itself, so
              deleting it only needs <code>head = head.next</code> &mdash; no traversal is required,
              regardless of how long the list is.
            </p>
            <h3>Space</h3>
            <p>
              <code>deleteAtBeginning</code> uses a single temporary pointer to hold the old head
              before it is discarded (O(1) auxiliary space), and no new memory is allocated. The list
              overall still occupies O(n) space for its remaining <code>n - 1</code> nodes.
            </p>
          </div>
        </div>
      </div>
...
/* DELETE-SPECIFIC (added for this component) */
.ll-box-del {
  border-color: #b91c1c;
  background: #b91c1c;
  opacity: .7;
