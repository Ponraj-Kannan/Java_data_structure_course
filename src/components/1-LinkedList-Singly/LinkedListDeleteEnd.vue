...
const CODES = {
  java: [
    ...
    ['c_build', '    }'],
    ['', ''],
    ['c_delete', '    void deleteAtEnd() {'],
    ['c_check', '        if (head == null) {'],
    ['c_check', '            System.out.println("Empty");'],
    ['c_check', '            return;'],
    ['c_check', '        }'],
    ['c_single', '        if (head.next == null) {   // single node'],
    ['c_single', '            head = null;'],
    ['c_single', '            return;'],
    ['c_single', '        }'],
    ['c_walk', '        Node temp = head;'],
    ['c_walk', '        while (temp.next.next != null)'],
    ['c_walk', '            temp = temp.next;'],
    ['c_unlink', '        temp.next = null;   // unlink last node'],
    ['c_delete', '    }'],
    ['', ''],
  ...
  c: [
    ...
    ['c_single', '    if (!head->next) { free(head); head = NULL; return; }'],
    ['c_walk', '    struct Node *temp = head;'],
    ['c_walk', '    while (temp->next->next) temp = temp->next;'],
    ['c_unlink', '    free(temp->next);'],
    ['c_unlink', '    temp->next = NULL;'],
    ['c_delete', '}'],
    ['', ''],
    ['c_display', 'void display() {'],
  ...
  cpp: [
    ...
    ['c_single', '    if (!head->next) { delete head; head = NULL; return; }'],
    ['c_walk', '    Node *temp = head;'],
    ['c_walk', '    while (temp->next->next) temp = temp->next;'],
    ['c_unlink', '    delete temp->next;'],
    ['c_unlink', '    temp->next = NULL;'],
    ['c_delete', '}'],
    ['', ''],
    ['c_display', 'void display() {'],
  ...
  python: [
    ...
    ['c_build', '            t.next = n'],
    ['', ''],
    ['c_delete', '    def delete_at_end(self):'],
    ...
    ['c_walk', '        while temp.next.next:'],
    ['c_walk', '            temp = temp.next'],
    ['c_unlink', '        temp.next = None'],
    ['', ''],
...
function buildSteps(values) {
  ...
  steps.push({
    nodes: [...nodes], headAddr, badge: 'deleteAtEnd(): check if empty', code: 'c_check',
    vars: [frame('main()', [['head', fmt(headAddr), true]]), frame('deleteAtEnd()', [['head', fmt(headAddr), true]])],
  });
  if (nodes.length === 1) {
    const da = nodes[0].addr;
    steps.push({
      nodes: [...nodes], headAddr, delAddr: da, badge: 'single node → head=null, free @' + da, code: 'c_single',
      vars: [frame('deleteAtEnd()', [['head', 'null', true]])],
    });
    nodes = [];
    headAddr = null;
    ...
    for (let t = 0; t < nodes.length - 2; t++) {
      steps.push({
        nodes: [...nodes], headAddr, tempAddr: nodes[t].addr, curAddr: nodes[t].addr, delAddr: nodes[nodes.length - 1].addr,
        badge: 'walk: temp=@' + nodes[t].addr + ', next.next=' + (t + 2 < nodes.length ? '@' + nodes[t + 2].addr : 'null') + ' → continue', code: 'c_walk',
        vars: [frame('deleteAtEnd()', [['temp', fmt(nodes[t].addr), true]])],
      });
    }
    const secLast = nodes[nodes.length - 2];
    const lastNode = nodes[nodes.length - 1];
    steps.push({
      nodes: [...nodes], headAddr, tempAddr: secLast.addr, curAddr: secLast.addr, delAddr: lastNode.addr,
      badge: 'temp=@' + secLast.addr + ': temp.next.next=null → stop', code: 'c_walk',
      vars: [frame('deleteAtEnd()', [['temp', fmt(secLast.addr), true]])],
    });
    steps.push({
      nodes: [...nodes], headAddr, tempAddr: secLast.addr, curAddr: secLast.addr, delAddr: lastNode.addr, cutAddr: lastNode.addr,
      badge: 'unlink: temp.next=null → @' + lastNode.addr + ' removed', code: 'c_unlink',
      vars: [frame('deleteAtEnd()', [['temp', fmt(secLast.addr), true]])],
    });
    nodes.pop();
    steps.push({
      nodes: [...nodes], headAddr, badge: 'last node freed → done', code: 'c_unlink',
      vars: [frame('deleteAtEnd()', [['temp', fmt(secLast.addr), true]])],
    });
  }
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>deleteAtEnd() &mdash; empty list</td><td>O(1)</td><td>O(1)</td></tr>
                <tr><td>deleteAtEnd() &mdash; single node</td><td>O(1)</td><td>O(1)</td></tr>
                <tr><td>deleteAtEnd() &mdash; multiple nodes</td><td>O(n)</td><td>O(1) auxiliary</td></tr>
                <tr><td>display()</td><td>O(n)</td><td>O(1) auxiliary</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              <b>Why deletion is O(n):</b> since there is no <code>tail</code> pointer or previous-node
              link, the only way to find the <b>second-to-last</b> node is to walk from
              <code>head</code> until <code>temp.next.next == null</code>, so removing the last node
              still costs time proportional to the list's length &mdash; unlike deleting from the
              beginning, which is O(1).
            </p>
            <h3>Space</h3>
            <p>
              <code>deleteAtEnd</code> uses only a single traversal pointer (O(1) auxiliary space)
              and frees exactly one node. The list overall still occupies O(n) space for its
              remaining <code>n - 1</code> nodes.
            </p>
          </div>
        </div>
      </div>
