        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>deleteAtPosition(pos) &mdash; empty list</td><td>O(1)</td><td>O(1)</td></tr>
                <tr><td>deleteAtPosition(pos) &mdash; pos = 1</td><td>O(1)</td><td>O(1)</td></tr>
                <tr><td>deleteAtPosition(pos) &mdash; pos &gt; 1</td><td>O(pos)</td><td>O(1) auxiliary</td></tr>
                <tr><td>display()</td><td>O(n)</td><td>O(1) auxiliary</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              <b>Why deletion is O(pos):</b> to delete the node at position <code>pos</code>, the code
              first walks from <code>head</code> to the node just before it
              (<code>for i = 1 to pos - 2</code>) so it can bypass the target with
              <code>temp.next = del.next</code> &mdash; the cost grows with how far into the list the
              position is, up to O(n) in the worst case (deleting near the end).
            </p>
            <h3>Space</h3>
            <p>
              Only two pointers are used &mdash; <code>temp</code> for the walk and <code>del</code>
              for the node being removed &mdash; so auxiliary space is O(1) regardless of
              <code>pos</code>. The list overall still occupies O(n) space for its remaining
              <code>n - 1</code> nodes.
            </p>
          </div>
        </div>
