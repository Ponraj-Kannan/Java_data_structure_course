...
// isEmpty() is the simplest possible query on the structure: it never allocates,
// never traverses past head, and never mutates head or size. It only exists to
// answer a single yes/no question, which every other operation (peek, insert,
// extractMax) relies on internally before doing its own work.
function buildSteps(elements) {
  ...
  // used by the insert visualizer, so isEmpty() always starts from a correctly
  ...
  const isEmptyFrame = (extra) => frame('isEmpty()', [...(extra || [])]);
  ...
  pushStep({
    badge: 'Initial list \u2014 size ' + size + ' \u2014 isEmpty() called',
    code: 'c_isempty',
    vars: [mainFrame()],
  });
  ...
  if (isEmpty) {
    pushStep({
      badge: 'head == null \u2192 the queue has no nodes',
      code: 'c_check',
      vars: [mainFrame(), isEmptyFrame()],
    });

    pushStep({
      badge: 'isEmpty() returns true \u2014 the queue IS empty',
      code: 'c_check',
      resultVal: true,
      vars: [mainFrame()],
    });
    ...
    pushStep({
      badge: 'head != null \u2192 the queue has at least one node',
      code: 'c_check',
      vars: [mainFrame(), isEmptyFrame()],
    });

    pushStep({
      badge: 'isEmpty() returns false \u2014 the queue is NOT empty',
      code: 'c_returnfalse',
      resultVal: false,
      vars: [mainFrame()],
    });
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.resultVal !== undefined" class="ll-ptr-chip">isEmpty() = <b :class="s.resultVal ? 'll-c-coral' : 'll-c-green'">{{ s.resultVal }}</b></div>
            </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm</h3>
            <p><code>isEmpty()</code> is the simplest read possible on the structure: it makes a single comparison &mdash; is <code>head</code> <code>null</code>? &mdash; and returns the answer immediately. It never traverses the chain, never reads <code>size</code>, and never touches any node, so it works identically no matter how many elements happen to be queued.</p>
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                ...
                <tr><td>Non-empty queue</td><td>O(1)</td><td>the same single null check &mdash; no traversal either way</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no new memory is allocated and no existing node is touched</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: every other operation &mdash; <code>peek()</code>, <code>extractMax()</code>, even
              <code>insert()</code> in some implementations &mdash; relies on the same <code>head == null</code>
              check internally before doing its own work. <code>isEmpty()</code> just exposes that check directly.
            </p>
            <h3>Advantages</h3>
            <p>Constant-time regardless of queue size, with zero side effects &mdash; it's always safe to call before <code>peek()</code> or <code>extractMax()</code> to avoid operating on an empty queue.</p>
            <h3>Limitations</h3>
            <p><code>isEmpty()</code> only reports whether the queue has zero elements right now &mdash; it says nothing about what those elements are, how many there are beyond "zero or more than zero", or whether the queue is about to receive an <code>insert()</code>. In a multi-threaded setting the answer could be stale by the time the caller acts on it.</p>
            <h3>Queue vs Circular Queue vs Deque vs Priority Queue</h3>
            <p>A <b>Queue</b>, <b>Circular Queue</b>, and <b>Deque</b> typically check <code>isEmpty()</code> the same way &mdash; either <code>front == -1</code>/<code>size == 0</code> for an array-based structure, or <code>head == null</code> for a linked one. A linked-list <b>Priority Queue</b> uses the identical <code>head == null</code> check; priority ordering doesn't change what "empty" means, only what gets returned once the queue has at least one node.</p>
            <h3>Key Observations</h3>
            <p><code>isEmpty()</code> and <code>isFull()</code> are natural opposites in spirit but not in cost: <code>isEmpty()</code> reads a pointer that's already tracked, while a linked-list <code>isFull()</code> has to probe the memory allocator since there's no fixed capacity to compare against. Both are still O(1), just for different reasons.</p>
            <h3>Edge Cases</h3>
            <p><b>Freshly created queue:</b> <code>head</code> starts as <code>null</code>, so <code>isEmpty()</code> returns <code>true</code> before any <code>insert()</code> is ever called. <b>After the last node is removed:</b> once <code>extractMax()</code> empties the list, <code>head</code> is reset to <code>null</code> and <code>isEmpty()</code> correctly returns <code>true</code> again. <b>Single-node queue:</b> <code>isEmpty()</code> returns <code>false</code> &mdash; the check only cares whether <code>head</code> is <code>null</code>, not how many nodes follow it.</p>
          </div>
        </div>
      </div>
    </div>
