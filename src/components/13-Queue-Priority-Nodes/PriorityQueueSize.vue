...
// getSize() relies on `size` being maintained incrementally by insert() (size++) and
// extractMax() (size--), the same way `head` is kept pointing at the highest-priority
// node. Because that bookkeeping already happened as a side effect of those operations,
// getSize() itself never has to walk the chain to count nodes - it just hands back the
// counter that's already correct.
function buildSteps(elements) {
  const steps = [];
  const phaseStarts = { getSize: 0 };

  // The list is (re)built through the exact same priority-queue insertion logic
  // used by the insert visualizer, so getSize() always starts from a correctly
  ...
  const getSizeFrame = (extra) => frame('getSize()', [...(extra || [])]);
  ...
  pushStep({
    badge: 'Initial list \u2014 size ' + size + ' \u2014 getSize() called',
    code: 'c_getsize',
    vars: [mainFrame()],
  });

  pushStep({
    badge: 'size is already tracked \u2014 insert() and extractMax() keep it up to date, so no traversal is needed',
    code: 'c_return',
    vars: [mainFrame(), getSizeFrame()],
  });

  pushStep({
    badge: 'getSize() returns ' + size + ' \u2014 this matches the number of nodes currently in the queue',
    code: 'c_return',
    resultVal: size,
    vars: [mainFrame()],
  });
        ...
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.resultVal !== undefined" class="ll-ptr-chip">getSize() = <b class="ll-c-green">{{ s.resultVal }}</b></div>
            </div>
            ...
            <div class="ll-stage">
              ...
              <div v-if="!s.nodes.length" class="ll-empty-note">queue is empty &mdash; head = null, size = 0</div>
            </div>
          </div>
        </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm</h3>
            <p><code>getSize()</code> hands back the <code>size</code> counter directly &mdash; it doesn't walk the chain to count nodes. That's only correct because <code>insert()</code> increments <code>size</code> every time a node is added, and <code>extractMax()</code> decrements it every time one is removed, so the counter is always kept in sync with the actual number of nodes. <code>getSize()</code> is simply exposing that already-maintained bookkeeping.</p>
            <h3>Time &amp; Space Complexity</h3>
            <table class="ll-complexity-table">
              <thead><tr><th>Case</th><th>Time</th><th>Why</th></tr></thead>
              <tbody>
                <tr><td>Any queue state</td><td>O(1)</td><td>a single field read &mdash; identical whether the queue has 0 or 1,000,000 nodes</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no new memory is allocated and no existing node is touched</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Alternative design: if <code>size</code> weren't maintained incrementally, <code>getSize()</code>
              would have to traverse the whole chain and count nodes one by one, costing O(n) instead of O(1).
              Keeping a running counter trades a tiny bit of extra bookkeeping in <code>insert()</code> and
              <code>extractMax()</code> for a constant-time <code>getSize()</code>.
            </p>
            <h3>Advantages</h3>
            <p>Constant-time regardless of how many elements are queued, with zero side effects &mdash; safe to call as often as needed, e.g. before deciding whether to <code>insert()</code> more elements or to loop <code>extractMax()</code> a fixed number of times.</p>
            <h3>Limitations</h3>
            <p><code>getSize()</code> is only as reliable as the bookkeeping behind it &mdash; if any future operation ever adds or removes a node without updating <code>size</code> to match, <code>getSize()</code> will silently report a stale count. It also only tells you how many nodes exist, not anything about their values or priorities.</p>
            <h3>Queue vs Circular Queue vs Deque vs Priority Queue</h3>
            <p>An array-based <b>Queue</b> often derives its size from <code>rear - front + 1</code>, while a <b>Circular Queue</b> needs a dedicated counter (or a sacrificed slot) since front/rear indices alone can't distinguish "empty" from "full" once they wrap. A <b>Deque</b> follows whichever of those its backing structure uses. A linked-list <b>Priority Queue</b> &mdash; like this one &mdash; simply maintains its own counter, exactly the way this <code>getSize()</code> does, independent of node ordering.</p>
            <h3>Key Observations</h3>
            <p><code>getSize()</code> is a read of state that other operations maintain as a side effect, the same way <code>isEmpty()</code> is a read of <code>head</code> that <code>insert()</code> and <code>extractMax()</code> keep correct. Both are O(1) precisely because the more expensive work (updating the counter, keeping the list sorted) was already paid for elsewhere.</p>
            <h3>Edge Cases</h3>
            <p><b>Freshly created queue:</b> <code>size</code> starts at <code>0</code>, so <code>getSize()</code> returns <code>0</code> before any <code>insert()</code> is ever called. <b>After every node is removed:</b> once <code>extractMax()</code> empties the list, <code>size</code> returns to <code>0</code> along with <code>head</code> resetting to <code>null</code>. <b>Repeated calls:</b> since <code>getSize()</code> never mutates the queue, calling it any number of times in a row returns the same value.</p>
          </div>
        </div>
      </div>
    </div>
