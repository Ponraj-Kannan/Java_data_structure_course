...
const CODES = {
  java: [
    ['', 'class Node {'],
    ['c_nodeNew', '    int value;'],
    ['c_nodeNew', '    int priority;'],
    ['c_nodeNew', '    Node next;'], ['', ''],
    ['c_nodeNew', '    Node(int value, int priority) {'],
    ['c_nodeNew', '        this.value = value;'],
    ['c_nodeNew', '        this.priority = priority;'],
    ...
    ['c_nodeNew', '        Node newNode = new Node(value, priority);'],
    ['c_headcheck', '        if (head == null || priority > head.priority) {'],
    ['c_linkhead', '            newNode.next = head;'],
    ['c_linkhead', '            head = newNode;'],
    ['c_headcheck', '        } else {'],
    ['c_travinit', '            Node current = head;'],
    ['c_while', '            while (current.next != null && current.next.priority >= priority) {'],
    ['c_advance', '                current = current.next;'],
    ['', '            }'],
    ['c_linkmid', '            newNode.next = current.next;'],
    ['c_linkmid', '            current.next = newNode;'],
    ['c_headcheck', '        }'],
    ['c_size', '        size++;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ...
    ['c_nodeNew', '    int value;'],
    ['c_nodeNew', '    int priority;'],
    ['c_nodeNew', '    struct Node *next;'],
    ['', '};'], ['', ''],
    ['c_sethead', 'struct Node *head = NULL;'],
    ['c_sethead', 'int size = 0;'], ['', ''],
    ['c_insert', 'void insert(int value, int priority) {'],
    ['c_nodeNew', '    struct Node *newNode = malloc(sizeof(struct Node));'],
    ['c_nodeNew', '    newNode->value = value;'],
    ['c_nodeNew', '    newNode->priority = priority;'],
    ['c_nodeNew', '    newNode->next = NULL;'],
    ['c_headcheck', '    if (head == NULL || priority > head->priority) {'],
    ['c_linkhead', '        newNode->next = head;'],
    ['c_linkhead', '        head = newNode;'],
    ['c_headcheck', '    } else {'],
    ['c_travinit', '        struct Node *current = head;'],
    ['c_while', '        while (current->next != NULL && current->next->priority >= priority) {'],
    ['c_advance', '            current = current->next;'],
    ['', '        }'],
    ['c_linkmid', '        newNode->next = current->next;'],
  ...
  python: [
    ['', 'class Node:'],
    ['c_nodeNew', '    def __init__(self, value, priority):'],
    ['c_nodeNew', '        self.value = value'],
    ['c_nodeNew', '        self.priority = priority'],
    ...
    ['c_nodeNew', '        new_node = Node(value, priority)'],
    ['c_headcheck', '        if self.head is None or priority > self.head.priority:'],
    ['c_linkhead', '            new_node.next = self.head'],
    ['c_linkhead', '            self.head = new_node'],
    ['c_headcheck', '        else:'],
    ['c_travinit', '            current = self.head'],
    ['c_while', '            while current.next is not None and current.next.priority >= priority:'],
    ['c_advance', '                current = current.next'],
    ['c_linkmid', '            new_node.next = current.next'],
    ['c_linkmid', '            current.next = new_node'],
    ['c_size', '        self.size += 1'],
  ],
  javascript: [
    ['', 'class Node {'],
    ['c_nodeNew', '  constructor(value, priority) {'],
    ['c_nodeNew', '    this.value = value;'],
    ['c_nodeNew', '    this.priority = priority;'],
    ['c_nodeNew', '    this.next = null;'],
    ['', '  }'], ['', '}'], ['', ''],
    ['', 'class PriorityQueue {'],
    ['c_sethead', '  constructor() {'],
    ['c_sethead', '    this.head = null;'],
    ['c_sethead', '    this.size = 0;'],
    ['', '  }'], ['', ''],
    ['c_insert', '  insert(value, priority) {'],
    ['c_nodeNew', '    const newNode = new Node(value, priority);'],
    ['c_headcheck', '    if (this.head === null || priority > this.head.priority) {'],
    ['c_linkhead', '      newNode.next = this.head;'],
    ['c_linkhead', '      this.head = newNode;'],
    ['c_headcheck', '    } else {'],
    ['c_travinit', '      let current = this.head;'],
    ['c_while', '      while (current.next !== null && current.next.priority >= priority) {'],
    ['c_advance', '        current = current.next;'],
    ['', '      }'],
    ['c_linkmid', '      newNode.next = current.next;'],
...
const PSEUDOCODE = [
  ...
  '    newNode.next = null',
  '    if head is null or priority > head.priority:',
  '        newNode.next = head',
  '        head = newNode',
  '    else:',
  '        current = head',
  '        while current.next is not null and current.next.priority >= priority:',
...
// the animated Value/Priority insert - instead of laying nodes out in raw input order.
...
//   'new'  - the newly created node (highlighted for the whole operation, both
//            while floating unlinked and after it settles into the list)
//   'cur'  - the node currently referenced by the 'current' traversal pointer
//   'peek' - the node being priority-compared as current.next, dashed outline
function buildSteps(elements, insertValue, insertPriority) {
  const steps = [];
  ...
  let addrCounter = elements.length;

  const mainFrame = () => frame('main()', [['size', String(size), true], ['head', fmt(headAddr), true]]);
  const insertFrame = (extra) => frame('insert(' + insertValue + ', ' + insertPriority + ')', [['value', String(insertValue)], ['priority', String(insertPriority)], ...(extra || [])]);

  function pushStep(opts) {
    steps.push({
      nodes: [...nodes],
      headAddr, size,
      neuAddr: opts.neuAddr,
      pendVal: opts.pendVal,
      pendPri: opts.pendPri,
      pendNext: opts.pendNext,
      currentAddr: opts.currentAddr,
      compareAddr: opts.compareAddr,
      badge: opts.badge,
      code: opts.code,
      vars: opts.vars,
    });
  ...
  pushStep({
    badge: 'Initial list \u2014 size ' + size + ' \u2014 insert(' + insertValue + ', ' + insertPriority + ') called',
    code: 'c_insert',
    vars: [mainFrame()],
  });
  ...
  pushStep({
    badge: 'create newNode { value: ' + insertValue + ', priority: ' + insertPriority + ', next: null } @' + newAddr,
    code: 'c_nodeNew',
    neuAddr: newAddr, pendVal: insertValue, pendPri: insertPriority,
    vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true]])],
  });

  const headIsEmpty = headAddr === null;
  const targetIdx = findInsertIndex(nodes, insertPriority);

  if (headIsEmpty) {
    pushStep({
      badge: 'head == null \u2192 the list is currently empty',
      code: 'c_headcheck',
      neuAddr: newAddr, pendVal: insertValue, pendPri: insertPriority,
      vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true]])],
    });

    nodes = [{ val: insertValue, priority: insertPriority, addr: newAddr }];
    headAddr = newAddr;
    pushStep({
      badge: 'head = newNode \u2192 single-node list',
      code: 'c_linkhead',
      neuAddr: newAddr,
      vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true]])],
    });
    ...
    pushStep({
      badge: 'priority > head.priority \u2192 new node has higher priority than the current head',
      code: 'c_headcheck',
      neuAddr: newAddr, pendVal: insertValue, pendPri: insertPriority,
      vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true]])],
    });

    const oldHeadAddr = headAddr;
    pushStep({
      badge: 'newNode.next = head \u2192 newNode.next = ' + fmt(oldHeadAddr),
      code: 'c_linkhead',
      neuAddr: newAddr, pendVal: insertValue, pendPri: insertPriority, pendNext: oldHeadAddr,
      vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true]])],
    });

    nodes = [{ val: insertValue, priority: insertPriority, addr: newAddr }, ...nodes];
    headAddr = newAddr;
    pushStep({
      badge: 'head = newNode \u2192 new node becomes the head',
      code: 'c_linkhead',
      neuAddr: newAddr,
      vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true]])],
    });
    ...
    pushStep({
      badge: 'priority <= head.priority \u2192 search for the correct insertion position, starting at head',
      code: 'c_headcheck',
      neuAddr: newAddr, pendVal: insertValue, pendPri: insertPriority,
      vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true]])],
    });
    ...
    let curIdx = nodes.findIndex(n => n.addr === currentAddr);
    pushStep({
      badge: 'current = head \u2192 current = ' + fmt(currentAddr),
      code: 'c_travinit',
      neuAddr: newAddr, pendVal: insertValue, pendPri: insertPriority, currentAddr,
      vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true], ['current', fmt(currentAddr), true]])],
    });

    while (true) {
      const nextNode = curIdx + 1 < nodes.length ? nodes[curIdx + 1] : null;
      const cond = nextNode !== null && nextNode.priority >= insertPriority;
      pushStep({
        badge: nextNode
          ? ('current.next (' + fmt(nextNode.addr) + ', value ' + nextNode.val + ', priority ' + nextNode.priority + ') ' + (cond ? '\u2265 ' : '< ') + insertPriority + ' \u2192 ' + (cond ? 'keep searching' : 'stop, insertion point found'))
          : 'current.next == null \u2192 reached the end of the list, stop',
        code: 'c_while',
        neuAddr: newAddr, pendVal: insertValue, pendPri: insertPriority, currentAddr, compareAddr: nextNode ? nextNode.addr : undefined,
        vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true], ['current', fmt(currentAddr), true]])],
      });
      if (!cond) break;
      currentAddr = nextNode.addr;
      curIdx += 1;
      pushStep({
        badge: 'current = current.next \u2192 current = ' + fmt(currentAddr),
        code: 'c_advance',
        neuAddr: newAddr, pendVal: insertValue, pendPri: insertPriority, currentAddr,
        vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true], ['current', fmt(currentAddr), true]])],
      });
    ...
    const stopNode = curIdx + 1 < nodes.length ? nodes[curIdx + 1] : null;
    const stopAddr = stopNode ? stopNode.addr : null;
    pushStep({
      badge: 'newNode.next = current.next \u2192 newNode.next = ' + fmt(stopAddr),
      code: 'c_linkmid',
      neuAddr: newAddr, pendVal: insertValue, pendPri: insertPriority, pendNext: stopAddr, currentAddr,
      vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true], ['current', fmt(currentAddr), true]])],
    });

    const newNodeObj = { val: insertValue, priority: insertPriority, addr: newAddr };
    nodes.splice(curIdx + 1, 0, newNodeObj);
    pushStep({
      badge: 'current.next = newNode \u2192 ' + fmt(currentAddr) + '.next = ' + fmt(newAddr),
      code: 'c_linkmid',
      neuAddr: newAddr, currentAddr,
      vars: [mainFrame(), insertFrame([['newNode', fmt(newAddr), true], ['current', fmt(currentAddr), true]])],
    });
  }
  ...
  pushStep({
    badge: 'insert complete \u2014 list (head \u2192 tail): ' + nodes.map(n => n.val + '|P' + n.priority).join(' \u2192 '),
    code: 'c_insert',
    neuAddr: newAddr,
    vars: [mainFrame()],
  });
...
const inpValue = ref(12);
const inpPriority = ref(4);
const lang = ref('java');
const speed = ref(650);
const si = ref(0);
...
const stepsData = reactive(buildSteps(initElems, 12, 4));
...
function applyInput() {
  ...
  playing.value = false;
  const built = buildSteps(elems, val, pri);
...
function nodeLabel(n) {
  ...
  if (n.addr === s.value.currentAddr) labels.push('current \u2192');
  if (n.addr === s.value.compareAddr) labels.push('comparing \u2192');
  return labels.join(' ');
}
...
<template>
  <div class="slide-wrapper">
    ...
    <div class="ll-toolbar">
      ...
      <input type="text" v-model="inpElems" placeholder="e.g. 20:5 10:3 15:5, or leave blank for an empty list" class="ll-text-input" style="width: 163px;"/>
      <label>Value</label>
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
        <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.currentAddr !== undefined" class="ll-ptr-chip">current = <b class="ll-c-orange">{{ fmt(s.currentAddr) }}</b></div>
              <div v-if="s.neuAddr !== undefined" class="ll-ptr-chip">newNode = <b class="ll-c-green">@{{ s.neuAddr }}</b></div>
            ...
            <div class="ll-stage">
              <template v-for="(n, idx) in s.nodes" :key="n.addr">
                <div class="ll-nwrap">
                  <div class="ll-who" :class="{ 'll-c-orange': n.addr === s.currentAddr || n.addr === s.compareAddr }">{{ nodeLabel(n) }}</div>
                  <div class="ll-addr">@{{ n.addr }}</div>
                  <div class="ll-node">
                    <div
                      class="ll-box"
                      :class="{
                        'll-box-new': n.addr === s.neuAddr,
                        'll-box-cur': n.addr === s.currentAddr,
                        'll-box-peek': n.addr === s.compareAddr
                      }"
                    >
                      <div class="ll-data">
              ...
              <div v-if="s.pendVal !== undefined" class="ll-nwrap">
                <div class="ll-who ll-c-green">newNode (pending) &#8594;</div>
                <div class="ll-addr">@{{ s.neuAddr }}</div>
                <div class="ll-node">
                  <div class="ll-box ll-box-new">
                    <div class="ll-data">
                      <div class="pq-node-val">{{ s.pendVal }}</div>
                      <div class="pq-node-pri">P:{{ s.pendPri }}</div>
                    </div>
              ...
              <div v-if="!s.nodes.length && s.pendVal === undefined" class="ll-empty-note">list is empty &mdash; head = null</div>
            </div>
          </div>
        </div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>current pointer</span>
          <span class="ll-leg"><span class="ll-legdot ll-legdot-peek"></span>comparing priority</span>
        </div>
      ...
      <div class="ll-right-col">
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            <h3>Algorithm</h3>
            <p>The list is always kept sorted in <b>descending priority order</b>, so <code>head</code> always points to the highest-priority node. Insertion first checks whether the new node beats the head outright (or the list is empty), which is an O(1) shortcut. Otherwise, a <code>current</code> pointer walks the list, stopping the moment it finds a node whose successor has a strictly lower priority than the new node &mdash; the new node is linked in right there. Nodes with priority <b>equal</b> to the new one are walked past (never stopped on), which places the new node right after them and preserves FIFO order among ties.</p>
            <h3>Time &amp; Space Complexity</h3>
            <table class="ll-complexity-table">
              <thead><tr><th>Case</th><th>Time</th><th>Why</th></tr></thead>
              <tbody>
                <tr><td>Empty list / first insertion</td><td>O(1)</td><td>no traversal &mdash; the new node is linked directly as <code>head</code></td></tr>
                <tr><td>Highest-priority insertion (new head)</td><td>O(1)</td><td>the head shortcut fires immediately, no traversal needed</td></tr>
                <tr><td>Lowest-priority insertion (new tail)</td><td>O(n)</td><td><code>current</code> must walk all the way to the end of the list</td></tr>
                <tr><td>Average case</td><td>O(n)</td><td><code>current</code> may need to walk past roughly half the existing nodes</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>a single new node is allocated per insert &mdash; no other extra memory</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: this is the opposite trade-off from the array-based version. There, insertion at the
              <i>front</i> (highest priority) was the expensive O(n) shift, while here insertion at the front is the
              cheap O(1) case &mdash; because a linked list only needs to relink a couple of pointers, not shift memory.
            </p>
            <h3>Advantages</h3>
            <p>No fixed capacity and therefore no <code>insertOverflow</code> / <b>Queue Overflow</b> check &mdash; the list simply keeps growing until memory runs out (a system boundary rather than a design limit). Inserting a new highest-priority element is O(1), unlike the array version which must shift every other element.</p>
            <h3>Limitations</h3>
            <p>Finding the correct insertion position for a non-head element requires an O(n) traversal, since a linked list has no random access the way an array does. Each node also carries the overhead of a <code>next</code> pointer alongside its data.</p>
            <h3>Queue vs Circular Queue vs Deque vs Priority Queue</h3>
            <p>A <b>Queue</b> inserts strictly at the rear in FIFO order. A <b>Circular Queue</b> also inserts at the rear but reuses freed slots by wrapping indices. A <b>Deque</b> can insert at either end. A <b>Priority Queue</b> inserts wherever the element's priority dictates &mdash; here, that means walking the list to find the first node whose priority is lower than the new element's.</p>
            <h3>Key Observations</h3>
            <p>Because there's no fixed capacity, insertion never fails outright the way an array-based <code>insert()</code> can with Queue Overflow. The trade-off is that the head-only fast path from a linked-list queue's <code>enqueue()</code> no longer applies in general &mdash; only the "new highest priority" case gets that O(1) benefit.</p>
            <h3>Edge Cases</h3>
            <p><b>Empty list:</b> the new node becomes <code>head</code> directly. <b>Insertion at the beginning:</b> triggered when the new priority exceeds the current head's priority &mdash; O(1), just two pointer updates. <b>Insertion in the middle:</b> <code>current</code> stops as soon as its successor's priority drops below the new priority. <b>Insertion at the end:</b> <code>current</code> walks until <code>current.next</code> is null. <b>Duplicate priorities:</b> the new node is placed immediately after all existing nodes of the same priority, preserving FIFO order among ties.</p>
          </div>
        </div>
      </div>
    </div>
...
.ll-box-new { border-color: var(--green); background: var(--nodeNew); }
.ll-box-cur { border-color: var(--orange); background: var(--nodeCur); box-shadow: 0 0 0 3px rgba(249,115,22,.25); }
.ll-box-peek { border: 2px dashed var(--orange); box-shadow: 0 0 0 3px rgba(249,115,22,.15); }
...
.ll-legdot-peek { background: var(--surface); border: 1.5px dashed var(--orange); }
