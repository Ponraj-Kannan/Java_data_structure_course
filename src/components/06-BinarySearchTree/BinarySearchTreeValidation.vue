...
const props = defineProps({
  topic:    { type: String, default: 'Binary Search Tree \u2014 Validation' },
  subTopic: { type: String, default: 'Validate if a Binary Tree is a Valid BST (Range Bounds Algorithm)' },
});
...
const CODES = {
  java: [
    ...
    ['',              '    Node root;'],
    ['',              ''],
    ['c_call',        '    boolean isValidBST(Node root) {'],
    ['c_validFn',     '        return validate(root, Long.MIN_VALUE, Long.MAX_VALUE);'],
    ['',              '    }'],
    ['',              ''],
    ['c_validFn',     '    boolean validate(Node root, long minVal, long maxVal) {'],
    ['c_nullCheck',   '        if (root == null) {'],
    ['c_nullReturn',  '            return true;'],
    ['',              '        }'],
    ['c_rangeCheck',  '        if (root.data <= minVal || root.data >= maxVal) {'],
    ['c_rangeReturn', '            return false;'],
    ['',              '        }'],
    ['c_leftRecurse', '        boolean leftValid = validate(root.left, minVal, root.data);'],
    ['c_leftCheck',   '        if (!leftValid) {'],
    ['c_leftCheck',   '            return false;'],
    ['',              '        }'],
    ['c_rightRecurse','        boolean rightValid = validate(root.right, root.data, maxVal);'],
    ['c_rightCheck',  '        if (!rightValid) {'],
    ['c_rightCheck',  '            return false;'],
    ['',              '        }'],
    ['c_retTrue',     '        return true;'],
    ['',              '    }'],
    ['',              '}'],
  ],
  c: [
    ['c_validFn',     'bool validate(struct Node* root, long minVal, long maxVal) {'],
    ['c_nullCheck',   '    if (root == NULL) {'],
    ['c_nullReturn',  '        return true;'],
    ['',              '    }'],
    ['c_rangeCheck',  '    if (root->data <= minVal || root->data >= maxVal) {'],
    ['c_rangeReturn', '        return false;'],
    ['',              '    }'],
    ['c_leftRecurse', '    bool leftValid = validate(root->left, minVal, root->data);'],
    ['c_leftCheck',   '    if (!leftValid) {'],
    ['c_leftCheck',   '        return false;'],
    ['',              '    }'],
    ['c_rightRecurse','    bool rightValid = validate(root->right, root->data, maxVal);'],
    ['c_rightCheck',  '    if (!rightValid) {'],
    ['c_rightCheck',  '        return false;'],
    ['',              '    }'],
    ['c_retTrue',     '    return true;'],
    ['',              '}'],
    ['',              ''],
    ['c_call',        'bool isValidBST(struct Node* root) {'],
    ['c_validFn',     '    return validate(root, LONG_MIN, LONG_MAX);'],
    ['',              '}'],
  ],
  cpp: [
    ['c_validFn',     'bool validate(Node* root, long minVal, long maxVal) {'],
    ...
    ['',              '}'],
    ['',              ''],
    ['c_call',        'bool isValidBST(Node* root) {'],
    ['c_validFn',     '    return validate(root, LONG_MIN, LONG_MAX);'],
    ['',              '}'],
  ],
  python: [
    ['c_call',        'def is_valid_bst(root):'],
    ['c_validFn',     '    return validate(root, float("-inf"), float("inf"))'],
    ['',              ''],
    ['c_validFn',     'def validate(node, min_val, max_val):'],
    ['c_nullCheck',   '    if node is None:'],
    ['c_nullReturn',  '        return True'],
    ['c_rangeCheck',  '    if node.data <= min_val or node.data >= max_val:'],
    ['c_rangeReturn', '        return False'],
    ['c_leftRecurse', '    left_valid = validate(node.left, min_val, node.data)'],
    ['c_leftCheck',   '    if not left_valid:'],
    ['c_leftCheck',   '        return False'],
    ['c_rightRecurse','    right_valid = validate(node.right, node.data, max_val)'],
    ['c_rightCheck',  '    if not right_valid:'],
    ['c_rightCheck',  '        return False'],
    ['c_retTrue',     '    return True'],
  ],
};
...
const PSEUDOCODE = [
  ...
  '',
  '    leftValid = validate(node.left, minVal, node.data)',
  '    if not leftValid:',
  '        return false',
  '',
  '    rightValid = validate(node.right, node.data, maxVal)',
  '    if not rightValid:',
  '        return false',
  '',
  '    return true',
...
function frameInfo(title, rows) { return { title, rows }; }

/* ------------------------------------------------------------------ */
/* Binary Tree Builder (Level Order)                                   */
/* ------------------------------------------------------------------ */
function buildTreeFromLevelOrder(values) {
  ...
  if (rootId === null) return { nodes: [], edges: [], rootId: null };
  ...
  while (queue.length && i < values.length) {
    ...
    // Left child
    if (i < values.length && values[i] !== null && values[i] !== undefined && !Number.isNaN(values[i])) {
      const leftNode = nodes.find(n => n.id === i);
      if (leftNode) {
    ...
    // Right child
    if (i < values.length && values[i] !== null && values[i] !== undefined && !Number.isNaN(values[i])) {
      const rightNode = nodes.find(n => n.id === i);
      if (rightNode) {
...
/* ------------------------------------------------------------------ */
/* Step Generation — recursive simulation, per-statement              */
/* ------------------------------------------------------------------ */
function buildSteps(nodes, edges, rootId) {
  ...
  const callStack   = [];
  let   finalResult  = null;   // set only when validate() finishes completely
  let   failedNodeId = null;   // the node that violated the BST constraint

  /* ── snap helper ────────────────────────────────────────────── */
  function snap(currId, code, badge, minVal, maxVal) {
    // treeStatus is null for ALL intermediate frames;
    // only the last summary frame (pushed after validate() returns) gets 'valid'/'invalid'
    const isLastFrame = finalResult !== null;
    const treeStatus  = isLastFrame ? (finalResult ? 'valid' : 'invalid') : null;

    const varsFrames = callStack.map((f, depth) => {
      const fmtMin = f.min === -Infinity ? '-∞' : f.min;
      const fmtMax = f.max === Infinity  ? '+∞' : f.max;
      return frameInfo(
        `validate([${f.val}], min=${fmtMin}, max=${fmtMax})`,
        [
          ['node',   fmt(f.addr), true],
          ['minVal', '' + fmtMin],
          ['maxVal', '' + fmtMax],
        ]
      );
    ...
    steps.push({
      ...
      rootId,
      currId:     currId ?? null,
      invalidId:  isLastFrame && !finalResult ? failedNodeId : null,
      visitedArr: [...visited],
      validArr:   [...validNodes],
      badge,
      code,
  ...
  if (rootId === null) {
    finalResult = true;
    snap(null, 'c_call',      'isValidBST(root) called. Tree is empty.', -Infinity, Infinity);
    snap(null, 'c_nullCheck', 'root == null? → TRUE → empty tree is a Valid BST.', -Infinity, Infinity);
    snap(null, 'c_nullReturn','return true', -Infinity, Infinity);
    return steps;
  }

  /* ── isValidBST entry ───────────────────────────────────────── */
  snap(rootId, 'c_call',    'isValidBST(root) called. Begin BST validation.', -Infinity, Infinity);
  snap(rootId, 'c_validFn', 'Call validate(root, min=-∞, max=+∞)', -Infinity, Infinity);

  /* ── recursive validate ─────────────────────────────────────── */
  function validate(nodeId, minVal, maxVal) {
    const node = nodesMap[nodeId] ?? null;
    const addr = node ? node.addr : null;
    const val  = node ? node.val  : null;

    const fmtMin = minVal === -Infinity ? '-∞' : minVal;
    const fmtMax = maxVal === Infinity  ? '+∞' : maxVal;

    callStack.push({ id: nodeId, val, addr, min: minVal, max: maxVal });

    /* — function entry — */
    snap(nodeId, 'c_validFn',
      `Enter validate(root = ${nodeId !== null ? '@' + addr + ' [' + val + ']' : 'null'}, min=${fmtMin}, max=${fmtMax})`,
      minVal, maxVal);

    /* — if (root == null) — */
    if (nodeId === null) {
      snap(null, 'c_nullCheck',
        'root == null? → TRUE → Base case: empty subtree is valid.',
        minVal, maxVal);
      snap(null, 'c_nullReturn',
    ...
    visited.add(nodeId);
    snap(nodeId, 'c_nullCheck',
      `root == null? → FALSE  (root = @${addr}, val = ${val})`,
      minVal, maxVal);

    /* — if (root.data <= minVal || root.data >= maxVal) — */
    const isViolated = val <= minVal || val >= maxVal;
    snap(nodeId, 'c_rangeCheck',
      `Check: ${fmtMin} < ${val} < ${fmtMax}? → ${isViolated ? 'FALSE! BST constraint violated.' : 'TRUE. Within valid range.'}`,
      minVal, maxVal);

    if (isViolated) {
      failedNodeId = nodeId;
      snap(nodeId, 'c_rangeReturn',
        `${val} is NOT in (${fmtMin}, ${fmtMax}) → return false.`,
        minVal, maxVal);
      callStack.pop();
      return false;
    }

    /* — leftValid = validate(root.left, minVal, root.data) — */
    snap(nodeId, 'c_leftRecurse',
      `Recurse left: validate(root.left, min=${fmtMin}, max=${val})`,
      minVal, maxVal);
    const leftValid = validate(node.left, minVal, val);

    /* — if (!leftValid) — */
    snap(nodeId, 'c_leftCheck',
      `if (!leftValid) → leftValid = ${leftValid} → ${!leftValid ? 'TRUE → return false' : 'FALSE → continue'}`,
      minVal, maxVal);
    if (!leftValid) {
      snap(nodeId, 'c_leftCheck',
        'Left subtree invalid → return false.',
        minVal, maxVal);
      callStack.pop();
      return false;
    }

    /* — rightValid = validate(root.right, root.data, maxVal) — */
    snap(nodeId, 'c_rightRecurse',
      `Recurse right: validate(root.right, min=${val}, max=${fmtMax})`,
      minVal, maxVal);
    const rightValid = validate(node.right, val, maxVal);

    /* — if (!rightValid) — */
    snap(nodeId, 'c_rightCheck',
      `if (!rightValid) → rightValid = ${rightValid} → ${!rightValid ? 'TRUE → return false' : 'FALSE → continue'}`,
      minVal, maxVal);
    if (!rightValid) {
      snap(nodeId, 'c_rightCheck',
        'Right subtree invalid → return false.',
        minVal, maxVal);
      callStack.pop();
      return false;
    }
    ...
    snap(nodeId, 'c_retTrue',
      `Node @${addr} [${val}] ✓ Both subtrees valid. return true.`,
      minVal, maxVal);
    callStack.pop();
    return true;
  }

  const result = validate(rootId, -Infinity, Infinity);

  /* ── final summary frame (only this frame gets treeStatus) ── */
  finalResult = result;
  snap(null,
    result ? 'c_retTrue' : 'c_rangeReturn',
    `Validation complete! Tree is ${result ? 'a Valid BST ✓' : 'Not a BST ✗'}`,
    -Infinity, Infinity);
...
const currentStep = computed(() => steps.value[si.value] || {
  nodes: [], edges: [], vars: [], badge: '', rootId: null, currId: null, invalidId: null, visitedArr: [], validArr: [], minVal: '-∞', maxVal: '+∞', treeStatus: null
});
...
/* ------------------------------------------------------------------ */
/* Tree layout — fixed node size & fixed canvas viewBox                 */
/* ------------------------------------------------------------------ */
const NODE_W     = 132;
const NODE_BOX_H = 44;
const NODE_H     = 68;
...
function getPointerBadgesForNode(id) {
  ...
  if (step.invalidId === id)  ptrs.push({ name: 'invalid', label: 'invalid', color: '#ef4444' });
...
function nodeBoxClass(n) {
  const step = currentStep.value;
  if (!step) return '';
  if (step.invalidId === n.id)                return 'bs-box-invalid';
  if (step.currId === n.id)                   return 'bs-box-current';
  if ((step.validArr || []).includes(n.id))   return 'bs-box-valid';
...
function applyInput() {
  const arr = inpElems.value
    .trim().split(/\s+/).filter(Boolean).map(v => (v === 'null' || v === 'N' ? null : Number(v)));
  clearTimeout(playTimer);
  playing.value = false;
  if (!arr.length) { steps.value = []; si.value = 0; return; }
  const { nodes, edges, rootId } = buildTreeFromLevelOrder(arr);
      ...
      <div class="row-main">
        <div class="ll-root">
          ...
          <div class="ll-toolbar">
            <label>Level Order Elements</label>
            ...
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              ...
              <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
                <div class="ll-perm-area" ref="vizSvgRef">
                  <div class="ll-ptrs">
                    <div class="ll-ptr-chip">minVal = <b class="ll-c-blue">{{ s.minVal }}</b></div>
                    <div class="ll-ptr-chip">maxVal = <b class="ll-c-blue">{{ s.maxVal }}</b></div>
                    <div v-if="s.currId !== null && s.currId !== undefined" class="ll-ptr-chip">curr = <b class="ll-c-orange">{{ fmt(ADDR(s.currId)) }}</b></div>
                    <div v-if="s.treeStatus === 'valid'" class="ll-ptr-chip bs-valid-chip">&#10003; Valid BST</div>
                    <div v-if="s.treeStatus === 'invalid'" class="ll-ptr-chip bs-invalid-chip">&#10007; Not a BST</div>
                  </div>
              ...
              <div class="ll-legend">
                ...
                <span class="ll-leg"><span class="ll-legdot ll-legdot-current"></span>curr (checking bounds)</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-valid"></span>validated BST node</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-invalid"></span>violation node</span>
                <span class="ll-leg"><span class="ll-legdot ll-legdot-visited"></span>visited</span>
              </div>

              <!-- VAR FRAMES -->
              <div class="ll-table-area" :style="{ height: tableHeight + 'px' }">
                <div class="ll-table-title">Call stack frames &mdash; innermost = active call</div>
              ...
              <div class="ll-badge-wrap">
                <div class="ll-badge" :class="{
                  'bs-badge-found':    s.treeStatus === 'valid',
                  'bs-badge-notfound': s.treeStatus === 'invalid',
                }">{{ s.badge }}</div>
              </div>
            </div>
            ...
            <div class="ll-right-col">
              <div class="ll-code-panel">
                ...
                <div v-else class="ll-info-scroll">
                  <h3>Time &amp; Space Complexity &mdash; Validate BST</h3>
                  <table class="ll-complexity-table">
                    <thead><tr><th>Case</th><th>Time</th><th>Why</th></tr></thead>
                    <tbody>
                      <tr><td>Best Case</td><td>O(1)</td><td>Root value itself violates min/max bounds immediately.</td></tr>
                      <tr><td>Average Case</td><td>O(N)</td><td>Must check every node in a balanced tree to confirm BST property.</td></tr>
                      <tr><td>Worst Case</td><td>O(N)</td><td>Must traverse all N nodes in a skewed or complete tree.</td></tr>
                      <tr><td>Space</td><td>O(h)</td><td>Call stack depth for recursion (h = tree height, O(log N) balanced, O(N) skewed).</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    <b>Range Bounds Algorithm for BST Validation:</b><br/>
                    1. Each node must satisfy <code>minVal &lt; node.data &lt; maxVal</code>.<br/>
                    2. When moving <b>left</b>, update <code>maxVal = node.data</code>.<br/>
                    3. When moving <b>right</b>, update <code>minVal = node.data</code>.<br/>
                    4. Empty subtrees (<code>null</code>) return <code>true</code>.
                  </p>
                </div>
              </div>
            </div>
...
.ll-root {
  ...
  --node: #1d4ed8; --nodeFound: #15803d; --nodeCur: #c2410c; --nodeVisited: #64748b; --nodeInvalid: #dc2626;
...
.ll-text-input { background: var(--surface); border: 1px solid var(--border2); color: var(--text); border-radius: var(--radius-sm); padding: 5px 8px; font-size: 13px; font-family: monospace; transition: border-color .15s; width: 170px; }
.ll-text-input:focus { outline: none; border-color: var(--coral); box-shadow: 0 0 0 3px rgba(240,77,77,.1); }
.ll-viz-btn { background: var(--coral); color: #fff; border: none; padding: 6px 14px; border-radius: var(--radius-sm); cursor: pointer; font-size: 12px; font-weight: 600; box-shadow: var(--shadow-sm); transition: filter .15s; }
...
.bs-valid-chip   { background: var(--green-light); border-color: var(--green); color: #15803d; font-weight: 700; }
.bs-invalid-chip { background: var(--red-light); border-color: var(--red); color: #b91c1c; font-weight: 700; }
...
.bs-box-invalid { border-color: #ef4444 !important; background: var(--nodeInvalid) !important; box-shadow: 0 0 0 3px rgba(239,68,68,.35) !important; }
...
.bs-box-valid   { border-color: #22c55e !important; background: var(--nodeFound) !important; }
...
.ll-legdot-valid   { background: var(--nodeFound);   border: 1.5px solid var(--green); }
.ll-legdot-invalid { background: var(--nodeInvalid); border: 1.5px solid var(--red); }
...
.bs-badge-notfound { border-left-color: #ef4444 !important; background: var(--red-light) !important; color: #991b1b !important; }
