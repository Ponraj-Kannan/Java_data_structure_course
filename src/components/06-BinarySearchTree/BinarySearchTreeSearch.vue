...
const props = defineProps({
  topic:    { type: String, default: 'Binary Search Tree \u2014 Search Operation' },
  subTopic: { type: String, default: 'BST Search with Node Pointers' },
});
...
const CODES = {
  java: [
    ...
    ['', '    Node root;'],
    ['', ''],
    ['c_search',     '    Node search(int target) {'],
    ['c_setCurr',    '        Node curr = root;'],
    ['c_whileLoop',  '        while (curr != null) {'],
    ['c_checkEqual', '            if (target == curr.data) {'],
    ['c_found',      '                return curr;'],
    ['c_cmpLeft',    '            } else if (target < curr.data) {'],
    ['c_moveLeft',   '                curr = curr.left;'],
    ['c_cmpRight',   '            } else {'],
    ['c_moveRight',  '                curr = curr.right;'],
    ['',             '            }'],
    ['',             '        }'],
    ['c_notFound',   '        return null;'],
    ['', '    }'],
    ['', ''],
    ['c_main', '    public static void main(String[] args) {'],
    ['c_main', '        BST bst = new BST();'],
    ['c_main', '        // ... insert nodes ...'],
    ['c_search', '        Node result = bst.search(target);'],
    ['', '    }'],
    ['', '}'],
  ],
  c: [
    ['c_search',     'struct Node* search(struct Node* root, int target) {'],
    ...
    ['c_main', 'int main() {'],
    ['c_main', '    // ... build tree ...'],
    ['c_search', '    struct Node* result = search(root, target);'],
    ['c_main', '    return 0;'],
    ['', '}'],
  ],
  cpp: [
    ['c_search',     'Node* search(Node* root, int target) {'],
    ['c_setCurr',    '    Node* curr = root;'],
    ['c_whileLoop',  '    while (curr != nullptr) {'],
    ['c_checkEqual', '        if (target == curr->data) {'],
    ['c_found',      '            return curr;'],
    ['c_cmpLeft',    '        } else if (target < curr->data) {'],
    ['c_moveLeft',   '            curr = curr->left;'],
    ...
    ['',             '        }'],
    ['',             '    }'],
    ['c_notFound',   '    return nullptr;'],
    ['', '}'],
    ['', ''],
    ['c_main', 'int main() {'],
    ['c_main', '    // ... build tree ...'],
    ['c_search', '    Node* result = search(root, target);'],
    ['c_main', '    return 0;'],
    ['', '}'],
  ],
  python: [
    ['c_search',     'def search(root, target):'],
    ['c_setCurr',    '    curr = root'],
    ['c_whileLoop',  '    while curr is not None:'],
    ['c_checkEqual', '        if target == curr.data:'],
    ['c_found',      '            return curr'],
    ['c_cmpLeft',    '        elif target < curr.data:'],
    ['c_moveLeft',   '            curr = curr.left'],
    ['c_cmpRight',   '        else:'],
    ['c_moveRight',  '            curr = curr.right'],
    ['c_notFound',   '    return None'],
    ['', ''],
    ['c_main', '# build tree ...'],
    ['c_search', 'result = search(root, target)'],
  ],
};
...
function buildSteps(nodes, edges, rootId, target) {
  const steps  = [];
  const nodesMap = {};
  nodes.forEach(n => (nodesMap[n.id] = n));

  function snap(currId, foundId, visited, badge, code, vars) {
    steps.push({
      nodes:      nodes.map(n => ({ ...n })),
      edges:      edges.map(e => ({ ...e })),
      rootId,
  ...
  if (rootId === null) {
    snap(null, null, [], 'Tree is empty \u2014 search cannot proceed.', 'c_notFound',
      [frame('search(target)', [['target', target], ['curr', 'null', true]])]);
    return steps;
  }

  const visited = [];

  snap(null, null, visited,
    `search(target = ${target}) called. Beginning BST search.`,
    'c_search',
    [frame('search(target)', [['target', target]])]);

  let currId = rootId;
  snap(currId, null, visited,
    `Set curr = root (@${ADDR(currId)}, val = ${nodesMap[currId].val}).`,
    'c_setCurr',
    [frame('search(target)', [['target', target], ['curr', fmt(ADDR(currId)), true]])]);

  while (true) {
    const nullCheck = currId !== null;
    snap(currId, null, visited,
      nullCheck
        ? `while curr != null \u2192 curr = @${ADDR(currId)} (val=${nodesMap[currId].val}) \u2192 continue loop`
        : `while curr != null \u2192 curr = null \u2192 exit loop`,
      'c_whileLoop',
      [frame('search(target)', [
        ['target', target],
        ['curr', currId !== null ? fmt(ADDR(currId)) : 'null', true],
      ])]);

    if (currId === null) {
      snap(null, null, visited,
        `curr == null \u2192 target ${target} NOT FOUND in BST. return null.`,
        'c_notFound',
        [frame('search(target)', [['target', target], ['curr', 'null', true], ['return', 'null', true]])]);
      break;
    }
    ...
    snap(currId, null, visited,
      `Compare: if (${target} == ${curr.val}) \u2192 ${target === curr.val ? 'TRUE \u2014 FOUND!' : 'FALSE'}`,
      'c_checkEqual',
      [frame('search(target)', [
        ['target', target],
        ['curr', fmt(curr.addr), true],
        ['curr.data', '' + curr.val],
      ])]);

    if (target === curr.val) {
      snap(currId, currId, visited,
        `target ${target} == curr.data ${curr.val} \u2713 FOUND! return curr (@${curr.addr}).`,
        'c_found',
        [frame('search(target)', [
          ['target', target],
          ['curr', fmt(curr.addr), true],
          ['return', fmt(curr.addr), true],
        ])]);
      break;
    }
    ...
    if (target < curr.val) {
      snap(currId, null, visited,
        `target ${target} < curr.data ${curr.val} \u2192 TRUE \u2192 go LEFT`,
        'c_cmpLeft',
        [frame('search(target)', [
          ['target', target],
          ['curr', fmt(curr.addr), true],
      ...
      snap(nextId, null, visited,
        nextId !== null
          ? `curr = curr.left \u2192 move to @${ADDR(nextId)} (val=${nodesMap[nextId].val})`
          : `curr = curr.left \u2192 curr becomes null`,
        'c_moveLeft',
        [frame('search(target)', [
          ['target', target],
          ['curr', nextId !== null ? fmt(ADDR(nextId)) : 'null', true],
        ])]);
      currId = nextId;

    } else {
      snap(currId, null, visited,
        `target ${target} < curr.data ${curr.val} \u2192 FALSE`,
        'c_cmpLeft',
        [frame('search(target)', [
          ['target', target],
          ['curr', fmt(curr.addr), true],
      ...
      snap(currId, null, visited,
        `target ${target} > curr.data ${curr.val} \u2192 else branch \u2192 go RIGHT`,
        'c_cmpRight',
        [frame('search(target)', [
          ['target', target],
          ['curr', fmt(curr.addr), true],
      ...
      snap(nextId, null, visited,
        nextId !== null
          ? `curr = curr.right \u2192 move to @${ADDR(nextId)} (val=${nodesMap[nextId].val})`
          : `curr = curr.right \u2192 curr becomes null`,
        'c_moveRight',
        [frame('search(target)', [
          ['target', target],
          ['curr', nextId !== null ? fmt(ADDR(nextId)) : 'null', true],
...
const currentStep = computed(() => steps.value[si.value] || {
  nodes: [], edges: [], vars: [], badge: '', rootId: null, currId: null, foundId: null, visitedArr: [],
});
...
function nodeBoxClass(n) {
  const step = currentStep.value;
  if (!step) return '';
  if (step.foundId === n.id) return 'bs-box-found';
          ...
          <div class="ll-main" ref="mainRef">
            <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
              ...
              <div class="ll-viz-wrap" :style="{ height: vizHeight + 'px' }">
                <div class="ll-perm-area" ref="vizSvgRef">
                  <div class="ll-ptrs">
                    ...
                    <div v-if="s.foundId !== null && s.foundId !== undefined" class="ll-ptr-chip bs-found-chip">&#10003; Target Found!</div>
              ...
              <div class="ll-legend">
                ...
                <span class="ll-leg"><span class="ll-legdot ll-legdot-found"></span>target found</span>
              </div>
            ...
            <div class="ll-right-col">
              <div class="ll-code-panel">
                ...
                <div v-else class="ll-info-scroll">
                  <h3>Time &amp; Space Complexity &mdash; BST Search</h3>
                  <table class="ll-complexity-table">
                    <thead><tr><th>Case</th><th>Time</th><th>Why</th></tr></thead>
                    <tbody>
                      <tr><td>Best Case</td><td>O(1)</td><td>Target is the root &mdash; found on the very first comparison.</td></tr>
                      <tr><td>Average Case</td><td>O(log n)</td><td>Balanced BST: each comparison halves the remaining subtree.</td></tr>
                      <tr><td>Worst Case</td><td>O(n)</td><td>Skewed tree (sorted input): degenerates to a linked list &mdash; all nodes may be visited.</td></tr>
                      <tr><td>Space</td><td>O(1)</td><td>Iterative search with a single <code>curr</code> pointer &mdash; no recursion stack.</td></tr>
                    </tbody>
                  </table>
                  <p class="ll-note">
                    Key insight: BST search time = O(tree height h). Balanced tree: h &asymp; log&#8322; n. Skewed tree: h = n. Always O(h) comparisons.
                  </p>
                  <h3>Search Algorithm</h3>
                  <p>Start at root. At each node: if target &lt; curr.data &rarr; go <b>left</b>; if target &gt; curr.data &rarr; go <b>right</b>; if equal &rarr; <b>found</b>. If curr becomes null &rarr; <b>not found</b>.</p>
                  <h3>BST Property Used</h3>
                  <p>Left subtree values &lt; node.data &lt; right subtree values. This guarantees binary elimination &mdash; no need to check both subtrees at each step.</p>
                  <h3>vs Linear Search</h3>
                  <p>Linear search: O(n) always. BST search: O(log n) average on balanced tree. For 1,000,000 nodes: linear needs up to 1,000,000 checks &mdash; BST needs at most ~20.</p>
                </div>
              </div>
            </div>
          </div>
