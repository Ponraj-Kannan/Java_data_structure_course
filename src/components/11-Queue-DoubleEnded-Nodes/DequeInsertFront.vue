...
const CODES = {
  java: [
    ...
    ['c_sethead', '    Node rear;'], ['', ''],
    ['c_ins', '    void insertFront(int value) {'],
    ['c_newnode', '        Node newNode = new Node(value);'],
    ['c_setnext', '        newNode.next = front;'],
    ['c_setprev', '        newNode.prev = null;'],
    ['c_emptycase', '        if (front == null) {'],
    ['c_emptycase', '            front = newNode;'],
    ['c_emptycase', '            rear = newNode;'],
    ['c_emptycase', '            return;'],
    ['c_emptycase', '        }'],
    ['c_linkfront', '        front.prev = newNode;'],
  ...
  python: [
    ...
    ['c_ins', '    def insert_front(self, value):'],
    ['c_newnode', '        new_node = Node(value)'],
    ['c_setnext', '        new_node.next = self.front'],
    ['c_setprev', '        new_node.prev = None'],
    ['c_emptycase', '        if self.front is None:'],
    ['c_emptycase', '            self.front = new_node'],
    ['c_emptycase', '            self.rear = new_node'],
    ['c_emptycase', '            return'],
    ['c_linkfront', '        self.front.prev = new_node'],
...
function buildSteps(values, insertValue) {
  ...
  const fnLabel = 'insertFront(' + insertValue + ')';
  ...
  steps.push({
    nodes: [...nodes0], frontAddr: frontAddr0, rearAddr: rearAddr0,
    badge: 'newNode.next = front \u2192 newNode points forward to ' + fmt(frontAddr0),
    code: 'c_setnext',
    newAddr, newVal: insertValue,
    vars: [frame('main()', [['front', fmt(frontAddr0), true], ['rear', fmt(rearAddr0), true]]), frame(fnLabel, [['newNode', fmt(newAddr), true]])],
  });
  ...
  steps.push({
    nodes: [...nodes0], frontAddr: frontAddr0, rearAddr: rearAddr0,
    badge: 'newNode.prev = null \u2192 newNode will become the new front (nothing before it)',
    code: 'c_setprev',
    newAddr, newVal: insertValue,
    vars: [frame('main()', [['front', fmt(frontAddr0), true], ['rear', fmt(rearAddr0), true]]), frame(fnLabel, [['newNode', fmt(newAddr), true]])],
  });
  ...
  if (n === 0) {
    steps.push({
      nodes: [...nodes0], frontAddr: frontAddr0, rearAddr: rearAddr0,
      badge: 'front == null \u2192 the deque is currently empty',
      code: 'c_emptycase',
      newAddr, newVal: insertValue,
  ...
  steps.push({
    nodes: [...nodes0], frontAddr: frontAddr0, rearAddr: rearAddr0,
    badge: 'front != null \u2192 deque is not empty, link the current front back to newNode',
    code: 'c_linkfront',
    curAddr: frontAddr0,
    newAddr, newVal: insertValue,
    vars: [frame('main()', [['front', fmt(frontAddr0), true], ['rear', fmt(rearAddr0), true]]), frame(fnLabel, [['newNode', fmt(newAddr), true]])],
  ...
  steps.push({
    nodes: [...nodes0], frontAddr: frontAddr0, rearAddr: rearAddr0,
    badge: 'front.prev = newNode \u2192 @' + frontAddr0 + '.prev now points back to newNode @' + newAddr,
    code: 'c_linkfront',
    curAddr: frontAddr0,
    newAddr, newVal: insertValue,
    vars: [frame('main()', [['front', fmt(frontAddr0), true], ['rear', fmt(rearAddr0), true]]), frame(fnLabel, [['newNode', fmt(newAddr), true]])],
  ...
  steps.push({
    nodes: finalNodes, frontAddr: newAddr, rearAddr: rearAddr0,
    badge: 'front = newNode \u2192 newNode @' + newAddr + ' is now the front, deque has ' + (n + 1) + ' node(s)',
    code: 'c_setfront',
    newAddr,
    vars: [frame('main()', [['front', fmt(newAddr), true], ['rear', fmt(rearAddr0), true]])],
  });
              ...
              <div v-if="s.newAddr !== undefined && !s.nodes.some(nd => nd.addr === s.newAddr)" class="ll-nwrap ll-nwrap-pending">
                ...
                <div class="ll-node">
                  <div class="ll-box ll-box-deque ll-box-new ll-box-pending">
                    <div class="ll-ptr ll-ptr-left"><small>prev</small>{{ s.code === 'c_newnode' ? '?' : 'null' }}</div>
                    <div class="ll-data">{{ s.newVal }}</div>
                    <div class="ll-ptr"><small>next</small>{{ s.code === 'c_newnode' ? '?' : fmt(s.frontAddr) }}</div>
                  </div>
                </div>
              </div>
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                ...
                <tr><td>Non-empty deque</td><td>O(1)</td><td>the <code>front</code> pointer gives direct access &mdash; no traversal is ever needed</td></tr>
            ...
            <p class="ll-note">
              ...
              at <b>both</b> ends &mdash; unlike a singly linked list, where operating on the rear would
              require an O(n) traversal to find the node just before it.
            </p>
          </div>
        </div>
      </div>
