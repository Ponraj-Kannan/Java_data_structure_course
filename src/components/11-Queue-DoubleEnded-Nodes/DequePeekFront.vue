...
const CODES = {
  java: [
    ...
    ['c_underflow', '        }'],
    ['c_peek', '        // front.data is the value at the front'],
  ...
  javascript: [
    ...
    ['c_underflow', '    }'],
    ['c_peek', '    // this.front.data is the value at the front'],
...
const PSEUDOCODE = [
  ...
  '        return null',
  '    return front.data   // deque is left unchanged',
];
...
function buildSteps(values) {
  ...
  if (n === 0) {
    steps.push({
      ...
      badge: 'front == null \u2192 the deque is empty, nothing to peek',
      code: 'c_underflow',
  ...
  steps.push({
    nodes: [...nodes0], frontAddr: frontAddr0, rearAddr: rearAddr0,
    badge: 'Inspecting front node @' + frontAddr0 + ' (value = ' + nodes0[0].val + ') \u2014 nothing is modified',
    code: 'c_peek',
    curAddr: frontAddr0,
  ...
  steps.push({
    nodes: [...nodes0], frontAddr: frontAddr0, rearAddr: rearAddr0,
    badge: 'return front.data \u2192 ' + nodes0[0].val + ' returned, deque unchanged (' + n + ' node(s))',
    code: 'c_return',
    curAddr: frontAddr0,
    vars: [frame('main()', [['front', fmt(frontAddr0), true], ['rear', fmt(rearAddr0), true]]), frame(fnLabel, [['returns', nodes0[0].val, false]])],
  });
...
<template>
  <div class="slide-wrapper">
    ...
    <div class="ll-toolbar">
      ...
      <button class="ll-viz-btn" @click="applyInput">&#9654; Peek Front</button>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty deque</td><td>O(1)</td><td>just check <code>front == null</code> and report that the deque is empty</td></tr>
                <tr><td>Non-empty deque</td><td>O(1)</td><td>the <code>front</code> pointer gives direct access to the value &mdash; no traversal is ever needed</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used; no node is created or removed</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <b>peekFront()</b> is a <b>read-only</b> operation &mdash; it only
              inspects <code>front.data</code> and never modifies <code>front</code>,
              ...
              <b>deleteFront()</b>, which also touches the front but actually unlinks and removes the node.
              As always, check for an empty deque first &mdash; peeking a <code>null</code> front has
