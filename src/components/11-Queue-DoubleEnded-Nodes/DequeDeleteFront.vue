...
const CODES = {
  java: [
    ...
    ['c_sethead', '    Node rear;'], ['', ''],
    ['c_ins', '    void deleteFront() {'],
    ['c_underflow', '        if (front == null) {'],
    ['c_underflow', '            System.out.println("Deque Underflow");'],
    ['c_underflow', '            return;'],
    ['c_underflow', '        }'],
    ['c_savefront', '        Node temp = front;'],
    ['c_advance', '        front = front.next;'],
    ['c_lastcase', '        if (front == null) {'],
    ['c_lastcase', '            rear = null;'],
    ['c_relinkfront', '        } else {'],
    ['c_relinkfront', '            front.prev = null;'],
    ['c_relinkfront', '        }'],
    ['', '    }'], ['', '}'],
  ],
  c: [
    ...
    ['c_underflow', '        return;'],
    ['c_underflow', '    }'],
    ['c_savefront', '    struct Node *temp = front;'],
    ['c_advance', '    front = front->next;'],
    ['c_lastcase', '    if (front == NULL) {'],
    ['c_lastcase', '        rear = NULL;'],
    ['c_relinkfront', '    } else {'],
  ...
  python: [
    ...
    ['c_ins', '    def delete_front(self):'],
    ['c_underflow', '        if self.front is None:'],
    ['c_underflow', '            print("Deque Underflow")'],
    ['c_underflow', '            return'],
    ['c_savefront', '        temp = self.front'],
    ['c_advance', '        self.front = self.front.next'],
    ['c_lastcase', '        if self.front is None:'],
    ['c_lastcase', '            self.rear = None'],
    ['c_relinkfront', '        else:'],
    ['c_relinkfront', '            self.front.prev = None'],
  ],
  javascript: [
    ['', 'class Node {'],
    ...
    ['c_ins', '  deleteFront() {'],
    ['c_underflow', '    if (this.front === null) {'],
    ['c_underflow', '      console.log("Deque Underflow");'],
    ['c_underflow', '      return;'],
    ['c_underflow', '    }'],
    ['c_savefront', '    const temp = this.front;'],
    ['c_advance', '    this.front = this.front.next;'],
    ['c_lastcase', '    if (this.front === null) {'],
    ['c_lastcase', '      this.rear = null;'],
    ['c_relinkfront', '    } else {'],
    ['c_relinkfront', '      this.front.prev = null;'],
    ['c_relinkfront', '    }'],
    ['', '  }'], ['', '}'],
  ],
};
...
//                the front pointer and the chain itself still reach it)
...
function buildSteps(values) {
  ...
  if (n === 0) {
    pushStep({
      badge: 'front == null \u2192 Deque Underflow, there is nothing to delete',
  ...
  pushStep({
    badge: 'front != null \u2192 deque is not empty, proceed',
    code: 'c_underflow',
    vars: [frame('main()', [['front', fmt(frontAddr), true], ['rear', fmt(rearAddr), true]])],
  });

  // --- Selection: temp now references the front node, but nothing about
  // the chain has changed yet, so the node stays fully linked and visible.
  tempAddr = frontAddr;
  stateOf[tempAddr] = 'selected';
  pushStep({
    badge: 'temp = front \u2192 node @' + tempAddr + ' is selected (still fully linked in the deque)',
    code: 'c_savefront',
    vars: [frame('main()', [['front', fmt(frontAddr), true], ['rear', fmt(rearAddr), true]]), frame(fnLabel, [['temp', fmt(tempAddr), true]])],
  });

  if (n === 1) {
    // front = front.next -> null. The node is still the value of `rear`,
    // so it is not orphaned yet - keep it selected, just move the front pointer.
    frontAddr = null;
    pushStep({
      badge: 'front = front.next \u2192 front becomes null (it was the only node); temp still holds @' + tempAddr,
      code: 'c_advance',
      vars: [frame('main()', [['front', 'null', true], ['rear', fmt(rearAddr), true]]), frame(fnLabel, [['temp', fmt(tempAddr), true]])],
    });

    // rear = null too - now nothing in the deque references the node, so
    // it becomes truly detached (orphaned, reachable only via temp).
    rearAddr = null;
    stateOf[tempAddr] = 'detached';
    pushStep({
      badge: 'front == null \u2192 rear = null too \u2014 node @' + tempAddr + ' is no longer referenced by front or rear',
      code: 'c_lastcase',
  ...
  // --- n >= 2: front advances, but the chain link to the old front node is
  // untouched so far - it is still reachable by traversal, just no longer
  // the official front. Keep it selected (not yet orphaned).
  const newFrontAddr = nextOf[tempAddr];
  frontAddr = newFrontAddr;
  pushStep({
    badge: 'front = front.next \u2192 front advances to @' + newFrontAddr + '; node @' + tempAddr + ' is still linked via @' + newFrontAddr + '.prev',
    code: 'c_advance',
    vars: [frame('main()', [['front', fmt(newFrontAddr), true], ['rear', fmt(rearAddr), true]]), frame(fnLabel, [['temp', fmt(tempAddr), true]])],
  });

  // --- front.prev = null actually cuts the chain link. Only now is the old
  // front node orphaned (unreachable by traversal, reachable only via temp).
  prevOf[newFrontAddr] = null;
  stateOf[tempAddr] = 'detached';
  pushStep({
    badge: 'front != null \u2192 front.prev = null \u2192 link to node @' + tempAddr + ' is cut; it is now orphaned, reachable only via temp',
    code: 'c_relinkfront',
    vars: [frame('main()', [['front', fmt(newFrontAddr), true], ['rear', fmt(rearAddr), true]]), frame(fnLabel, [['temp', fmt(tempAddr), true]])],
  });
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty deque (Underflow)</td><td>O(1)</td><td>just check <code>front == null</code> and report the error</td></tr>
                <tr><td>Non-empty deque</td><td>O(1)</td><td>the <code>front</code> pointer gives direct access to the node being removed &mdash; no traversal is ever needed</td></tr>
            ...
            <p class="ll-note">
              ...
              traversal. Always check for <b>Deque Underflow</b> (<code>front == null</code>) before
