...
const CODES = {
  java: [
    ...
    ['c_underflow', '        }'],
    ['c_peek', '        // rear.data is the value at the rear'],
    ['c_return', '        return rear.data;'],
    ['', '    }'], ['', '}'],
  ],
  c: [
  ...
  python: [
    ...
    ['c_ins', '    def peek_rear(self):'],
    ['c_underflow', '        if self.rear is None:'],
    ['c_underflow', '            print("Deque is empty")'],
    ['c_underflow', '            return None'],
    ['c_peek', '        # self.rear.data is the value at the rear'],
    ['c_return', '        return self.rear.data'],
  ],
  javascript: [
    ['', 'class Node {'],
    ...
    ['c_underflow', '    }'],
    ['c_peek', '    // this.rear.data is the value at the rear'],
    ['c_return', '    return this.rear.data;'],
    ['', '  }'], ['', '}'],
  ],
};
...
const PSEUDOCODE = [
  ...
  '        return null',
  '    return rear.data   // deque is left unchanged',
];
...
function buildSteps(values) {
  ...
  if (n === 0) {
    steps.push({
      nodes: [], frontAddr: null, rearAddr: null, invalid: true,
      badge: 'rear == null \u2192 the deque is empty, nothing to peek',
      code: 'c_underflow',
  ...
  steps.push({
    nodes: [...nodes0], frontAddr: frontAddr0, rearAddr: rearAddr0,
    badge: 'Inspecting rear node @' + rearAddr0 + ' (value = ' + nodes0[n - 1].val + ') \u2014 nothing is modified',
    code: 'c_peek',
    curAddr: rearAddr0,
    vars: [frame('main()', [['front', fmt(frontAddr0), true], ['rear', fmt(rearAddr0), true]]), frame(fnLabel, [])],
  });

  steps.push({
    nodes: [...nodes0], frontAddr: frontAddr0, rearAddr: rearAddr0,
    badge: 'return rear.data \u2192 ' + nodes0[n - 1].val + ' returned, deque unchanged (' + n + ' node(s))',
    code: 'c_return',
    curAddr: rearAddr0,
    vars: [frame('main()', [['front', fmt(frontAddr0), true], ['rear', fmt(rearAddr0), true]]), frame(fnLabel, [['returns', nodes0[n - 1].val, false]])],
  });
...
function nodeLabel(n) {
  ...
  if (n.addr === s.value.curAddr && n.addr !== s.value.frontAddr && n.addr !== s.value.rearAddr) labels.push('peek \u2192');
  return labels.join(' ');
}
    ...
    <div class="ll-main" ref="mainRef">
      <div class="ll-left-col" ref="leftColRef" :style="{ width: leftWidth + '%' }">
          ...
          <div class="ll-perm-area">
            <div class="ll-ptrs">
              ...
              <div v-if="s.curAddr !== undefined" class="ll-ptr-chip">
                peek = <b class="ll-c-orange">@{{ s.curAddr }}</b>
              </div>
            </div>
            ...
            <div class="ll-stage">
              <template v-for="(n, idx) in s.nodes" :key="n.addr">
                <div class="ll-nwrap">
                  <div class="ll-who" :class="{ 'll-c-orange': n.addr === s.curAddr }">{{ nodeLabel(n) }}</div>
        ...
        <div class="ll-legend">
          ...
          <span class="ll-leg"><span class="ll-legdot ll-legdot-cur"></span>node being inspected (peek)</span>
        </div>
        ...
        <div class="ll-code-panel">
          ...
          <div v-else class="ll-info-scroll">
            ...
            <table class="ll-complexity-table">
              ...
              <tbody>
                <tr><td>Empty deque</td><td>O(1)</td><td>just check <code>rear == null</code> and report that the deque is empty</td></tr>
                <tr><td>Non-empty deque</td><td>O(1)</td><td>the <code>rear</code> pointer gives direct access to the value &mdash; no traversal is ever needed</td></tr>
                <tr><td>Space</td><td>O(1)</td><td>no extra memory is used; no node is created or removed</td></tr>
              </tbody>
            </table>
            <p class="ll-note">
              Key observation: <b>peekRear()</b> is a <b>read-only</b> operation &mdash; it only
              inspects <code>rear.data</code> and never modifies <code>front</code>,
              <code>rear</code>, or any node's <code>prev</code>/<code>next</code> pointers. The deque's
              structure and size are exactly the same before and after the call. This is different from
              <b>deleteRear()</b>, which also touches the rear but actually unlinks and removes the node.
              As always, check for an empty deque first &mdash; peeking a <code>null</code> rear has
...
.ll-stage { display: flex; align-items: flex-start; flex-wrap: nowrap; overflow-x: auto; overflow-y: hidden; scrollbar-width: thin; padding: 4px 16px 14px; min-height: 80px; gap: 6px 6px; width: 100%; box-sizing: border-box; min-width: 0; }
.ll-nwrap { display: flex; flex-direction: column; align-items: center; flex-shrink: 0; }
...
.ll-box { display: flex; border: 2px solid var(--blue); border-radius: var(--radius-sm); overflow: hidden; background: var(--node); width: 90px; flex-shrink: 0; color: #fff; animation: ll-pop .3s ease; box-shadow: var(--shadow-sm); }
